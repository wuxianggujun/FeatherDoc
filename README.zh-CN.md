...
`output/project-template-delivery-readiness/summary.json`，以及自动发现根目录下的
`*.restore-audit.summary.json`。它会调用
...
restore-audit `open_command` 等辅助命令；`governance_detail_source` 会标明顶层治理
...
blocker 和 action item 转成带 action、source schema、source report、composite id
以及 `command` / `open_command` / `audit_command` 指引的可勾选处理项。
...
时也会继续保留这些辅助命令。
当使用 `-InputRoot` 扫描目录时，rollup 也会自动纳入
`*.restore-audit.summary.json` 这类 style-merge restore audit 产物；release candidate
auto-discovery 和 release governance pipeline 也会把这些 summary 带进最终 blocker rollup。
release candidate 的 `report/summary.json` 和 step summary 也会保留 rollup / handoff
里的 blocker 明细、action helper commands 和 warnings，reviewer 不必打开嵌套 JSON
也能定位具体阻断项。
release governance pipeline 的 stage summary 和 Markdown 也会继续保留 blocker 明细和这些
action helper commands，便于直接从 pipeline 总览执行 restore-audit 复核入口。
