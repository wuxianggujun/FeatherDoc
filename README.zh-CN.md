...
`delete_table`；表格结构别名包括 `insert_table_before`、
`insert_table_like_before`、`merge_table_cell`、`unmerge_table_cells` 和
`unmerge_table_cell`；
...
`clear_paragraph_horizontal_alignment` 是显式清除对齐入口；`set_paragraph_line_spacing`
可作为段落间距入口，`clear_paragraph_spacing` 会移除直接段落间距；
...
`clear_table_position`。`set_table_position` 支持
`horizontal_spec` / `horizontal_position_spec` 透传到 `--horizontal-spec`，
也支持 `vertical_spec` / `vertical_position_spec` 透传到 `--vertical-spec`；
水平 spec 取值为 `left`、`center`、`right`、`inside`、`outside`，
垂直 spec 取值为 `top`、`center`、`bottom`、`inside`、`outside`。
需要回放 `plan-table-position-presets` 生成的 JSON
...
`manual_review_recommendations`，并用
`featherdoc.project_template_onboarding_summary.v1` / `summary_schema_version = 1`
标记 summary 契约，用于快速判断 schema approval 是否阻断发布、应先复核
schema update candidate 还是补齐 render-data。
...
- 自定义表格样式定义已经有第一版 typed API：`ensure_table_style(...)`
  ...
  `w:tblpPr` 浮动定位，覆盖水平 / 垂直参照、twips 偏移、Word 原生
  水平 / 垂直 spec（`tblpXSpec` / `tblpYSpec`）、环绕距离、重叠策略、
  常用 preset、多表批量目标、plan/apply 回放和 Word 渲染可视化验证。
