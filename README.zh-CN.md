...
`apply_style_refactor` 和 `prune_unused_styles` 进入 edit-plan。
`rename_style` 接受 `old_style_id` / `old_style` 和 `new_style_id` /
`new_style`；`merge_style` 接受 `source_style_id` / `source_style` 和
`target_style_id` / `target_style`；`apply_style_refactor` 接受 `plan_file`
或内联 `renames` / `merges` pair，并可选写出 `rollback_plan`。
