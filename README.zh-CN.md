...
- 支持模板 schema 校验 / 导出、书签填充、content control schema slot 与按 tag / alias 的纯文本替换 API
...
`release_body.zh-CN.md` 和 `release_summary.zh-CN.md` 也会同步列出 smoke、
fixed-grid、section/page-number 和 curated visual verdict。
...
- 做模板填充、书签校验或 content control 检查 / 替换：`list_bookmarks()`、`validate_template()`、`fill_bookmarks()`、`replace_bookmark_with_*()`、`body_template()` / `header_template()` / `footer_template()`、`list_content_controls()`、`find_content_controls_by_*()`、`replace_content_control_text_by_*()`
...
- 书签填充、模板表格扩展、条件块显隐
- content control 的枚举、tag / alias 过滤，以及 C++ API / CLI 层的纯文本替换
- 内联图片与浮动图片
- 页眉、页脚、分节复制 / 插入 / 移动 / 删除
...
- 现在已经可以做 content control 的基础枚举与 `--tag` / `--alias` 过滤（`list_content_controls` / `inspect-content-controls`），也可以通过 `replace_content_control_text_by_tag(...)` / `replace_content_control_text_by_alias(...)` 在 `Document` 或 `TemplatePart` 层做纯文本替换，并可用 `replace-content-control-text` 做 CLI 一次性替换；也已经纳入 template schema 的导出 / 校验；后续还缺结构化内容替换
