...
stage 级 `action_items`，并在 Markdown 中按 stage 展示 restore-audit `open_command`
等辅助命令；该脚本只读输入 summary，不重跑 CLI、CMake、Word 或视觉自动化。
...
release governance pipeline 的 stage summary 和 Markdown 也会继续保留这些 action
helper commands，便于直接从 pipeline 总览执行 restore-audit 复核入口。
