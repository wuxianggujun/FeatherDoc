...
- Tables can now be appended, extended structurally, given explicit cell,
  ...
  expose `pdf_floating_table_support` evidence with
  `pdf_floating_table_supported_geometry_percent` and
  `pdf_floating_table_tracked_geometry_count`, keeping the stable PDF geometry
  subset separate from metadata-only `tblpPr` wrapping scope. Release material
  handoff also carries `pdf_floating_table_reviewer_focus` so reviewers know
  where metadata-only placement still needs attention.
...
- Paragraph and run style references can now be attached and cleared, and a
  ...
  (alias `--plan-only`) can audit the restore plan without saving changes, `--entry` can be repeated to select multiple rollback entries, `--source-style` / `--target-style` can filter merge rollback entries by style id, restore audits expose `selection_summary`, `requested_count`, `review_handoff_step_count`, `review_handoff_steps` with `copy_command`, `next_handoff_step`, `next_copy_command`, `next_step_reason`, top-level `issue_review_commands` / `issue_review_command_count` / `issue_review_group_summary` / `first_issue_review_command` / `copy_issue_review_command`, `handoff_status_summary`, `rollback_plan_summary`, `source_schema`, `source_report_display`, `source_json_display`, and `rollback_plan_display`, `issue_summary_group_count`, `issue_summary_groups`, `issue_review_groups` with `first_review_command` / `copy_review_command`, `status_reason`, `minimum_risk_next_action`, `minimum_risk_next_action_command`, `selected_restore_command_template`, `per_entry_dry_run_commands`, `per_entry_restore_command_templates`, `batch_restorable_dry_run_command`, and `batch_restorable_restore_command_template`; release governance action/blocker entries also carry `source_schema`, `source_report_display`, `source_json_display`, `repair_strategy`, the same `review_handoff_steps`, `next_handoff_step`, `next_copy_command`, `next_step_reason`, `issue_review_commands`, `issue_review_command_count`, `issue_review_group_summary`, `first_issue_review_command`, `copy_issue_review_command`, `handoff_status_summary`, and `rollback_plan_summary`, and restore issues include actionable `suggestion` text plus top-level `issue_count` / `issue_summary`, while
