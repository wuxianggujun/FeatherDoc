...
- Paragraph and run style references can now be attached and cleared, and a
  ...
  (alias `--plan-only`) can audit the restore plan without saving changes, `--entry` can be repeated to select multiple rollback entries, `--source-style` / `--target-style` can filter merge rollback entries by style id, restore audits expose `selection_summary`, `requested_count`, `issue_summary_group_count`, `issue_summary_groups`, `status_reason`, `minimum_risk_next_action`, and `minimum_risk_next_action_command`; release governance action/blocker entries also carry `source_schema`, `source_report_display`, `source_json_display`, and `repair_strategy`, and restore issues include actionable `suggestion` text plus top-level `issue_count` / `issue_summary`, while
