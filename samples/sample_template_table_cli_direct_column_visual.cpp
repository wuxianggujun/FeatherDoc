...
bool seed_header_insert_before(featherdoc::Document &doc) {
    ...
    if (!require_step(configure_table(target_table, {1800U, 3600U}),
                      "configure direct header target")) {
        return false;
    }
    ...
    return append_body_context(
        doc,
        "The direct-header insert-before sample keeps body text below the header area.",
        "After the purple direct header table gains a new leading column, the retained blue table and the body text should stay stable.");
}

bool seed_footer_insert_after(featherdoc::Document &doc) {
    auto &footer_paragraph = doc.ensure_footer_paragraphs();
    if (!footer_paragraph.has_next() ||
        !footer_paragraph.set_text(
            "Template table CLI direct footer column baseline")) {
        return false;
    }
    ...
    if (!require_step(configure_table(target_table, {1600U, 2800U}),
                      "configure direct footer target")) {
        return false;
    }
    ...
    return append_body_context(
        doc,
        "The direct-footer insert-after sample keeps the body paragraphs above the footer area.",
        "After the orange direct footer table gains a new middle column, it should stay inside the footer without colliding with body text.");
}
...
bool seed_body_remove_column(featherdoc::Document &doc) {
    ...
    if (!paragraph.has_next() ||
        !paragraph.set_text(
            "Template table CLI direct-part remove-column body baseline")) {
        return false;
    }
...
bool seed_case(featherdoc::Document &doc, std::string_view case_id) {
    if (case_id == "header-insert-before") {
        return seed_header_insert_before(doc);
    }
    if (case_id == "footer-insert-after") {
        return seed_footer_insert_after(doc);
    }
...
int main(int argc, char **argv) {
    if (argc < 3) {
        std::cerr << "usage: sample_template_table_cli_direct_column_visual "
                     "<case-id> <output.docx>\n";
        return 2;
    }
