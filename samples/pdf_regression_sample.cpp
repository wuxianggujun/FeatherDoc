...
[[nodiscard]] bool define_document_cjk_style_overlay_styles(
    featherdoc::Document &document) {
    if (!define_document_cjk_font_embed_styles(document)) {
        return false;
    }

    auto superscript = featherdoc::character_style_definition{};
    superscript.name = "Document PDF CJK Style Overlay Superscript";
    superscript.run_font_family = std::string{"Helvetica"};
    superscript.run_east_asia_font_family =
        std::string{"Document CJK Font Embed"};
    ...
    subscript.name = "Document PDF CJK Style Overlay Subscript";
    ...
    strike.name = "Document PDF CJK Style Overlay Strike";
    strike.run_font_family = std::string{"Helvetica"};
    strike.run_east_asia_font_family = std::string{"Document CJK Font Embed"};
...
build_document_cjk_multi_anchor_table_flow_text_sample(
    ...
    const auto image_path = write_quadrant_rgb_png(
        asset_dir,
        "featherdoc-pdf-regression-document-cjk-multi-anchor-table-flow.png");
    ...
    if (!intro.has_next() ||
        ...
        !add_styled_contract_run(document, intro, utf8_from_u8(u8"连续文档流"),
                                 "DocumentPdfCjkFontEmbedAccent") ||
        !intro.add_run(" / FE-MA-901 / ").has_next() ||
        !add_styled_contract_run(document, intro, utf8_from_u8(u8"锚点切换矩阵"),
                                 "DocumentPdfCjkFontEmbedNote")) {
        return sample;
    }
    ...
    auto even_footer = document.ensure_section_footer_paragraphs(
        0U, featherdoc::section_reference_kind::even_page);
    if (!default_header.has_next() ||
        !default_header.set_text("Multi flow header MA-303 page {{page}}") ||
        !default_footer.has_next() ||
        !default_footer.set_text("Multi flow footer {{page}} / {{total_pages}}") ||
        !first_header.has_next() ||
        !first_header.set_text("Multi first header MA-101 page {{page}}") ||
        !first_footer.has_next() ||
        !first_footer.set_text("Multi first footer {{page}} / {{total_pages}}") ||
        !even_header.has_next() ||
        !even_header.set_text("Multi even header MA-202 page {{page}}") ||
        !even_footer.has_next() ||
        !even_footer.set_text("Multi even footer {{page}} / {{total_pages}}")) {
        return sample;
    }
    ...
    if (!append_document_text_paragraph(
            document,
            utf8_from_u8(
                u8"起始段落先验证内联图片后的整栏正文、复制搜索与页码占位符展开，再进入多锚点连续流场景。")) ||
        !document.append_image(image_path, 96U, 42U) ||
        !append_document_text_paragraph(
            document,
            utf8_from_u8(
                u8"内联图片之后必须立即恢复全宽正文，避免后续表格、裁剪图和页眉页脚继续沿用错误回流边界。"))) {
        return sample;
    }
    ...
    if (!stripe.has_next() ||
        ...
        !add_styled_contract_run(document, stripe, utf8_from_u8(u8"锚点切换"),
                                 "DocumentPdfCjkFontEmbedAccent") ||
        !stripe.add_run(" / ").has_next() ||
        !add_styled_contract_run(document, stripe, utf8_from_u8(u8"样式叠加"),
                                 "DocumentPdfCjkFontEmbedNote")) {
        return sample;
    }
    ...
    auto style_matrix = append_document_paragraph(document, "");
    if (!style_matrix.has_next() ||
        !style_matrix.add_run("Style matrix: ").has_next() ||
        !add_styled_contract_run(document, style_matrix,
                                 utf8_from_u8(u8"注记回读"),
                                 "DocumentPdfCjkFontEmbedNote") ||
        !style_matrix.add_run(" / ").has_next() ||
        !add_styled_contract_run(document, style_matrix,
                                 utf8_from_u8(u8"合同矩阵"),
                                 "DocumentPdfCjkFontEmbedAccent") ||
        !style_matrix.add_run(" / ").has_next() ||
        !add_styled_contract_run(document, style_matrix,
                                 utf8_from_u8(u8"大字嵌字"),
    ...
    const auto append_flow_paragraph = [&](int index, std::u8string_view label) {
        const auto text =
            utf8_from_u8(u8"第 ") + std::to_string(index) +
            utf8_from_u8(u8" 组连续流验证：") + utf8_from_u8(label) +
            utf8_from_u8(
                u8"、复制搜索、页码展开、字宽回读与样式叠加保持一致，混排 English token FE-MA-") +
            std::to_string(920 + index) +
            utf8_from_u8(
                u8"，并确认图片或表格切换后正文仍然稳定，不会出现错误缩进或残留环绕边界。");
        return append_document_text_paragraph(document, text);
    };
    ...
    left_square_options.wrap_mode =
        featherdoc::floating_image_wrap_mode::square;
    left_square_options.wrap_distance_right_px = 16U;
    left_square_options.wrap_distance_bottom_px = 8U;
    left_square_options.crop =
        featherdoc::floating_image_crop{180U, 20U, 60U, 120U};
    if (!document.append_floating_image(image_path, 110U, 82U,
                                        left_square_options)) {
        return sample;
    }

    for (int index = 1; index <= 4; ++index) {
        if (!append_flow_paragraph(index, u8"左侧裁剪包围")) {
            return sample;
        }
    }
    ...
    auto table_a = document.append_table(table_a_row_count, 3U);
    if (!table_a.has_next() || !table_a.set_width_twips(7200U) ||
        !table_a.set_column_width_twips(0U, 1540U) ||
        !table_a.set_column_width_twips(1U, 1760U) ||
        !table_a.set_column_width_twips(2U, 3900U) ||
        !table_a.set_cell_text(0U, 0U, utf8_from_u8(u8"串联看板")) ||
        !table_a.set_cell_text(1U, 0U, utf8_from_u8(u8"阶段")) ||
        !table_a.set_cell_text(1U, 1U, utf8_from_u8(u8"锚点")) ||
        !table_a.set_cell_text(1U, 2U, utf8_from_u8(u8"说明")) ||
        !table_a.set_cell_text(2U, 0U, "MA-A-01") ||
        !table_a.set_cell_text(2U, 1U, utf8_from_u8(u8"整栏恢复")) ||
        !table_a.set_cell_text(
            2U, 2U,
            utf8_from_u8(u8"内联图片后先恢复全宽正文，再进入左侧裁剪包围与中英混排验证。")) ||
        !table_a.set_cell_text(3U, 0U, "MA-A-02") ||
        !table_a.set_cell_text(3U, 1U, utf8_from_u8(u8"左侧包围")) ||
        !table_a.set_cell_text(
            3U, 2U,
            utf8_from_u8(u8"图像裁剪后正文仍需保持可搜索，不能因为图片包围丢掉中文文本层。")) ||
        !table_a.set_cell_text(4U, 0U, "MA-A-03") ||
        !table_a.set_cell_text(4U, 1U, utf8_from_u8(u8"分页接力")) ||
        !table_a.set_cell_text(
            4U, 2U,
            utf8_from_u8(u8"重复表头跨页后继续可检索，并且页眉页脚分页变体要和正文页数一致。")) ||
        !table_a.set_cell_text(5U, 0U, "MA-A-04") ||
        !table_a.set_cell_text(5U, 1U, utf8_from_u8(u8"禁拆块")) ||
        !table_a.set_cell_text(
            5U, 2U,
            utf8_from_u8(u8"这行刻意放长，并要求整块迁移，观察段落、表格与图片之间的紧余量分页。")) ||
        !table_a.set_cell_text(6U, 0U, "MA-A-05") ||
        !table_a.set_cell_text(6U, 1U, utf8_from_u8(u8"样式回读")) ||
        !table_a.set_cell_text(
            6U, 2U,
            utf8_from_u8(u8"Accent、Note 与 Large 混排后，复制搜索与视觉渲染都要保持一致。")) ||
        !table_a.set_cell_text(7U, 0U, "MA-A-06") ||
        !table_a.set_cell_text(7U, 1U, utf8_from_u8(u8"收口检查")) ||
        !table_a.set_cell_text(
            7U, 2U,
            utf8_from_u8(u8"表格结束后正文必须从表格下方恢复，不得继续继承左侧图片的包围边界。"))) {
        return sample;
    }

    auto table_a_banner = table_a.find_cell(0U, 0U);
    auto table_a_heading_stage = table_a.find_cell(1U, 0U);
    auto table_a_heading_anchor = table_a.find_cell(1U, 1U);
    auto table_a_heading_note = table_a.find_cell(1U, 2U);
    if (!table_a_banner.has_value() ||
        !table_a_heading_stage.has_value() ||
        !table_a_heading_anchor.has_value() ||
        !table_a_heading_note.has_value() ||
        !table_a_banner->merge_right(2U)) {
        return sample;
    }
    if (!table_a_banner->set_fill_color("D9EAF7") ||
        !table_a_heading_stage->set_fill_color("EAF2F8") ||
        !table_a_heading_anchor->set_fill_color("EAF2F8") ||
        !table_a_heading_note->set_fill_color("EAF2F8")) {
        return sample;
    }
    ...
    auto checkpoint_a = append_document_paragraph(document, "");
    if (!checkpoint_a.has_next() ||
        !checkpoint_a.add_run("Anchor checkpoint A: ").has_next() ||
        !add_styled_contract_run(document, checkpoint_a,
                                 utf8_from_u8(u8"表格收口"),
                                 "DocumentPdfCjkFontEmbedAccent") ||
        !checkpoint_a.add_run(" / FE-MA-951 / ").has_next() ||
        !add_styled_contract_run(document, checkpoint_a,
                                 utf8_from_u8(u8"上中下锚点"),
                                 "DocumentPdfCjkFontEmbedNote")) {
        return sample;
    }
    ...
    top_bottom_options.horizontal_reference =
        featherdoc::floating_image_horizontal_reference::margin;
    top_bottom_options.horizontal_offset_px = 64;
    top_bottom_options.vertical_reference =
        featherdoc::floating_image_vertical_reference::paragraph;
    top_bottom_options.vertical_offset_px = 0;
    top_bottom_options.wrap_mode =
    ...
    top_bottom_options.wrap_distance_bottom_px = 10U;
    top_bottom_options.crop =
        featherdoc::floating_image_crop{0U, 120U, 140U, 20U};
    if (!document.append_floating_image(image_path, 126U, 56U,
                                        top_bottom_options)) {
        return sample;
    }

    for (int index = 5; index <= 8; ++index) {
        if (!append_flow_paragraph(index, u8"顶底切换回流")) {
            return sample;
        }
    }

    constexpr std::size_t table_b_row_count = 8U;
    auto table_b = document.append_table(table_b_row_count, 3U);
    if (!table_b.has_next() || !table_b.set_width_twips(7000U) ||
        !table_b.set_column_width_twips(0U, 1500U) ||
        !table_b.set_column_width_twips(1U, 1700U) ||
        !table_b.set_column_width_twips(2U, 3800U) ||
        !table_b.set_cell_text(0U, 0U, utf8_from_u8(u8"切换矩阵")) ||
        !table_b.set_cell_text(1U, 0U, utf8_from_u8(u8"节点")) ||
        !table_b.set_cell_text(1U, 1U, utf8_from_u8(u8"锚点")) ||
        !table_b.set_cell_text(1U, 2U, utf8_from_u8(u8"说明")) ||
        !table_b.set_cell_text(2U, 0U, "MA-B-01") ||
        !table_b.set_cell_text(2U, 1U, utf8_from_u8(u8"顶底切换")) ||
        !table_b.set_cell_text(
            2U, 2U,
            utf8_from_u8(u8"图片切到顶底环绕后，正文需要在图片上下恢复完整宽度。")) ||
        !table_b.set_cell_text(3U, 0U, "MA-B-02") ||
        !table_b.set_cell_text(3U, 1U, utf8_from_u8(u8"右侧锚点")) ||
        !table_b.set_cell_text(
            3U, 2U,
            utf8_from_u8(u8"右侧锚点的第一行说明承担样式叠加、复制搜索和页码回读验证。")) ||
        !table_b.set_cell_text(4U, 0U, "MA-B-03") ||
        !table_b.set_cell_text(
            4U, 2U,
            utf8_from_u8(u8"第二行说明继续跟随同一个右侧锚点块，验证合并单元格与段落回流联动。")) ||
        !table_b.set_cell_text(5U, 0U, "MA-B-04") ||
        !table_b.set_cell_text(5U, 1U, utf8_from_u8(u8"禁拆落页")) ||
        !table_b.set_cell_text(
            5U, 2U,
            utf8_from_u8(u8"禁拆块在紧余量情况下应整块迁移，不能只把底部行残留在上一页。")) ||
        !table_b.set_cell_text(6U, 0U, "MA-B-05") ||
        !table_b.set_cell_text(6U, 1U, utf8_from_u8(u8"样式复核")) ||
        !table_b.set_cell_text(
            6U, 2U,
            utf8_from_u8(u8"中英混排与大字嵌字在表后继续稳定，视觉和文本层都要能回读。")) ||
        !table_b.set_cell_text(7U, 0U, "MA-B-06") ||
        !table_b.set_cell_text(7U, 1U, utf8_from_u8(u8"终页恢复")) ||
        !table_b.set_cell_text(
            7U, 2U,
            utf8_from_u8(u8"末页需要在图片下方恢复整栏正文，避免右侧锚点把收口段持续挤压。"))) {
        return sample;
    }
    ...
    auto table_b_heading_anchor = table_b.find_cell(1U, 1U);
    auto table_b_heading_note = table_b.find_cell(1U, 2U);
    auto table_b_merged_anchor = table_b.find_cell(3U, 1U);
    if (!table_b_banner.has_value() ||
        !table_b_heading_case.has_value() ||
        !table_b_heading_anchor.has_value() ||
        !table_b_heading_note.has_value() ||
        !table_b_merged_anchor.has_value() ||
        !table_b_banner->merge_right(2U) ||
        !table_b_merged_anchor->merge_down(1U)) {
        return sample;
    }
    if (!table_b_banner->set_fill_color("E2F0D9") ||
        !table_b_heading_case->set_fill_color("F2F8EC") ||
        !table_b_heading_anchor->set_fill_color("F2F8EC") ||
        !table_b_heading_note->set_fill_color("F2F8EC") ||
        !table_b_merged_anchor->set_fill_color("D9EAF7") ||
        !table_b_merged_anchor->set_vertical_alignment(
            featherdoc::cell_vertical_alignment::center)) {
        return sample;
    }
    ...
    for (std::size_t row_index = 0U; row_index < table_b_row_count;
        ...
        if (row_index < 2U) {
            ...
            if (!table_b_row.set_height_twips(
                    620U, featherdoc::row_height_rule::exact)) {
                return sample;
            }
        } else if (row_index == 4U) {
            if (!table_b_row.set_height_twips(
                    560U, featherdoc::row_height_rule::exact)) {
                return sample;
            }
        } else if (row_index == 5U) {
            if (!table_b_row.set_cant_split() ||
                !table_b_row.set_height_twips(
                    780U, featherdoc::row_height_rule::at_least)) {
                return sample;
            }
        } else if (!table_b_row.set_height_twips(
                       620U, featherdoc::row_height_rule::at_least)) {
    ...
    auto checkpoint_b = append_document_paragraph(document, "");
    if (!checkpoint_b.has_next() ||
        !checkpoint_b.add_run("Anchor checkpoint B: ").has_next() ||
        !add_styled_contract_run(document, checkpoint_b,
                                 utf8_from_u8(u8"终页恢复"),
                                 "DocumentPdfCjkFontEmbedAccent") ||
        !checkpoint_b.add_run(" / FE-MA-971 / ").has_next() ||
        !add_styled_contract_run(document, checkpoint_b,
                                 utf8_from_u8(u8"右侧收口"),
                                 "DocumentPdfCjkFontEmbedNote") ||
        !checkpoint_b.add_run(" / ").has_next() ||
        !add_styled_contract_run(document, checkpoint_b,
                                 utf8_from_u8(u8"大字复核"),
                                 "DocumentPdfCjkFontEmbedLarge")) {
        return sample;
    }
    ...
    right_square_options.horizontal_reference =
        featherdoc::floating_image_horizontal_reference::margin;
    right_square_options.horizontal_offset_px = 224;
    right_square_options.vertical_reference =
        featherdoc::floating_image_vertical_reference::paragraph;
    right_square_options.vertical_offset_px = 0;
    right_square_options.wrap_mode =
    ...
    right_square_options.wrap_distance_bottom_px = 8U;
    right_square_options.crop =
        featherdoc::floating_image_crop{40U, 60U, 160U, 40U};
    if (!document.append_floating_image(image_path, 114U, 92U,
                                        right_square_options)) {
        return sample;
    }

    for (int index = 9; index <= 12; ++index) {
        if (!append_flow_paragraph(index, u8"右侧锚点收口")) {
            return sample;
        }
    }
    ...
    auto settle = append_document_paragraph(document, "");
    if (!settle.has_next() ||
        !settle.add_run("Anchor settle: ").has_next() ||
        !add_styled_contract_run(document, settle,
                                 utf8_from_u8(u8"恢复全宽"),
                                 "DocumentPdfCjkFontEmbedAccent") ||
        !settle.add_run(" / FE-MA-981 / ").has_next() ||
        !add_styled_contract_run(document, settle,
    ...
    if (!append_document_text_paragraph(
            document,
            utf8_from_u8(u8"补线段落一：这里必须在右侧图片下方恢复整栏正文，不能让收口段继续贴着图片窄列流动。")) ||
        !append_document_text_paragraph(
            document,
            utf8_from_u8(u8"补线段落二：继续验证末页 copy/search、合同检索、字宽回读与页脚总页数展开保持一致。"))) {
        return sample;
    }
    ...
    if (!closing.has_next() ||
        ...
        !add_styled_contract_run(document, closing, utf8_from_u8(u8"终页合同"),
                                 "DocumentPdfCjkFontEmbedAccent") ||
        !closing.add_run(" / FE-MA-999 / ").has_next() ||
        !add_styled_contract_run(document, closing, utf8_from_u8(u8"回读复检"),
                                 "DocumentPdfCjkFontEmbedLarge")) {
        return sample;
    }
    ...
    options.render_inline_images = true;
    options.header_footer_font_size_points = 8.0;
    options.margin_left_points = 42.0;
    options.margin_right_points = 42.0;
    options.margin_top_points = 42.0;
    options.margin_bottom_points = 46.0;
...
build_document_cjk_anchor_font_matrix_boundary_text_sample(
    ...
    const auto image_path = write_quadrant_rgb_png(
        asset_dir,
        "featherdoc-pdf-regression-document-cjk-anchor-font-matrix-boundary.png");
    ...
    if (!intro.has_next() ||
        ...
        !add_styled_contract_run(document, intro, utf8_from_u8(u8"跨界锚点"),
                                 "DocumentPdfCjkFontEmbedAccent") ||
        !intro.add_run(" / FE-AM-901 / ").has_next() ||
        !add_styled_contract_run(document, intro, utf8_from_u8(u8"字体密度矩阵"),
                                 "DocumentPdfCjkFontEmbedNote")) {
        return sample;
    }
    ...
    if (!default_header.has_next() ||
        ...
        !default_footer.has_next() ||
        !default_footer.set_text("Anchor matrix footer {{page}} / {{total_pages}}") ||
        !first_header.has_next() ||
        !first_header.set_text("Anchor matrix first header AM-101 page {{page}}") ||
        !first_footer.has_next() ||
        !first_footer.set_text(
            "Anchor matrix first footer {{page}} / {{total_pages}}") ||
        !even_header.has_next() ||
        !even_header.set_text("Anchor matrix even header AM-202 page {{page}}") ||
        !even_footer.has_next() ||
        !even_footer.set_text(
    ...
    if (!append_document_text_paragraph(
            document,
            utf8_from_u8(
                u8"起始段落先验证内联图片后的整栏正文，再把锚点切换、字体矩阵、复制搜索与分页变体放进同一个连续流里。")) ||
        !document.append_image(image_path, 96U, 40U) ||
        !append_document_text_paragraph(
            document,
            utf8_from_u8(
                u8"内联图片之后必须立即恢复全宽正文，避免后续表格和锚点切换继承错误缩进或错误回流边界。"))) {
        return sample;
    }
    ...
    if (!stripe.has_next() ||
        ...
        !add_styled_contract_run(document, stripe, utf8_from_u8(u8"字号联测"),
                                 "DocumentPdfCjkFontEmbedAccent") ||
        !stripe.add_run(" / ").has_next() ||
        !add_styled_contract_run(document, stripe, utf8_from_u8(u8"检索密排"),
                                 "DocumentPdfCjkFontEmbedNote")) {
        return sample;
    }
    ...
    if (!ladder.has_next() || !ladder.add_run("Run ladder: ").has_next() ||
        !add_styled_contract_run(document, ladder, utf8_from_u8(u8"注记回读"),
                                 "DocumentPdfCjkFontEmbedNote") ||
        !ladder.add_run(" / ").has_next() ||
        !add_styled_contract_run(document, ladder, utf8_from_u8(u8"合同矩阵"),
                                 "DocumentPdfCjkFontEmbedAccent") ||
        !ladder.add_run(" / ").has_next() ||
        !add_styled_contract_run(document, ladder, utf8_from_u8(u8"大字嵌字"),
                                 "DocumentPdfCjkFontEmbedLarge")) {
        return sample;
    }

    const auto append_matrix_paragraph = [&](int index,
                                             std::u8string_view label) {
        auto paragraph = append_document_paragraph(document, "");
        const auto prefix = utf8_from_u8(u8"第 ") + std::to_string(index) +
                            utf8_from_u8(u8" 组边界矩阵验证：");
        const auto token = " / token FE-AM-" + std::to_string(920 + index) +
                           " / ";
        const auto suffix = utf8_from_u8(
            u8"，并确认表格边界、图片裁剪与多字号回读都保持一致，不会出现丢字、重叠或残留窄列。");
    ...
    left_square_options.wrap_distance_bottom_px = 8U;
    left_square_options.crop =
        featherdoc::floating_image_crop{180U, 0U, 50U, 130U};
    if (!document.append_floating_image(image_path, 112U, 84U,
                                        left_square_options)) {
        return sample;
    }

    for (int index = 1; index <= 3; ++index) {
        if (!append_matrix_paragraph(index, u8"左侧裁剪密排")) {
            return sample;
        }
    }

    constexpr std::size_t table_a_row_count = 8U;
    auto table_a = document.append_table(table_a_row_count, 4U);
    if (!table_a.has_next() || !table_a.set_width_twips(7200U) ||
        !table_a.set_column_width_twips(0U, 1180U) ||
        !table_a.set_column_width_twips(1U, 1200U) ||
        !table_a.set_column_width_twips(2U, 1480U) ||
        !table_a.set_column_width_twips(3U, 3340U) ||
        !table_a.set_cell_text(0U, 0U, utf8_from_u8(u8"锚点矩阵表")) ||
        !table_a.set_cell_text(1U, 0U, utf8_from_u8(u8"区域")) ||
        !table_a.set_cell_text(1U, 1U, utf8_from_u8(u8"字号")) ||
        !table_a.set_cell_text(1U, 2U, utf8_from_u8(u8"检索键")) ||
        !table_a.set_cell_text(1U, 3U, utf8_from_u8(u8"说明")) ||
        !table_a.set_cell_text(2U, 0U, "AM-A-01") ||
        !table_a.set_cell_text(2U, 1U, utf8_from_u8(u8"小字")) ||
        !table_a.set_cell_text(2U, 2U, utf8_from_u8(u8"字宽校验")) ||
        !table_a.set_cell_text(
            2U, 3U,
            utf8_from_u8(u8"左侧裁剪包围后，小字号中文与英文 token 仍需保持稳定复制与搜索。")) ||
        !table_a.set_cell_text(3U, 0U, "AM-A-02") ||
        !table_a.set_cell_text(3U, 1U, utf8_from_u8(u8"常规")) ||
        !table_a.set_cell_text(3U, 2U, utf8_from_u8(u8"复制回读")) ||
        !table_a.set_cell_text(
            3U, 3U,
            utf8_from_u8(u8"正文与表格切换时，复制搜索结果不能分裂，也不能把中英混排拆散。")) ||
        !table_a.set_cell_text(4U, 0U, "AM-A-03") ||
        !table_a.set_cell_text(4U, 1U, utf8_from_u8(u8"大字")) ||
        !table_a.set_cell_text(4U, 2U, utf8_from_u8(u8"终页嵌字")) ||
        !table_a.set_cell_text(
            4U, 3U,
            utf8_from_u8(u8"Large run 与页码占位符共存时，文本层和视觉渲染都需要稳定回读。")) ||
        !table_a.set_cell_text(5U, 0U, "AM-A-04") ||
        !table_a.set_cell_text(5U, 1U, utf8_from_u8(u8"混排")) ||
        !table_a.set_cell_text(5U, 2U, utf8_from_u8(u8"跨页边界")) ||
        !table_a.set_cell_text(
            5U, 3U,
            utf8_from_u8(u8"这行刻意拉长并要求整块迁移，用来观察图片、段落、表格与分页边界之间的紧余量。")) ||
        !table_a.set_cell_text(6U, 0U, "AM-A-05") ||
        !table_a.set_cell_text(6U, 1U, utf8_from_u8(u8"表后")) ||
        !table_a.set_cell_text(6U, 2U, utf8_from_u8(u8"恢复全宽")) ||
        !table_a.set_cell_text(
            6U, 3U,
            utf8_from_u8(u8"表格结束后必须恢复整栏正文，不能继续继承左侧图片留下的窄列回流。")) ||
        !table_a.set_cell_text(7U, 0U, "AM-A-06") ||
        !table_a.set_cell_text(7U, 1U, utf8_from_u8(u8"页脚")) ||
        !table_a.set_cell_text(7U, 2U, utf8_from_u8(u8"总页数")) ||
        !table_a.set_cell_text(
            7U, 3U,
            utf8_from_u8(u8"页脚总页数与终页搜索结果必须一致，不能因为嵌字或裁剪出现回读偏差。"))) {
        return sample;
    }

    auto table_a_banner = table_a.find_cell(0U, 0U);
    auto table_a_heading_region = table_a.find_cell(1U, 0U);
    auto table_a_heading_size = table_a.find_cell(1U, 1U);
    auto table_a_heading_key = table_a.find_cell(1U, 2U);
    auto table_a_heading_note = table_a.find_cell(1U, 3U);
    if (!table_a_banner.has_value() ||
        !table_a_heading_region.has_value() ||
        !table_a_heading_size.has_value() ||
        !table_a_heading_key.has_value() ||
        !table_a_heading_note.has_value() ||
        !table_a_banner->merge_right(3U)) {
        return sample;
    }
    if (!table_a_banner->set_fill_color("D9EAF7") ||
        !table_a_heading_region->set_fill_color("EAF2F8") ||
        !table_a_heading_size->set_fill_color("EAF2F8") ||
        !table_a_heading_key->set_fill_color("EAF2F8") ||
        !table_a_heading_note->set_fill_color("EAF2F8")) {
        return sample;
    }
    ...
    auto checkpoint_a = append_document_paragraph(document, "");
    if (!checkpoint_a.has_next() ||
        !checkpoint_a.add_run("Boundary checkpoint A: ").has_next() ||
        !add_styled_contract_run(document, checkpoint_a,
                                 utf8_from_u8(u8"跨表切换"),
                                 "DocumentPdfCjkFontEmbedAccent") ||
        !checkpoint_a.add_run(" / FE-AM-951 / ").has_next() ||
        !add_styled_contract_run(document, checkpoint_a,
                                 utf8_from_u8(u8"顶底压测"),
                                 "DocumentPdfCjkFontEmbedNote")) {
        return sample;
    }
    ...
    top_bottom_options.horizontal_reference =
        featherdoc::floating_image_horizontal_reference::margin;
    top_bottom_options.horizontal_offset_px = 56;
    top_bottom_options.vertical_reference =
        featherdoc::floating_image_vertical_reference::paragraph;
    top_bottom_options.vertical_offset_px = 0;
    top_bottom_options.wrap_mode =
    ...
    top_bottom_options.wrap_distance_bottom_px = 10U;
    top_bottom_options.crop =
        featherdoc::floating_image_crop{0U, 120U, 180U, 0U};
    if (!document.append_floating_image(image_path, 122U, 54U,
                                        top_bottom_options)) {
        return sample;
    }

    for (int index = 4; index <= 5; ++index) {
        if (!append_matrix_paragraph(index, u8"顶底跨表")) {
            return sample;
        }
    }
    ...
    auto table_b = document.append_table(table_b_row_count, 3U);
    if (!table_b.has_next() || !table_b.set_width_twips(7000U) ||
        !table_b.set_column_width_twips(0U, 1480U) ||
        !table_b.set_column_width_twips(1U, 1560U) ||
        !table_b.set_column_width_twips(2U, 3960U) ||
        !table_b.set_cell_text(0U, 0U, utf8_from_u8(u8"边界复核表")) ||
        !table_b.set_cell_text(1U, 0U, utf8_from_u8(u8"节点")) ||
        !table_b.set_cell_text(1U, 1U, utf8_from_u8(u8"锚点")) ||
        !table_b.set_cell_text(1U, 2U, utf8_from_u8(u8"说明")) ||
        !table_b.set_cell_text(2U, 0U, "AM-B-01") ||
        !table_b.set_cell_text(2U, 1U, utf8_from_u8(u8"顶底切换")) ||
        !table_b.set_cell_text(
            2U, 2U,
            utf8_from_u8(u8"图片切到顶底环绕后，后续表格必须完整落在图片下方，不能贴在图片侧面。")) ||
        !table_b.set_cell_text(3U, 0U, "AM-B-02") ||
        !table_b.set_cell_text(3U, 1U, utf8_from_u8(u8"右侧锚点")) ||
        !table_b.set_cell_text(
            3U, 2U,
            utf8_from_u8(u8"第一行说明承担跨表切换、复制搜索与页码回读复核。")) ||
        !table_b.set_cell_text(4U, 0U, "AM-B-03") ||
        !table_b.set_cell_text(
            4U, 2U,
            utf8_from_u8(u8"第二行说明继续跟随同一锚点块，验证表后回流与文本层连续性。")) ||
        !table_b.set_cell_text(5U, 0U, "AM-B-04") ||
        !table_b.set_cell_text(5U, 1U, utf8_from_u8(u8"禁拆落页")) ||
        !table_b.set_cell_text(
            5U, 2U,
            utf8_from_u8(u8"紧余量下整块迁移，不能把尾行留在上一页，也不能让图片压到表格内容。")) ||
        !table_b.set_cell_text(6U, 0U, "AM-B-05") ||
        !table_b.set_cell_text(6U, 1U, utf8_from_u8(u8"样式密排")) ||
        !table_b.set_cell_text(
            6U, 2U,
            utf8_from_u8(u8"中英 CJK 与大小字混排继续稳定，复制搜索和视觉回读都要一致。")) ||
        !table_b.set_cell_text(7U, 0U, "AM-B-06") ||
        !table_b.set_cell_text(7U, 1U, utf8_from_u8(u8"终页恢复")) ||
        !table_b.set_cell_text(
            7U, 2U,
            utf8_from_u8(u8"末页必须在图片下方恢复整栏正文，避免收口段继续贴着图片窄列。"))) {
        return sample;
    }

    auto table_b_banner = table_b.find_cell(0U, 0U);
    auto table_b_heading_node = table_b.find_cell(1U, 0U);
    auto table_b_heading_anchor = table_b.find_cell(1U, 1U);
    auto table_b_heading_note = table_b.find_cell(1U, 2U);
    auto table_b_merged_anchor = table_b.find_cell(3U, 1U);
    if (!table_b_banner.has_value() ||
        !table_b_heading_node.has_value() ||
        !table_b_heading_anchor.has_value() ||
        !table_b_heading_note.has_value() ||
        !table_b_merged_anchor.has_value() ||
        !table_b_banner->merge_right(2U) ||
    ...
    if (!table_b_banner->set_fill_color("E2F0D9") ||
        !table_b_heading_node->set_fill_color("F2F8EC") ||
        !table_b_heading_anchor->set_fill_color("F2F8EC") ||
        !table_b_heading_note->set_fill_color("F2F8EC") ||
        !table_b_merged_anchor->set_fill_color("D9EAF7") ||
        !table_b_merged_anchor->set_vertical_alignment(
    ...
    auto checkpoint_b = append_document_paragraph(document, "");
    if (!checkpoint_b.has_next() ||
        !checkpoint_b.add_run("Boundary checkpoint B: ").has_next() ||
        !add_styled_contract_run(document, checkpoint_b,
                                 utf8_from_u8(u8"右侧收口"),
                                 "DocumentPdfCjkFontEmbedAccent") ||
        !checkpoint_b.add_run(" / FE-AM-971 / ").has_next() ||
        !add_styled_contract_run(document, checkpoint_b,
    ...
    for (int index = 6; index <= 9; ++index) {
        if (!append_matrix_paragraph(index, u8"右侧收口")) {
            return sample;
        }
    }
    ...
    auto settle = append_document_paragraph(document, "");
    if (!settle.has_next() ||
        !settle.add_run("Boundary settle: ").has_next() ||
        !add_styled_contract_run(document, settle,
                                 utf8_from_u8(u8"恢复整栏"),
                                 "DocumentPdfCjkFontEmbedAccent") ||
        !settle.add_run(" / FE-AM-981 / ").has_next() ||
        !add_styled_contract_run(document, settle,
                                 utf8_from_u8(u8"图片下方全宽"),
                                 "DocumentPdfCjkFontEmbedNote")) {
        return sample;
    }
    ...
    if (!append_document_text_paragraph(
        ...
        !append_document_text_paragraph(
            document,
            utf8_from_u8(u8"补线段落二：继续验证末页 copy/search、字体嵌字、页脚总页数与合同检索保持一致。"))) {
        return sample;
    }
    ...
    if (!closing.has_next() ||
        ...
        !add_styled_contract_run(document, closing, utf8_from_u8(u8"终页合同"),
                                 "DocumentPdfCjkFontEmbedAccent") ||
        !closing.add_run(" / FE-AM-999 / ").has_next() ||
    ...
    options.page_size = featherdoc::pdf::PdfPageSize{460.0, 620.0};
    options.metadata.title =
        "FeatherDoc regression sample: document CJK anchor font matrix boundary";
    options.metadata.creator = "FeatherDoc regression tests";
    options.font_family = "Helvetica";
    options.font_mappings = {
        featherdoc::pdf::PdfFontMapping{"Document CJK Font Embed",
...
build_document_cjk_font_search_density_flow_text_sample(
    ...
    const auto image_path = write_quadrant_rgb_png(
        asset_dir,
        "featherdoc-pdf-regression-document-cjk-font-search-density-flow.png");
    ...
    if (!intro.has_next() ||
        ...
        !add_styled_contract_run(document, intro, utf8_from_u8(u8"密排检索"),
                                 "DocumentPdfCjkFontEmbedAccent") ||
        !intro.add_run(" / FE-SD-901 / ").has_next() ||
        !add_styled_contract_run(document, intro, utf8_from_u8(u8"字号回读矩阵"),
                                 "DocumentPdfCjkFontEmbedNote")) {
        return sample;
    }
    ...
    if (!default_header.has_next() ||
        ...
        !default_footer.has_next() ||
        !default_footer.set_text("Search density footer {{page}} / {{total_pages}}") ||
        !first_header.has_next() ||
        !first_header.set_text("Search density first header SD-101 page {{page}}") ||
        !first_footer.has_next() ||
        !first_footer.set_text(
            "Search density first footer {{page}} / {{total_pages}}") ||
        !even_header.has_next() ||
        !even_header.set_text("Search density even header SD-202 page {{page}}") ||
        !even_footer.has_next() ||
        !even_footer.set_text(
    ...
    if (!append_document_text_paragraph(
            document,
            utf8_from_u8(
                u8"起始段落先验证内联图片后的整栏正文，再把密排字号、复制搜索、页码变体和图片锚点放进连续流里。")) ||
        !document.append_image(image_path, 94U, 38U) ||
        !append_document_text_paragraph(
            document,
            utf8_from_u8(
                u8"内联图片之后必须立即恢复全宽正文，避免后续表格、密排段落与浮动图继续继承错误回流边界。"))) {
        return sample;
    }
    ...
    if (!stripe.has_next() ||
        ...
        !add_styled_contract_run(document, stripe, utf8_from_u8(u8"字宽联测"),
                                 "DocumentPdfCjkFontEmbedAccent") ||
        !stripe.add_run(" / ").has_next() ||
        !add_styled_contract_run(document, stripe, utf8_from_u8(u8"复制回读"),
                                 "DocumentPdfCjkFontEmbedNote")) {
        return sample;
    }

    auto ladder = append_document_paragraph(document, "");
    if (!ladder.has_next() || !ladder.add_run("Density ladder: ").has_next() ||
        !add_styled_contract_run(document, ladder, utf8_from_u8(u8"注记回读"),
                                 "DocumentPdfCjkFontEmbedNote") ||
        !ladder.add_run(" / ").has_next() ||
        !add_styled_contract_run(document, ladder, utf8_from_u8(u8"合同矩阵"),
    ...
    const auto append_density_paragraph = [&](int index,
                                              std::u8string_view label) {
        auto paragraph = append_document_paragraph(document, "");
        const auto prefix = utf8_from_u8(u8"第 ") + std::to_string(index) +
                            utf8_from_u8(u8" 组密排检索验证：");
        const auto token = " / token FE-SD-" + std::to_string(920 + index) +
                           " / ";
        const auto suffix = utf8_from_u8(
            u8"，并确认图片锚点切换、字号变化、复制搜索与终页回读全部一致，不出现丢字或重叠。");
        if (!paragraph.has_next() || !paragraph.add_run(prefix).has_next() ||
            !add_styled_contract_run(document, paragraph, utf8_from_u8(label),
                                     "DocumentPdfCjkFontEmbedNote") ||
            !paragraph.add_run(token).has_next() ||
    ...
    left_square_options.wrap_distance_bottom_px = 8U;
    left_square_options.crop =
        featherdoc::floating_image_crop{170U, 0U, 60U, 120U};
    if (!document.append_floating_image(image_path, 108U, 84U,
                                        left_square_options)) {
        return sample;
    }

    for (int index = 1; index <= 4; ++index) {
        if (!append_density_paragraph(index, u8"左侧密排")) {
            return sample;
        }
    }
    ...
    auto table_a = document.append_table(table_a_row_count, 4U);
    if (!table_a.has_next() || !table_a.set_width_twips(7200U) ||
        !table_a.set_column_width_twips(0U, 1160U) ||
        !table_a.set_column_width_twips(1U, 1240U) ||
        !table_a.set_column_width_twips(2U, 1520U) ||
        !table_a.set_column_width_twips(3U, 3280U) ||
        !table_a.set_cell_text(0U, 0U, utf8_from_u8(u8"检索密度表")) ||
        !table_a.set_cell_text(1U, 0U, utf8_from_u8(u8"区域")) ||
        !table_a.set_cell_text(1U, 1U, utf8_from_u8(u8"字号")) ||
        !table_a.set_cell_text(1U, 2U, utf8_from_u8(u8"检索键")) ||
        !table_a.set_cell_text(1U, 3U, utf8_from_u8(u8"说明")) ||
        !table_a.set_cell_text(2U, 0U, "SD-A-01") ||
        !table_a.set_cell_text(2U, 1U, utf8_from_u8(u8"小字")) ||
        !table_a.set_cell_text(2U, 2U, utf8_from_u8(u8"字宽校验")) ||
        !table_a.set_cell_text(
            2U, 3U,
            utf8_from_u8(u8"左侧图片包围后，小字号中文与英文 token 仍需稳定复制、搜索和回读。")) ||
        !table_a.set_cell_text(3U, 0U, "SD-A-02") ||
        !table_a.set_cell_text(3U, 1U, utf8_from_u8(u8"常规")) ||
        !table_a.set_cell_text(3U, 2U, utf8_from_u8(u8"复制回读")) ||
        !table_a.set_cell_text(
            3U, 3U,
            utf8_from_u8(u8"正文与表格交界处不能打散中英混排，也不能让同一检索键跨行失真。")) ||
        !table_a.set_cell_text(4U, 0U, "SD-A-03") ||
        !table_a.set_cell_text(4U, 1U, utf8_from_u8(u8"大字")) ||
        !table_a.set_cell_text(4U, 2U, utf8_from_u8(u8"终页嵌字")) ||
        !table_a.set_cell_text(
            4U, 3U,
            utf8_from_u8(u8"Large run 与页码占位符共存时，文本层与视觉层都要保持稳定。")) ||
        !table_a.set_cell_text(5U, 0U, "SD-A-04") ||
        !table_a.set_cell_text(5U, 1U, utf8_from_u8(u8"紧余量")) ||
        !table_a.set_cell_text(5U, 2U, utf8_from_u8(u8"跨页密排")) ||
        !table_a.set_cell_text(
            5U, 3U,
            utf8_from_u8(u8"这行刻意加长并要求整块迁移，用于观察密排段落、图片和表格之间的分页边界。")) ||
        !table_a.set_cell_text(6U, 0U, "SD-A-05") ||
        !table_a.set_cell_text(6U, 1U, utf8_from_u8(u8"表后")) ||
        !table_a.set_cell_text(6U, 2U, utf8_from_u8(u8"恢复全宽")) ||
        !table_a.set_cell_text(
            6U, 3U,
            utf8_from_u8(u8"表格结束后必须恢复整栏正文，不能继续沿用左侧图片留下的窄列流。")) ||
        !table_a.set_cell_text(7U, 0U, "SD-A-06") ||
        !table_a.set_cell_text(7U, 1U, utf8_from_u8(u8"页脚")) ||
        !table_a.set_cell_text(7U, 2U, utf8_from_u8(u8"总页数")) ||
        !table_a.set_cell_text(
            7U, 3U,
            utf8_from_u8(u8"页脚总页数、终页检索与复制回读必须对齐，不能因嵌字造成偏差。"))) {
        return sample;
    }
    ...
    auto checkpoint_a = append_document_paragraph(document, "");
    if (!checkpoint_a.has_next() ||
        !checkpoint_a.add_run("Density checkpoint A: ").has_next() ||
        !add_styled_contract_run(document, checkpoint_a,
                                 utf8_from_u8(u8"表格衔接"),
                                 "DocumentPdfCjkFontEmbedAccent") ||
        !checkpoint_a.add_run(" / FE-SD-951 / ").has_next() ||
        !add_styled_contract_run(document, checkpoint_a,
                                 utf8_from_u8(u8"顶底切换"),
                                 "DocumentPdfCjkFontEmbedNote")) {
        return sample;
    }
    ...
    top_bottom_options.wrap_distance_bottom_px = 10U;
    top_bottom_options.crop =
        featherdoc::floating_image_crop{0U, 110U, 170U, 10U};
    if (!document.append_floating_image(image_path, 124U, 56U,
                                        top_bottom_options)) {
        return sample;
    }

    for (int index = 5; index <= 8; ++index) {
        if (!append_density_paragraph(index, u8"顶底密排")) {
            return sample;
        }
    }
    ...
    auto checkpoint_b = append_document_paragraph(document, "");
    if (!checkpoint_b.has_next() ||
        !checkpoint_b.add_run("Density checkpoint B: ").has_next() ||
        !add_styled_contract_run(document, checkpoint_b,
                                 utf8_from_u8(u8"右侧收口"),
                                 "DocumentPdfCjkFontEmbedAccent") ||
        !checkpoint_b.add_run(" / FE-SD-971 / ").has_next() ||
        !add_styled_contract_run(document, checkpoint_b,
                                 utf8_from_u8(u8"终页检索"),
                                 "DocumentPdfCjkFontEmbedNote") ||
        !checkpoint_b.add_run(" / ").has_next() ||
        !add_styled_contract_run(document, checkpoint_b,
                                 utf8_from_u8(u8"大字复核"),
    ...
    right_square_options.wrap_distance_bottom_px = 8U;
    right_square_options.crop =
        featherdoc::floating_image_crop{50U, 60U, 150U, 40U};
    if (!document.append_floating_image(image_path, 114U, 90U,
                                        right_square_options)) {
        return sample;
    }

    for (int index = 9; index <= 12; ++index) {
        if (!append_density_paragraph(index, u8"右侧密排")) {
            return sample;
        }
    }
    ...
    auto settle = append_document_paragraph(document, "");
    if (!settle.has_next() ||
        !settle.add_run("Density settle: ").has_next() ||
        !add_styled_contract_run(document, settle,
                                 utf8_from_u8(u8"恢复整栏"),
                                 "DocumentPdfCjkFontEmbedAccent") ||
        !settle.add_run(" / FE-SD-981 / ").has_next() ||
        !add_styled_contract_run(document, settle,
                                 utf8_from_u8(u8"图片下方回流"),
                                 "DocumentPdfCjkFontEmbedNote")) {
        return sample;
    }
    ...
    if (!append_document_text_paragraph(
            document,
            utf8_from_u8(u8"补线段落一：这里必须在右侧图片下方恢复整栏正文，不能让末页收口继续贴着图片窄列流动。")) ||
        !append_document_text_paragraph(
            document,
            utf8_from_u8(u8"补线段落二：继续验证 copy-search、合同检索、终页嵌字与页脚总页数在末页保持一致。"))) {
        return sample;
    }
    ...
    if (!closing.has_next() ||
        ...
        !add_styled_contract_run(document, closing, utf8_from_u8(u8"终页合同"),
                                 "DocumentPdfCjkFontEmbedAccent") ||
        !closing.add_run(" / FE-SD-999 / ").has_next() ||
        !add_styled_contract_run(document, closing, utf8_from_u8(u8"回读复检"),
                                 "DocumentPdfCjkFontEmbedLarge")) {
        return sample;
    }
    ...
    options.margin_top_points = 42.0;
    options.margin_bottom_points = 46.0;
    options.line_height_points = 14.5;
    options.paragraph_spacing_after_points = 5.0;
    options.image_spacing_after_points = 6.0;
...
build_document_cjk_style_overlay_page_flow_text_sample(
    ...
    const auto image_path = write_quadrant_rgb_png(
        asset_dir,
        "featherdoc-pdf-regression-document-cjk-style-overlay-page-flow.png");
    ...
    if (!intro.has_next() ||
        ...
        !add_styled_contract_run(document, intro, utf8_from_u8(u8"混排叠加"),
                                 "DocumentPdfCjkFontEmbedAccent") ||
        !intro.add_run(" / FE-SO-901 / ").has_next() ||
        !add_styled_contract_run(document, intro, utf8_from_u8(u8"上下标复制"),
                                 "DocumentPdfCjkFontEmbedNote") ||
    ...
    featherdoc::section_page_setup setup{};
    setup.width_twips = 8400U;
    setup.height_twips = 7200U;
    setup.margins.top_twips = 640U;
    setup.margins.bottom_twips = 640U;
    setup.margins.left_twips = 700U;
    setup.margins.right_twips = 700U;
    setup.margins.header_twips = 240U;
    setup.margins.footer_twips = 240U;
    if (!document.set_section_page_setup(0U, setup)) {
        return sample;
    ...
    if (!append_document_text_paragraph(
            document,
            utf8_from_u8(u8"起始段落包含 SO-888 样式检索键，并在内联图片之后继续验证 CJK、Latin、数字、上下标和删除线的文本层一致性。")) ||
        !document.append_image(image_path, 92U, 32U) ||
        !append_document_text_paragraph(
            document,
            utf8_from_u8(u8"内联图片之后必须恢复整栏正文，SO-888 样式检索键 不能因为图片或样式缩放造成错位、缺字或搜索失败。"))) {
        return sample;
    }
    ...
    if (!matrix.has_next() ||
        ...
        !add_styled_contract_run(document, matrix, utf8_from_u8(u8"下标B2"),
                                 "DocumentPdfCjkOverlaySubscript") ||
        !matrix.add_run(" / ").has_next() ||
        !add_styled_contract_run(document, matrix,
                                 utf8_from_u8(u8"删除线仍可搜"),
                                 "DocumentPdfCjkOverlayStrike")) {
        return sample;
    }

    const auto append_overlay_paragraph = [&](int index,
                                              std::u8string_view label) {
        auto paragraph = append_document_paragraph(document, "");
        const auto prefix = utf8_from_u8(u8"第 ") + std::to_string(index) +
                            utf8_from_u8(u8" 组样式叠加分页验证：");
        const auto token = " / token FE-SO-" + std::to_string(920 + index) +
                           " / ";
        const auto suffix = utf8_from_u8(
            u8"，同一行内要稳定处理 CJK、Latin、数字、上下标和删除线，分页后仍保持可复制可搜索。");
        if (!paragraph.has_next()) {
            return false;
        }
        if (index % 3 == 0 &&
        ...
        if (!paragraph.add_run(prefix).has_next() ||
            !add_styled_contract_run(document, paragraph, utf8_from_u8(label),
                                     "DocumentPdfCjkFontEmbedNote") ||
            !paragraph.add_run(" / ").has_next() ||
            !add_styled_contract_run(document, paragraph, overlay_key,
                                     "DocumentPdfCjkFontEmbedAccent") ||
            !paragraph.add_run(token).has_next() ||
            !add_styled_contract_run(document, paragraph,
                                     utf8_from_u8(u8"上标检索"),
                                     "DocumentPdfCjkOverlaySuperscript") ||
            !paragraph.add_run(" + ").has_next() ||
            !add_styled_contract_run(document, paragraph,
    ...
    left_options.wrap_distance_right_px = 14U;
    left_options.wrap_distance_bottom_px = 8U;
    left_options.crop = featherdoc::floating_image_crop{160U, 0U, 80U, 120U};
    if (!document.append_floating_image(image_path, 100U, 74U, left_options)) {
        return sample;
    }

    for (int index = 1; index <= 4; ++index) {
        if (!append_overlay_paragraph(index, u8"左图窄栏")) {
            return sample;
        }
    }
    ...
    constexpr std::size_t table_row_count = 8U;
    auto table = document.append_table(table_row_count, 3U);
    if (!table.has_next() || !table.set_width_twips(6200U) ||
        !table.set_column_width_twips(0U, 1280U) ||
        !table.set_column_width_twips(1U, 1700U) ||
        !table.set_column_width_twips(2U, 3220U) ||
        !table.set_cell_text(0U, 0U, utf8_from_u8(u8"样式叠加表")) ||
        !table.set_cell_text(1U, 0U, utf8_from_u8(u8"区域")) ||
        !table.set_cell_text(1U, 1U, utf8_from_u8(u8"样式键")) ||
        !table.set_cell_text(1U, 2U, utf8_from_u8(u8"说明")) ||
        !table.set_cell_text(2U, 0U, "SO-A-01") ||
        !table.set_cell_text(2U, 1U, overlay_key) ||
        !table.set_cell_text(2U, 2U,
                             utf8_from_u8(u8"表格首行复现同一检索键，验证表头之后文本层连续。")) ||
        !table.set_cell_text(3U, 0U, "SO-A-02") ||
        !table.set_cell_text(3U, 1U, overlay_key) ||
        !table.set_cell_text(3U, 2U,
                             utf8_from_u8(u8"合并单元格第一行承载样式键。")) ||
        !table.set_cell_text(4U, 0U, "SO-A-03") ||
        !table.set_cell_text(4U, 2U, "") ||
        !table.set_cell_text(5U, 0U, "SO-A-04") ||
        !table.set_cell_text(5U, 1U, utf8_from_u8(u8"禁拆样式块")) ||
        !table.set_cell_text(
            5U, 2U,
            utf8_from_u8(u8"紧余量下整行必须迁移，不能把上下标或删除线压到页脚。")) ||
        !table.set_cell_text(6U, 0U, "SO-A-05") ||
        !table.set_cell_text(6U, 1U, utf8_from_u8(u8"分页接力")) ||
        !table.set_cell_text(
            6U, 2U,
            utf8_from_u8(u8"重复表头、合并块和样式行跨页后仍需保持可检索。")) ||
        !table.set_cell_text(7U, 0U, "SO-A-06") ||
        !table.set_cell_text(7U, 1U, overlay_key) ||
        !table.set_cell_text(
            7U, 2U,
            utf8_from_u8(u8"表格结束后继续恢复整栏正文，末页再次复检样式键。"))) {
        return sample;
    }
    ...
    auto heading_key = table.find_cell(1U, 1U);
    auto heading_note = table.find_cell(1U, 2U);
    auto merged_key = table.find_cell(3U, 1U);
    auto styled_cell = table.find_cell(4U, 2U);
    if (!banner.has_value() || !heading_region.has_value() ||
        !heading_key.has_value() || !heading_note.has_value() ||
        !merged_key.has_value() || !styled_cell.has_value() ||
        !banner->merge_right(2U) || !merged_key->merge_down(1U)) {
        return sample;
    }
    if (!banner->set_text(utf8_from_u8(u8"样式叠加表")) ||
        !banner->set_fill_color("D9EAF7") ||
        !banner->set_vertical_alignment(
            featherdoc::cell_vertical_alignment::center) ||
        !heading_region->set_fill_color("EAF2F8") ||
    ...
    auto styled_cell_paragraph = styled_cell->paragraphs();
    if (!styled_cell_paragraph.has_next() ||
        !styled_cell_paragraph.set_text("") ||
        !styled_cell_paragraph.add_run("单元格混排: ").has_next() ||
        !add_styled_contract_run(document, styled_cell_paragraph, overlay_key,
                                 "DocumentPdfCjkFontEmbedAccent") ||
        !styled_cell_paragraph.add_run(" / ").has_next() ||
        !add_styled_contract_run(document, styled_cell_paragraph,
                                 utf8_from_u8(u8"行内上下标"),
                                 "DocumentPdfCjkOverlaySuperscript") ||
        !styled_cell_paragraph.add_run(" + ").has_next() ||
        !add_styled_contract_run(document, styled_cell_paragraph,
                                 utf8_from_u8(u8"下标单元格"),
                                 "DocumentPdfCjkOverlaySubscript") ||
        !styled_cell_paragraph.add_run(" / ").has_next() ||
        !add_styled_contract_run(document, styled_cell_paragraph,
                                 utf8_from_u8(u8"删除线单元格"),
                                 "DocumentPdfCjkOverlayStrike")) {
        return sample;
    }
    ...
    auto checkpoint = append_document_paragraph(document, "");
    if (!checkpoint.has_next() ||
        !checkpoint.add_run("Overlay checkpoint: ").has_next() ||
        !add_styled_contract_run(document, checkpoint, overlay_key,
                                 "DocumentPdfCjkFontEmbedAccent") ||
        !checkpoint.add_run(" / FE-SO-961 / ").has_next() ||
        !add_styled_contract_run(document, checkpoint,
                                 utf8_from_u8(u8"表格样式收口"),
                                 "DocumentPdfCjkFontEmbedNote")) {
        return sample;
    }
    ...
    for (int index = 5; index <= 7; ++index) {
        if (!append_overlay_paragraph(index, u8"顶底样式")) {
            return sample;
        }
    }
    ...
    right_options.horizontal_reference =
        featherdoc::floating_image_horizontal_reference::margin;
    right_options.horizontal_offset_px = 204;
    right_options.vertical_reference =
        featherdoc::floating_image_vertical_reference::paragraph;
    right_options.vertical_offset_px = 0;
    right_options.wrap_mode = featherdoc::floating_image_wrap_mode::top_bottom;
    right_options.wrap_distance_top_px = 8U;
    right_options.wrap_distance_bottom_px = 18U;
    right_options.crop = featherdoc::floating_image_crop{50U, 60U, 150U, 40U};
    if (!document.append_floating_image(image_path, 98U, 112U, right_options)) {
        return sample;
    }

    for (int index = 8; index <= 9; ++index) {
        if (!append_overlay_paragraph(index, u8"右图收口")) {
            return sample;
        }
    }
    ...
    auto settle = append_document_paragraph(document, "");
    if (!settle.has_next() ||
        !settle.add_run("Overlay settle: ").has_next() ||
        !add_styled_contract_run(document, settle, overlay_key,
                                 "DocumentPdfCjkFontEmbedAccent") ||
        !settle.add_run(" / FE-SO-981 / ").has_next() ||
        !add_styled_contract_run(document, settle,
                                 utf8_from_u8(u8"图片下方整栏"),
                                 "DocumentPdfCjkFontEmbedNote")) {
        return sample;
    ...
    if (!append_document_text_paragraph(
            document,
            utf8_from_u8(u8"补线段落一：右侧图片结束后必须恢复整栏正文，SO-888 样式检索键 仍然应该完整可搜。")) ||
        !append_document_text_paragraph(
            document,
            utf8_from_u8(u8"补线段落二：末页再次复现 SO-888 样式检索键，确认上下标、删除线、正文和表格共享同一文本层表现。"))) {
        return sample;
    }
    ...
    if (!closing.has_next() ||
        ...
        !add_styled_contract_run(document, closing, overlay_key,
                                 "DocumentPdfCjkFontEmbedAccent") ||
        !closing.add_run(" / FE-SO-999 / ").has_next() ||
        !add_styled_contract_run(document, closing, utf8_from_u8(u8"终页复检"),
                                 "DocumentPdfCjkFontEmbedLarge")) {
        return sample;
    }
    ...
    featherdoc::pdf::PdfDocumentAdapterOptions options;
    options.page_size = featherdoc::pdf::PdfPageSize{420.0, 360.0};
    options.metadata.title =
...
[[nodiscard]] ScenarioResult
build_document_cjk_numbered_list_page_flow_text_sample(
    const std::filesystem::path &cjk_font_path,
    const std::filesystem::path &asset_dir) {
    ScenarioResult sample;
    ...
    const auto image_path = write_quadrant_rgb_png(
        asset_dir,
        "featherdoc-pdf-regression-document-cjk-numbered-list-page-flow.png");
    const auto list_key = utf8_from_u8(u8"NL-888 编号检索键");
    ...
    if (!intro.has_next() ||
        ...
        !add_styled_contract_run(document, intro, utf8_from_u8(u8"编号重启"),
                                 "DocumentPdfCjkFontEmbedAccent") ||
        !intro.add_run(" / FE-NL-901 / ").has_next() ||
        !add_styled_contract_run(document, intro, utf8_from_u8(u8"嵌套列表"),
                                 "DocumentPdfCjkFontEmbedNote")) {
        return sample;
    }
    ...
    if (!default_header.has_next() ||
        !default_header.set_text(utf8_from_u8(
            u8"List header NL-303 编号检索键 page {{page}}")) ||
        !default_footer.has_next() ||
        !default_footer.set_text(utf8_from_u8(
            u8"List footer NL-888 编号检索键 {{page}} / {{total_pages}}")) ||
        !first_header.has_next() ||
        !first_header.set_text(utf8_from_u8(
            u8"List first header NL-101 编号检索键 page {{page}}")) ||
        !first_footer.has_next() ||
        !first_footer.set_text(utf8_from_u8(
            u8"List first footer NL-888 编号检索键 {{page}} / {{total_pages}}")) ||
        !even_header.has_next() ||
        !even_header.set_text(utf8_from_u8(
            u8"List even header NL-202 编号检索键 page {{page}}")) ||
        !even_footer.has_next() ||
        !even_footer.set_text(utf8_from_u8(
            u8"List even footer NL-888 编号检索键 {{page}} / {{total_pages}}"))) {
        return sample;
    }
    ...
    if (!append_document_text_paragraph(
            document,
            utf8_from_u8(u8"起始段落包含 NL-888 编号检索键，接下来把 CJK 编号列表、嵌套层级、重启编号、图片锚点和表格分页组合在同一文档流中验证。")) ||
        !document.append_image(image_path, 92U, 34U) ||
        !append_document_text_paragraph(
            document,
            utf8_from_u8(u8"内联图片之后列表必须恢复整栏正文，编号前缀和 NL-888 编号检索键 都要可复制可搜索。"))) {
        return sample;
    }

    auto marker = append_document_paragraph(document, "");
    if (!marker.has_next() ||
        !marker.add_run("List marker: ").has_next() ||
        !add_styled_contract_run(document, marker, list_key,
                                 "DocumentPdfCjkFontEmbedAccent") ||
        !marker.add_run(" / FE-NL-911 / ").has_next() ||
        !add_styled_contract_run(document, marker, utf8_from_u8(u8"列表起点"),
                                 "DocumentPdfCjkFontEmbedNote")) {
        return sample;
    }

    const auto append_list_item =
        [&](int index, std::u8string_view label, featherdoc::list_kind kind,
            bool restart, std::uint32_t level) {
            auto paragraph = append_document_paragraph(document, "");
            if (!paragraph.has_next()) {
                return false;
            }
            const bool list_set =
                restart
                    ? document.restart_paragraph_list(paragraph, kind, level)
                    : document.set_paragraph_list(paragraph, kind, level);
            if (!list_set) {
                return false;
            }
            const auto prefix = utf8_from_u8(u8"第 ") + std::to_string(index) +
                                utf8_from_u8(u8" 项 CJK 编号流：");
            const auto token = " / token FE-NL-" + std::to_string(920 + index) +
                               " / ";
            const auto suffix = utf8_from_u8(
                u8"，编号前缀、正文换行、图片绕排和页眉页脚需要保持文本层连续。");
            if (!paragraph.add_run(prefix).has_next() ||
                !add_styled_contract_run(document, paragraph,
                                         utf8_from_u8(label),
                                         "DocumentPdfCjkFontEmbedNote") ||
    ...
    if (!append_list_item(1, u8"左图编号", featherdoc::list_kind::decimal,
                          true, 0U) ||
        !append_list_item(2, u8"左图编号", featherdoc::list_kind::decimal,
                          false, 0U) ||
        !append_list_item(3, u8"嵌套编号", featherdoc::list_kind::decimal,
                          false, 1U) ||
        !append_list_item(4, u8"左图收口", featherdoc::list_kind::decimal,
                          false, 0U)) {
        return sample;
    }
    ...
    if (!table.has_next() || !table.set_width_twips(6400U) ||
        ...
        !table.set_cell_text(0U, 0U, utf8_from_u8(u8"编号分页表")) ||
        !table.set_cell_text(1U, 0U, utf8_from_u8(u8"阶段")) ||
        !table.set_cell_text(1U, 1U, utf8_from_u8(u8"编号键")) ||
        !table.set_cell_text(1U, 2U, utf8_from_u8(u8"说明")) ||
        !table.set_cell_text(2U, 0U, "NL-A-01") ||
        !table.set_cell_text(2U, 1U, list_key) ||
        !table.set_cell_text(
            2U, 2U,
            utf8_from_u8(u8"表格前的列表编号需要完整结束，再进入重复表头验证。")) ||
        !table.set_cell_text(3U, 0U, "NL-A-02") ||
        !table.set_cell_text(3U, 1U, utf8_from_u8(u8"重启编号")) ||
        !table.set_cell_text(
            3U, 2U,
            utf8_from_u8(u8"下一段会重启 decimal 列表，PDF 文本层仍应可搜索。")) ||
        !table.set_cell_text(4U, 0U, "NL-A-03") ||
        !table.set_cell_text(4U, 1U, list_key) ||
        !table.set_cell_text(
            4U, 2U,
            utf8_from_u8(u8"合并单元格跨行承载 NL-888 编号检索键。")) ||
        !table.set_cell_text(5U, 0U, "NL-A-04") ||
        !table.set_cell_text(5U, 2U,
                             utf8_from_u8(u8"合并块第二行继续压分页边界。")) ||
        !table.set_cell_text(6U, 0U, "NL-A-05") ||
        !table.set_cell_text(6U, 1U, utf8_from_u8(u8"禁拆列表块")) ||
        !table.set_cell_text(
            6U, 2U,
            utf8_from_u8(u8"紧余量下整行必须迁移，不能把编号项压到页脚或图片上。")) ||
        !table.set_cell_text(7U, 0U, "NL-A-06") ||
        !table.set_cell_text(7U, 1U, list_key) ||
        !table.set_cell_text(
            7U, 2U,
            utf8_from_u8(u8"表格结束后继续恢复列表，验证编号重启和图片下方整栏。"))) {
        return sample;
    }

    auto banner = table.find_cell(0U, 0U);
    auto heading_stage = table.find_cell(1U, 0U);
    auto heading_key = table.find_cell(1U, 1U);
    auto heading_note = table.find_cell(1U, 2U);
    auto merged_key = table.find_cell(4U, 1U);
    if (!banner.has_value() || !heading_stage.has_value() ||
        !heading_key.has_value() || !heading_note.has_value() ||
        !merged_key.has_value() || !banner->merge_right(2U) ||
        !merged_key->merge_down(1U)) {
        return sample;
    }
    if (!banner->set_text(utf8_from_u8(u8"编号分页表")) ||
        !banner->set_fill_color("D9EAF7") ||
        !banner->set_vertical_alignment(
            featherdoc::cell_vertical_alignment::center) ||
        !heading_stage->set_fill_color("EAF2F8") ||
        !heading_key->set_fill_color("EAF2F8") ||
        !heading_note->set_fill_color("EAF2F8") ||
        !merged_key->set_fill_color("FCE4D6") ||
        !merged_key->set_vertical_alignment(
    ...
    auto checkpoint = append_document_paragraph(document, "");
    if (!checkpoint.has_next() ||
        !checkpoint.add_run("List checkpoint: ").has_next() ||
        !add_styled_contract_run(document, checkpoint, list_key,
                                 "DocumentPdfCjkFontEmbedAccent") ||
        !checkpoint.add_run(" / FE-NL-961 / ").has_next() ||
        !add_styled_contract_run(document, checkpoint,
    ...
    if (!append_list_item(5, u8"重启编号", featherdoc::list_kind::decimal,
                          true, 0U) ||
        !append_list_item(6, u8"重启编号", featherdoc::list_kind::decimal,
                          false, 0U) ||
        !append_list_item(7, u8"项目符号", featherdoc::list_kind::bullet,
                          true, 0U) ||
        !append_list_item(8, u8"项目符号", featherdoc::list_kind::bullet,
                          false, 0U)) {
        return sample;
    }
    ...
    right_options.horizontal_reference =
        featherdoc::floating_image_horizontal_reference::margin;
    right_options.horizontal_offset_px = 206;
    right_options.vertical_reference =
        featherdoc::floating_image_vertical_reference::paragraph;
    right_options.vertical_offset_px = 0;
    right_options.wrap_mode = featherdoc::floating_image_wrap_mode::top_bottom;
    ...
    right_options.crop = featherdoc::floating_image_crop{50U, 60U, 150U, 40U};
    if (!document.append_floating_image(image_path, 100U, 92U, right_options)) {
        return sample;
    }

    if (!append_list_item(9, u8"右图收口", featherdoc::list_kind::decimal,
                          true, 0U) ||
        !append_list_item(10, u8"右图收口", featherdoc::list_kind::decimal,
                          false, 0U)) {
        return sample;
    }
    ...
    auto settle = append_document_paragraph(document, "");
    if (!settle.has_next() ||
        !settle.add_run("List settle: ").has_next() ||
        !add_styled_contract_run(document, settle, list_key,
                                 "DocumentPdfCjkFontEmbedAccent") ||
        !settle.add_run(" / FE-NL-981 / ").has_next() ||
        !add_styled_contract_run(document, settle,
                                 utf8_from_u8(u8"图片下方整栏"),
                                 "DocumentPdfCjkFontEmbedNote")) {
        return sample;
    ...
    if (!append_document_text_paragraph(
            document,
            utf8_from_u8(u8"补线段落：末页再次复现 NL-888 编号检索键，确认编号前缀、列表正文、表格和页脚共享同一文本层表现。"))) {
        return sample;
    }
    ...
    if (!closing.has_next() ||
        ...
        !add_styled_contract_run(document, closing, list_key,
                                 "DocumentPdfCjkFontEmbedAccent") ||
        !closing.add_run(" / FE-NL-999 / ").has_next() ||
        !add_styled_contract_run(document, closing, utf8_from_u8(u8"终页复检"),
                                 "DocumentPdfCjkFontEmbedLarge")) {
        return sample;
    }
...
int run_program(const std::vector<std::string> &args) {
    ...
    if (config.scenario == "single_text") {
    ...
    } else if (config.scenario == "document_cjk_multi_anchor_table_flow_text") {
        if (cjk_font.empty() || !std::filesystem::exists(cjk_font)) {
            if (require_cjk_font) {
                std::cerr << "skipping CJK regression sample: no usable CJK font "
                             "found; set FEATHERDOC_TEST_CJK_FONT or install a "
        ...
        sample = build_document_cjk_multi_anchor_table_flow_text_sample(
            cjk_font, output_parent);
    } else if (config.scenario ==
        ...
        sample = build_document_cjk_anchor_font_matrix_boundary_text_sample(
            cjk_font, output_parent);
    } else if (config.scenario ==
        ...
        sample = build_document_cjk_style_overlay_page_flow_text_sample(
            cjk_font, output_parent);
    } else if (config.scenario ==
