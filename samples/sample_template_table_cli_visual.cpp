...
bool seed_body(featherdoc::Document &doc) {
    ...
    if (!intro.has_next() ||
        !intro.set_text("Template table CLI visual regression sample.")) {
        return false;
    }

    if (!require_step(configure_two_column_table(
                          body_template.append_table(2U, 2U), "Body", "Status",
                          "D9EAF7", "Retained body table",
                          "Should remain unchanged."),
                      "configure retained body table")) {
        return false;
    }
    ...
    if (!require_step(configure_two_column_table(
                      ...
                      "configure target body table")) {
        return false;
    }

    if (!body_template.append_paragraph(
             "The green body table should keep its borders and stay between "
             "this text and the footer.")
             .has_next()) {
        return false;
    }

    if (!body_template.append_paragraph(
             "Footer growth should not overlap the body paragraphs.")
             .has_next()) {
        return false;
    }
...
bool seed_header(featherdoc::Document &doc) {
    ...
    if (!header_paragraph.has_next() ||
        !header_paragraph.set_text("Header template table CLI visual baseline")) {
        return false;
    }
    ...
    if (!require_step(configure_two_column_table(
                          ...
                          "Status", "D9EAF7", "Retained header table",
                          "Should remain unchanged."),
...
bool seed_footer(featherdoc::Document &doc) {
    ...
    if (!footer_paragraph.has_next() ||
        !footer_paragraph.set_text("Footer template table CLI visual baseline")) {
        return false;
    }
...
int main(int argc, char **argv) {
    const fs::path output_path = argc > 1 ? fs::path(argv[1])
                                          : fs::current_path() /
                                                "template_table_cli_visual.docx";
