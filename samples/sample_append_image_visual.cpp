...
<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
  <w:body>
    <w:p>
      <w:r>
        <w:t>Append image visual regression baseline.</w:t>
      </w:r>
    </w:p>
    <w:p>
      <w:r>
        <w:t>The body text stays fixed so appended inline and header floating image cases are easy to compare against the same baseline.</w:t>
      </w:r>
    </w:p>
    <w:p>
      <w:r>
        <w:t>Body inline case: a red image should appear below this retained paragraph block after the append-image command runs.</w:t>
      </w:r>
    </w:p>
    <w:p>
      <w:r>
        <w:t>Header floating case: the baseline starts with no header content, and the mutated render should show an orange badge near the top-right margin.</w:t>
      </w:r>
    </w:p>
    <w:p>
      <w:r>
        <w:t>Review note: appended image mutations should not reorder the existing body paragraphs.</w:t>
      </w:r>
    </w:p>
    <w:sectPr>
...
int main(int argc, char **argv) {
    const fs::path output_path =
        argc > 1 ? fs::path(argv[1])
                 : fs::current_path() / "append_image_visual_baseline.docx";
