...
[[nodiscard]] auto report_duplicate_remove_slot_member(
...
[[nodiscard]] auto parse_string_member(
    ...
    if (target.has_value()) {
        return report_duplicate_remove_slot_member(member_name, error_message);
    }
...
auto parse_template_schema_patch_remove_slot_member(
    std::string_view content, std::size_t &index, std::string_view member_name,
