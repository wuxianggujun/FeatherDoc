        ...
        while (index < content.size()) {
            ...
            if (member_name == "definition_name") {
                ...
                if (patch.instance_index.has_value()) {
                    error_message = "override patch entries must not contain both "
                                    "'instance_index' and 'instance_id'";
                    return false;
                }
                std::uint32_t parsed_id = 0U;
                if (!parse_json_numbering_catalog_uint32(content, index,
                                                         parsed_id, member_name,
                                                         error_message)) {
                    return false;
                }
                ...
                saw_level = true;
                if (!parse_json_numbering_catalog_uint32(content, index, patch.level,
                                                         member_name,
                                                         error_message)) {
                    return false;
                }
