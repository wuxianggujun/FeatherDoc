...
if (-not [string]::IsNullOrWhiteSpace($projectTemplateWorkflowDashboardBusinessDocumentTypesText)) {
    [void]$lines.Add("- Project template workflow dashboard business document types: $projectTemplateWorkflowDashboardBusinessDocumentTypesText")
}
if (-not [string]::IsNullOrWhiteSpace($projectTemplateWorkflowDashboardCorpusRolesText)) {
    [void]$lines.Add("- Project template workflow dashboard corpus roles: $projectTemplateWorkflowDashboardCorpusRolesText")
}
...
if ($projectTemplateWorkflowDashboardHasEvidence) {
    ...
    foreach ($actionSummaryLine in @($projectTemplateWorkflowDashboardActionSummaryLines)) {
        Add-CheckboxLine -Lines $lines -Text ('Review project template workflow dashboard action group: {0}' -f $actionSummaryLine)
    }
    if (-not [string]::IsNullOrWhiteSpace($projectTemplateWorkflowDashboardNextActionCommand)) {
...
if ($requiresProjectTemplateGovernanceSignoff -or $hasProjectTemplateReleaseEntryEvidence) {
    Add-CheckboxLine -Lines $lines -Text ('Confirm the packaged release manifest preserves project-template governance contracts before publishing: {0} must include `project_template_delivery_readiness_contract` and `project_template_onboarding_governance_contract` with `status`, `release_ready`, `release_blocker_count`, `warning_count`, `schema_approval_status_summary`, `onboarding_governance_next_action`, `onboarding_governance_next_action_summary`, `onboarding_governance_next_action_group_count`, `next_action`, `next_action_summary`, `next_action_group_count`, `requires_reviewer_action`, `reviewer_action_summary`, `reviewer_action_reason`, `reviewer_actions`, `business_document_type_summary`, `corpus_role_summary`, `source_report_display`, and `source_json_display`.' -f $releaseAssetsManifestDisplayPath)
}
if (-not [string]::IsNullOrWhiteSpace($projectTemplateChecklistHandoffEvidenceLine)) {
