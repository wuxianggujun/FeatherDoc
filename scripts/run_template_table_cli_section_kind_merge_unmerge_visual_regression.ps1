param(
    [string]$BuildDir = "build-template-table-cli-section-kind-merge-unmerge-visual-nmake",
    [string]$OutputDir = "output/template-table-cli-section-kind-merge-unmerge-visual-regression",
    [int]$Dpi = 144,
    [switch]$SkipBuild,
    [switch]$SkipVisual,
    [switch]$KeepWordOpen,
...
function Write-Step {
    ...
    Write-Host "[template-table-cli-section-kind-merge-unmerge-visual] $Message"
}
...
$cases = @(
    [ordered]@{
        ...
        sample_case_id = "section-header-default-merge-right"
        description = "Merge the default section-header target cell horizontally and verify the change appears only on the later odd page."
        ...
        baseline_rows = @(
            @("Default Merge Anchor", "Merge source", "Green tail"),
            @("1600 cue", "1800 cue", "3000 cue")
        )
        mutated_rows = @(
            @("Default Merge Anchor", "Green tail"),
            @("1600 cue", "1800 cue", "3000 cue")
        )
        ...
        expected_visual_cues = @(
            "Page 1 stays on the first-page header control path and must not show the default-header merge target.",
            "Page 3 keeps the retained blue default-header table unchanged while the purple anchor cell widens across the first two columns.",
            "The page 3 body flow still starts below the header area without overlap or clipping."
        )
    },
    [ordered]@{
        id = "section-header-even"
        sample_case_id = "section-header-even-unmerge-right"
        description = "Split the merged even section-header target span and verify the change appears only on page 2."
        ...
        baseline_rows = @(
            @("Merged even span", "Green tail"),
            @("1200 cue", "Restore cue", "3000 cue")
        )
        mutated_rows = @(
            @("Merged even span", "", "Green tail"),
            @("1200 cue", "Restore cue", "3000 cue")
        ...
        expected_visual_cues = @(
            "Page 2 keeps the retained blue even-header table unchanged while the merged first-row span splits back into two cells before the green tail.",
            "Page 3 still follows the default-header control path and must not show the even-header split.",
            "The page 2 body flow still starts below the even header area without overlap or clipping."
        )
    },
    [ordered]@{
        id = "section-footer-first"
        sample_case_id = "section-footer-first-merge-down"
        description = "Merge the first-page section-footer target cell downward and verify the change appears only on page 1."
        command = "merge-template-table-cells"
        part = "section-footer"
        ...
        baseline_rows = @(
            @("First pillar anchor", "Footer top"),
        ...
        expected_visual_cues = @(
            "Page 1 keeps the retained blue first-footer table unchanged while the orange first column becomes one vertical pillar across all three rows.",
            "Page 3 still follows the default-footer control path and must not show the first-page footer merge.",
            "The enlarged first-page footer remains inside the footer area without colliding with the body."
        )
    },
    [ordered]@{
        id = "section-footer-default"
        sample_case_id = "section-footer-default-unmerge-down"
        description = "Split the merged default section-footer pillar and verify the change appears only on the later odd page."
        command = "unmerge-template-table-cells"
        part = "section-footer"
        ...
        baseline_rows = @(
            @("Merged default pillar", "Footer top"),
            @("", "Footer middle"),
            @("", "Footer bottom")
        )
        mutated_rows = @(
        ...
        expected_visual_cues = @(
            "Page 1 still follows the first-page footer control path and must not show the default-footer split.",
            "Page 3 keeps the retained blue default-footer table unchanged while the green merged pillar turns back into stacked cells with visible horizontal borders.",
            "The default footer on page 3 stays inside the footer area without visibly overlapping the body."
        )
    }
)
...
if (-not $SkipBuild) {
    ...
    Write-Step "Building CLI and section-kind merge/unmerge visual sample"
    Invoke-MsvcCommand -VcvarsPath $vcvarsPath -CommandText "cmake --build `"$resolvedBuildDir`" --target featherdoc_cli featherdoc_sample_template_table_cli_section_kind_merge_unmerge_visual"
}

$cliExecutable = Find-BuildExecutable -BuildRoot $resolvedBuildDir -TargetName "featherdoc_cli"
$sampleExecutable = Find-BuildExecutable -BuildRoot $resolvedBuildDir -TargetName "featherdoc_sample_template_table_cli_section_kind_merge_unmerge_visual"
$renderPython = $null
if (-not $SkipVisual) {
...
foreach ($case in $cases) {
    ...
    $baselineRows = Get-Content -Raw -LiteralPath $baselineRowsJsonPath | ConvertFrom-Json
    $entryName = $baselineRows.entry_name
    $baselineXml = Get-DocxEntryText -DocxPath $baselineDocxPath -EntryName $entryName
    Assert-EntryPatterns `
        -XmlText $baselineXml `
        -MustContain @($case.baseline_xml_contains) `
        -MustNotContain @($case.baseline_xml_not_contains) `
        -Label "baseline $($case.id) $entryName"
    ...
    [void]$commandLines.Add("# Template table CLI section-kind merge/unmerge command sequence for $($case.id)")
    ...
    $mutatedXml = Get-DocxEntryText -DocxPath $mutatedDocxPath -EntryName $entryName
    Assert-EntryPatterns `
        -XmlText $mutatedXml `
        -MustContain @($case.mutated_xml_contains) `
        -MustNotContain @($case.mutated_xml_not_contains) `
        -Label "mutated $($case.id) $entryName"
...
[void]$reviewChecklistLines.Add("# Template table CLI section-kind merge/unmerge visual regression checklist")
...
[void]$finalReviewLines.Add("# Template table CLI section-kind merge/unmerge final review")
...
Write-Step "Completed template table CLI section-kind merge/unmerge visual regression"
