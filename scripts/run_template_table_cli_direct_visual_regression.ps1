param(
    [string]$BuildDir = "build-template-table-cli-direct-visual-nmake",
    [string]$OutputDir = "output/template-table-cli-direct-visual-regression",
    [int]$Dpi = 144,
    [switch]$SkipBuild,
    [switch]$SkipVisual,
    [switch]$KeepWordOpen,
...
function Write-Step {
    ...
    Write-Host "[template-table-cli-direct-visual] $Message"
}
...
$cases = @(
    [ordered]@{
        id = "header"
        description = "Mutate the purple direct-header template table while keeping the retained blue direct header table intact."
        part = "header"
        ...
        expected_visual_cues = @(
            "The retained blue direct header table stays unchanged at the top of the page.",
            "The purple direct header target table keeps its borders and shows Header visual row / 7, Header inserted after / 2, and Header appended row / 3.",
            "Body text still starts below the header area with no overlap or clipping."
        )
    },
    [ordered]@{
        id = "footer"
        description = "Mutate the orange direct-footer template table and verify the taller direct footer still renders cleanly."
        part = "footer"
        ...
        expected_visual_cues = @(
            ...
            "The orange direct footer target table keeps visible borders and shows Footer visual row / 9, Footer inserted after / 4, and Footer appended row / 5.",
            "The enlarged direct footer stays inside the page footer area and does not visibly overlap the body paragraphs."
        )
    },
    [ordered]@{
        id = "body"
...
$baselineDocxPath = Join-Path $baselineDir "template_table_cli_direct_visual_baseline.docx"
...
if ($LASTEXITCODE -ne 0) {
    throw "Sample execution failed: featherdoc_sample_template_table_cli_direct_visual"
}
...
foreach ($case in $cases) {
    ...
    [void]$commandLines.Add("# Template table CLI direct mutation sequence for $($case.id)")
...
[void]$reviewChecklistLines.Add("# Template table CLI direct visual regression checklist")
...
[void]$finalReviewLines.Add("# Template table CLI direct visual final review")
...
Write-Step "Completed template table CLI direct visual regression"
