param(
    [string]$BuildDir = "build-template-table-cli-direct-column-visual-nmake",
    [string]$OutputDir = "output/template-table-cli-direct-column-visual-regression",
    [int]$Dpi = 144,
    [switch]$SkipBuild,
    [switch]$SkipVisual,
    [switch]$KeepWordOpen,
...
function Write-Step {
    ...
    Write-Host "[template-table-cli-direct-column-visual] $Message"
}
...
$cases = @(
    [ordered]@{
        id = "header"
        sample_case_id = "header-insert-before"
        description = "Insert one leading column into the purple direct header target table while keeping the retained blue header table and body text stable."
        ...
        expected_visual_cues = @(
            "The purple direct header target table grows from two columns to three columns with a new leading Priority / Elevated column.",
            "The retained blue direct header table stays unchanged and is not accidentally mutated.",
            "Body text still starts below the header area with no overlap or clipping."
        )
    },
    [ordered]@{
        id = "footer"
        sample_case_id = "footer-insert-after"
        description = "Insert one middle column into the orange direct footer target table while keeping the footer safely inside the page footer area."
        ...
        expected_visual_cues = @(
            "The orange direct footer target table grows from two columns to three columns with a new middle Status / Pinned column.",
            "The retained blue direct footer table stays unchanged above the orange target table.",
            "The footer content stays inside the footer area without colliding with body text."
        )
    },
    [ordered]@{
        id = "body"
        sample_case_id = "body-remove-column"
        description = "Remove the temporary middle column from the green body target table while keeping the retained blue body table and surrounding paragraphs stable."
...
if (-not $SkipBuild) {
    ...
    Write-Step "Building CLI and direct column visual sample"
    Invoke-MsvcCommand -VcvarsPath $vcvarsPath -CommandText "cmake --build `"$resolvedBuildDir`" --target featherdoc_cli featherdoc_sample_template_table_cli_direct_column_visual"
}

$cliExecutable = Find-BuildExecutable -BuildRoot $resolvedBuildDir -TargetName "featherdoc_cli"
$sampleExecutable = Find-BuildExecutable -BuildRoot $resolvedBuildDir -TargetName "featherdoc_sample_template_table_cli_direct_column_visual"
$renderPython = $null
if (-not $SkipVisual) {
...
foreach ($case in $cases) {
    ...
    [void]$commandLines.Add("# Template table CLI direct-column command sequence for $($case.id)")
    ...
    foreach ($step in $steps) {
        ...
        Invoke-CommandCapture `
            ...
            -FailureMessage "Failed during direct template table CLI mutation step '$($step.id)' for case '$($case.id)'."
...
[void]$reviewChecklistLines.Add("# Template table CLI direct-column visual regression checklist")
...
[void]$finalReviewLines.Add("# Template table CLI direct-column final review")
...
Write-Step "Completed template table CLI direct-column visual regression"
