param(
    [string]$BuildDir = "build-template-table-cli-direct-merge-unmerge-visual-nmake",
    [string]$OutputDir = "output/template-table-cli-direct-merge-unmerge-visual-regression",
    [int]$Dpi = 144,
    [switch]$SkipBuild,
    [switch]$SkipVisual,
    [switch]$KeepWordOpen,
...
function Write-Step {
    ...
    Write-Host "[template-table-cli-direct-merge-visual] $Message"
}
...
$cases = @(
    [ordered]@{
        id = "header-merge-right"
        sample_case_id = "header-merge-right"
        description = "Merge the purple direct-header target table horizontally while keeping the retained blue table and body text stable."
        command = "merge-template-table-cells"
        part = "header"
        part_args = @("--part", "header", "--index", "0")
        ...
        expected_visual_cues = @(
            "The retained blue direct header table stays unchanged above the purple target table.",
            "The purple direct header anchor cell becomes a visibly wider horizontal span while the green tail column remains separate.",
            "Body text still starts below the header area with no overlap or clipping."
        )
    },
    [ordered]@{
        id = "footer-unmerge-down"
        sample_case_id = "footer-unmerge-down"
        description = "Split the orange direct-footer pillar while keeping the retained blue footer table and the body/footer separation stable."
        command = "unmerge-template-table-cells"
        part = "footer"
        part_args = @("--part", "footer", "--index", "0")
        target_table_index = "1"
        row_index = "1"
        cell_index = "0"
        command_args = @("--direction", "down")
        baseline_rows = @(
            @("Merged orange pillar", "Footer top"),
            @("", "Footer middle"),
            @("", "Footer bottom")
        )
        mutated_rows = @(
        ...
        expected_visual_cues = @(
            "The retained blue direct footer table stays unchanged above the orange target table.",
            "The orange direct footer pillar is split back into stacked standalone cells with visible horizontal borders.",
            "The footer stays inside the footer area and does not collide with the body text above it."
        )
    },
    [ordered]@{
        id = "body-merge-down"
        sample_case_id = "body-merge-down"
        description = "Merge the green body target table downward while keeping the retained blue body table and following paragraph stable."
        command = "merge-template-table-cells"
        part = "body"
        part_args = @("--part", "body")
        target_table_index = "1"
        ...
        baseline_rows = @(
            @("Green pillar anchor", "Body top"),
            @("Green middle", "Body middle"),
            @("Green tail", "Body bottom")
        )
        mutated_rows = @(
            @("Green pillar anchor", "Body top"),
            @("", "Body middle"),
        ...
        expected_visual_cues = @(
            ...
            "The green first-column cells merge into one vertical pillar spanning all three body rows.",
...
if (-not $SkipBuild) {
    ...
    Write-Step "Building CLI and direct merge/unmerge visual sample"
    Invoke-MsvcCommand -VcvarsPath $vcvarsPath -CommandText "cmake --build `"$resolvedBuildDir`" --target featherdoc_cli featherdoc_sample_template_table_cli_direct_merge_unmerge_visual"
}

$cliExecutable = Find-BuildExecutable -BuildRoot $resolvedBuildDir -TargetName "featherdoc_cli"
$sampleExecutable = Find-BuildExecutable -BuildRoot $resolvedBuildDir -TargetName "featherdoc_sample_template_table_cli_direct_merge_unmerge_visual"
...
foreach ($case in $cases) {
    ...
    [void]$commandLines.Add("# Template table CLI direct merge/unmerge command sequence for $($case.id)")
...
[void]$reviewChecklistLines.Add("# Template table CLI direct merge/unmerge visual regression checklist")
...
[void]$finalReviewLines.Add("# Template table CLI direct merge/unmerge final review")
...
Write-Step "Completed template table CLI direct merge/unmerge visual regression"
