param(
    [string]$BuildDir = "build-template-table-cli-section-kind-row-visual-nmake",
    [string]$OutputDir = "output/template-table-cli-section-kind-row-visual-regression",
    [int]$Dpi = 144,
    [switch]$SkipBuild,
    [switch]$SkipVisual,
    [switch]$KeepWordOpen,
...
function Write-Step {
    ...
    Write-Host "[template-table-cli-section-kind-row-visual] $Message"
}
...
function Get-MutationArguments {
    param($CaseConfig)

    $partArgs = @($CaseConfig.part_args)
    switch ($CaseConfig.command) {
        "append-template-table-row" {
            return @($CaseConfig.target_table_index) + $partArgs
        }
        "insert-template-table-row-before" {
            return @($CaseConfig.target_table_index, $CaseConfig.row_index) + $partArgs
        }
        "insert-template-table-row-after" {
            return @($CaseConfig.target_table_index, $CaseConfig.row_index) + $partArgs
        }
        "remove-template-table-row" {
            return @($CaseConfig.target_table_index, $CaseConfig.row_index) + $partArgs
        }
        default {
            throw "Unsupported row mutation command '$($CaseConfig.command)'."
        }
    }
}
...
function Build-CaseSteps {
    ...
    [void]$steps.Add([ordered]@{
            ...
            args = Get-MutationArguments -CaseConfig $CaseConfig
        })
...
$cases = @(
    [ordered]@{
        ...
        sample_case_id = "section-header-default-insert-before-row"
        description = "Insert a middle row into the default section-header target table and verify the change appears only on the later odd page."
        command = "insert-template-table-row-before"
        mutation_step_id = "insert-row-before"
        part = "section-header"
        ...
        mutated_rows = @(
            @("Default Header Final", "Value"),
            @("Inserted default row", "42"),
            @("Keep default header seed", "Seed detail")
        )
        post_steps = @(
            [ordered]@{
                id = "set-inserted-left"
                row_index = "1"
                cell_index = "0"
                text = "Inserted default row"
            },
            [ordered]@{
                id = "set-inserted-right"
                row_index = "1"
                cell_index = "1"
                text = "42"
            }
        ...
        expected_visual_cues = @(
            "Page 1 stays on the first-page header control path and must not render the default-header row insertion target.",
            "Page 3 keeps the retained blue default-header table unchanged while the purple target table gains a new middle row labelled Inserted default row / 42.",
            "The page 3 body flow still begins below the header area without overlap or clipped text."
        )
    },
    [ordered]@{
        id = "section-header-even"
        sample_case_id = "section-header-even-append-row"
        description = "Append a new trailing row to the even section-header target table and verify the change appears only on page 2."
        command = "append-template-table-row"
        mutation_step_id = "append-row"
        part = "section-header"
        ...
        mutated_rows = @(
            @("Even Header Final", "Value"),
            @("Keep even header seed", "Seed detail"),
            @("Even header appended", "27")
        )
        post_steps = @(
            [ordered]@{
                id = "set-appended-left"
                row_index = "2"
                cell_index = "0"
                text = "Even header appended"
            },
            [ordered]@{
                id = "set-appended-right"
                row_index = "2"
                cell_index = "1"
                text = "27"
            }
        ...
        expected_visual_cues = @(
            "Page 2 keeps the retained blue even-header table unchanged while the peach target table gains a new trailing row labelled Even header appended / 27.",
            "Page 3 still follows the default-header control path and must not show the even-header appended row.",
            "The page 2 body flow still starts below the even header area without overlap or clipping."
        )
    },
    [ordered]@{
        id = "section-footer-first"
        sample_case_id = "section-footer-first-remove-row"
        description = "Remove the middle row from the first-page section-footer target table and verify the change appears only on page 1."
        command = "remove-template-table-row"
        mutation_step_id = "remove-row"
        part = "section-footer"
        ...
        baseline_rows = @(
            ...
            @("Remove this row", "Tighten spacing"),
            @("Keep footer tail", "Seed detail")
        )
        mutated_rows = @(
            @("First Footer Final", "Value"),
            @("Keep footer tail", "Seed detail")
        ...
        expected_visual_cues = @(
            "Page 1 keeps the retained blue first-footer table unchanged while the orange target table shrinks from three rows to two after the middle row is removed.",
            "Page 3 still follows the default-footer control path and must not show the first-page footer row removal.",
            "The shortened first-page footer remains inside the footer area without visibly colliding with the page 1 body paragraphs."
        )
    },
    [ordered]@{
        id = "section-footer-default"
        sample_case_id = "section-footer-default-insert-after-row"
        description = "Insert a new middle row into the default section-footer target table and verify the change appears only on the later odd page."
        command = "insert-template-table-row-after"
        mutation_step_id = "insert-row-after"
        part = "section-footer"
        ...
        expected_visual_cues = @(
            "Page 1 still follows the first-page footer control path and must not render the default-footer row insertion target.",
            "Page 3 keeps the retained blue default-footer table unchanged while the green target table gains a new middle row labelled Default footer bridge / 31.",
            "The page 3 footer remains inside the footer area without visibly overlapping the body paragraphs."
        )
    }
)
...
if (-not $SkipBuild) {
    ...
    Write-Step "Building CLI and section-kind row visual sample"
    Invoke-MsvcCommand -VcvarsPath $vcvarsPath -CommandText "cmake --build `"$resolvedBuildDir`" --target featherdoc_cli featherdoc_sample_template_table_cli_section_kind_row_visual"
}

$cliExecutable = Find-BuildExecutable -BuildRoot $resolvedBuildDir -TargetName "featherdoc_cli"
$sampleExecutable = Find-BuildExecutable -BuildRoot $resolvedBuildDir -TargetName "featherdoc_sample_template_table_cli_section_kind_row_visual"
$renderPython = $null
if (-not $SkipVisual) {
...
foreach ($case in $cases) {
    ...
    [void]$commandLines.Add("# Template table CLI section-kind row command sequence for $($case.id)")
    ...
    $caseSummary = [ordered]@{
        ...
        row_index = if ($null -eq $case.row_index) { $null } else { [int]$case.row_index }
...
[void]$reviewChecklistLines.Add("# Template table CLI section-kind row visual regression checklist")
...
[void]$finalReviewLines.Add("# Template table CLI section-kind row final review")
...
Write-Step "Completed template table CLI section-kind row visual regression"
