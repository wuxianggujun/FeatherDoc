param(
    [string]$BuildDir = "build-msvc-nmake-bookmark-floating-image-visual",
    [string]$OutputDir = "output/bookmark-floating-image-visual-regression",
    [int]$Dpi = 144,
    [switch]$SkipBuild,
    [switch]$KeepWordOpen,
    [switch]$VisibleWord
...
function Write-Step {
    ...
    Write-Host "[bookmark-floating-image-visual] $Message"
}
...
if (-not $SkipBuild) {
    ...
    Write-Step "Building featherdoc_cli and bookmark floating image sample"
    Invoke-MsvcCommand `
        -VcvarsPath $vcvarsPath `
        -CommandText "cmake --build `"$resolvedBuildDir`" --target featherdoc_cli featherdoc_sample_bookmark_floating_image_visual"
}

$cliExecutable = Find-BuildExecutable -BuildRoot $resolvedBuildDir -TargetName "featherdoc_cli"
$sampleExecutable = Find-BuildExecutable -BuildRoot $resolvedBuildDir -TargetName "featherdoc_sample_bookmark_floating_image_visual"
...
Invoke-Capture `
    ...
    -FailureMessage "Failed to generate shared bookmark floating image baseline sample."
...
$caseId = "body-replace-bookmark-floating-image-visual"
...
Invoke-Capture `
    ...
    -FailureMessage "Failed to replace bookmark floating image for case '$caseId'."
...
foreach ($expected in @(
        ...
        '"horizontal_offset_px":320',
        '"vertical_reference":"paragraph"',
        '"vertical_offset_px":0',
        '"behind_text":false',
        '"allow_overlap":false',
        '"wrap_mode":"square"',
        '"wrap_distance_left_px":16',
        '"wrap_distance_right_px":16',
        '"wrap_distance_top_px":8',
        '"wrap_distance_bottom_px":8'
    )) {
...
foreach ($expected in @(
        "part: body",
        "text=This retained line should stay above the anchored image and makes vertical placement easy to compare.",
        "text=This deliberately long paragraph should wrap beside the anchored image on the left side of the page before continuing below it. The wording is intentionally extended so the rendered evidence makes floating layout, spacing, and bookmark replacement order easy to compare.",
        "text=A second retained paragraph continues after the wrapped region and helps confirm the anchored drawing does not reorder surrounding content when the placeholder paragraph is removed.",
        "text=The final retained paragraph should remain below the wrapped text so the rendered evidence keeps a clear reading order."
    )) {
    Assert-Contains -Path $paragraphInspectPath -Expected $expected -Label $caseId
}
Assert-NotContains -Path $paragraphInspectPath -Unexpected "text=body floating placeholder" -Label $caseId
...
Assert-ImageState `
    ...
    -ExpectedFields @(
        ...
        '"width_px":168',
        '"height_px":96',
        '"floating_options":{"horizontal_reference":"margin"',
        '"horizontal_offset_px":320',
        '"vertical_reference":"paragraph"',
        '"vertical_offset_px":0',
        '"behind_text":false',
    ...
    cases = @(
        [ordered]@{
            ...
            expected_visual_cues = @(
                "The placeholder is replaced by a visible anchored red image near the right margin.",
                "The long paragraph wraps beside the image before continuing below it.",
                "The retained paragraphs above and below the wrap region keep their reading order."
            )
...
Write-Step "Completed bookmark floating image visual regression"
