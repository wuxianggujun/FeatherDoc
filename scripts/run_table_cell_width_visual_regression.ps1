param(
    [string]$BuildDir = "build-table-cell-width-visual-nmake",
    [string]$OutputDir = "output/table-cell-width-visual-regression",
    [int]$Dpi = 144,
    [switch]$SkipBuild,
    [switch]$KeepWordOpen,
    [switch]$VisibleWord
...
function Write-Step {
    ...
    Write-Host "[table-cell-width-visual] $Message"
}
...
function Assert-TableCellWidthState {
    param(
        [string]$JsonPath,
        ...
        [string]$ExpectedText,
        [AllowNull()][Nullable[uint32]]$ExpectedWidthTwips,
        [string]$Label
    )
    ...
    if ($null -eq $ExpectedWidthTwips) {
        if ($null -ne $cell.width_twips) {
            throw "$Label expected width_twips=null, got '$($cell.width_twips)'."
        }
    } elseif ([uint32]$cell.width_twips -ne $ExpectedWidthTwips) {
        throw "$Label expected width_twips '$ExpectedWidthTwips', got '$($cell.width_twips)'."
    }
}

function Assert-SetTableCellWidthResult {
    param(
        [string]$JsonPath,
        [int]$ExpectedTableIndex,
        [int]$ExpectedRowIndex,
        [int]$ExpectedCellIndex,
        [uint32]$ExpectedWidthTwips,
        [string]$Label
    )

    $payload = Read-JsonFile -Path $JsonPath
    if ([string]$payload.command -ne "set-table-cell-width") {
        throw "$Label expected command set-table-cell-width, got '$($payload.command)'."
    }
    ...
    if ([uint32]$payload.width_twips -ne $ExpectedWidthTwips) {
        throw "$Label expected width_twips '$ExpectedWidthTwips', got '$($payload.width_twips)'."
    }
}

function Assert-ClearTableCellWidthResult {
    param(
        [string]$JsonPath,
        [int]$ExpectedTableIndex,
        [int]$ExpectedRowIndex,
    ...
    $payload = Read-JsonFile -Path $JsonPath
    if ([string]$payload.command -ne "clear-table-cell-width") {
        throw "$Label expected command clear-table-cell-width, got '$($payload.command)'."
    }
    if (-not [bool]$payload.ok) {
        throw "$Label expected ok=true."
    }
...
function Get-BodyTableCellXmlWidthState {
    param(
        [string]$DocxPath,
        [int]$TableIndex,
        [int]$RowIndex,
    ...
    $widthNode = $cellNodes[$CellIndex].SelectSingleNode("w:tcPr/w:tcW", $namespaceManager)
    $hasWidth = $null -ne $widthNode
    $type = $null
    $value = $null
    if ($hasWidth) {
        $typeAttribute = $widthNode.GetAttribute("type", $script:WordMlNamespace)
        $valueAttribute = $widthNode.GetAttribute("w", $script:WordMlNamespace)
...
function Assert-BodyTableCellXmlWidthState {
    param(
        [object]$State,
        [bool]$ExpectedHasWidth,
        [AllowNull()][string]$ExpectedType,
        [AllowNull()][string]$ExpectedValue,
        [string]$Label
    )

    if ([bool]$State.has_width -ne $ExpectedHasWidth) {
        throw "$Label expected has_width=$ExpectedHasWidth, got $($State.has_width)."
    }

    if (-not $ExpectedHasWidth) {
        return
    }

    if ([string]$State.type -ne $ExpectedType) {
        throw "$Label expected XML tcW type '$ExpectedType', got '$($State.type)'."
    }
    if ([string]$State.value -ne $ExpectedValue) {
        throw "$Label expected XML tcW value '$ExpectedValue', got '$($State.value)'."
    }
}
...
if (-not $SkipBuild) {
    ...
    Write-Step "Building featherdoc_cli and table cell width visual sample"
    Invoke-MsvcCommand `
        -VcvarsPath $vcvarsPath `
        -CommandText "cmake --build `"$resolvedBuildDir`" --target featherdoc_cli featherdoc_sample_table_cell_width_visual"
}

$cliExecutable = Find-BuildExecutable -BuildRoot $resolvedBuildDir -TargetName "featherdoc_cli"
$sampleExecutable = Find-BuildExecutable -BuildRoot $resolvedBuildDir -TargetName "featherdoc_sample_table_cell_width_visual"
...
if ($LASTEXITCODE -ne 0) {
    throw "Failed to generate shared table cell width baseline sample."
}
...
Assert-TableCellWidthState `
    -JsonPath $baselineSetTargetJson `
    -ExpectedTableIndex 0 `
    -ExpectedRowIndex 0 `
    -ExpectedCellIndex 0 `
    -ExpectedColumnSpan 1 `
    -ExpectedText "SET WIDTH SHOULD WRAP THIS SENTENCE INTO A NARROW COLUMN" `
    -ExpectedWidthTwips $null `
...
Assert-TableCellWidthState `
    ...
    -ExpectedCellIndex 0 `
    -ExpectedColumnSpan 1 `
    -ExpectedText "CLEAR WIDTH SHOULD RETURN THIS SENTENCE TO A WIDE AUTO TABLE" `
    -ExpectedWidthTwips 2222 `
    -Label "shared-baseline-table-1-target-cell"

$baselineSetTargetXmlState = Get-BodyTableCellXmlWidthState `
    -DocxPath $baselineDocxPath `
    -TableIndex 0 `
    -RowIndex 0 `
    -CellIndex 0
Assert-BodyTableCellXmlWidthState `
    -State $baselineSetTargetXmlState `
    -ExpectedHasWidth $false `
    -ExpectedType $null `
    -ExpectedValue $null `
    -Label "shared-baseline-table-0-target-width-xml"

$baselineClearTargetXmlState = Get-BodyTableCellXmlWidthState `
    -DocxPath $baselineDocxPath `
    -TableIndex 1 `
    -RowIndex 0 `
    -CellIndex 0
Assert-BodyTableCellXmlWidthState `
    ...
    -ExpectedHasWidth $true `
    -ExpectedType "dxa" `
    -ExpectedValue "2222" `
    -Label "shared-baseline-table-1-target-width-xml"
...
$caseDefinitions = @(
    [pscustomobject][ordered]@{
        id = "set-table-cell-width-2222-visual"
        command = "set-table-cell-width"
        arguments = @("0", "0", "0", "2222")
        expected_visual_cues = @(
            "The blue table in case 0 collapses from auto width into a visibly narrower single-cell column.",
            "The target sentence wraps into many more lines after the width is forced to 2222 twips.",
            "The yellow pre-seeded narrow table in case 1 stays unchanged in this mutation."
        )
    },
    [pscustomobject][ordered]@{
        id = "clear-table-cell-width-2222-visual"
        command = "clear-table-cell-width"
        arguments = @("1", "0", "0")
        expected_visual_cues = @(
            "The yellow table in case 1 expands from the narrow 2222-twip column back to auto width.",
            "The target sentence occupies fewer wrapped lines after the explicit cell width is cleared.",
            "The blue table in case 0 remains auto width in this mutation."
        )
    }
)
...
foreach ($case in $caseDefinitions) {
    ...
    if ($case.command -eq "set-table-cell-width") {
        Assert-SetTableCellWidthResult `
            ...
            -ExpectedWidthTwips 2222 `
            -Label $case.id
    } elseif ($case.command -eq "clear-table-cell-width") {
        Assert-ClearTableCellWidthResult `
            -JsonPath $mutationJsonPath `
            -ExpectedTableIndex 1 `
            -ExpectedRowIndex 0 `
            -ExpectedCellIndex 0 `
    ...
    $setTargetXmlState = Get-BodyTableCellXmlWidthState `
        -DocxPath $mutatedDocxPath `
        -TableIndex 0 `
        -RowIndex 0 `
        -CellIndex 0
    $clearTargetXmlState = Get-BodyTableCellXmlWidthState `
        -DocxPath $mutatedDocxPath `
        -TableIndex 1 `
        -RowIndex 0 `
        -CellIndex 0

    if ($case.id -eq "set-table-cell-width-2222-visual") {
        Assert-TableCellWidthState `
        ...
        Assert-BodyTableCellXmlWidthState `
            ...
            -ExpectedType "dxa" `
            -ExpectedValue "2222" `
            -Label "$($case.id)-table-0-target-width-xml"
        Assert-BodyTableCellXmlWidthState `
            -State $clearTargetXmlState `
            -ExpectedHasWidth $true `
            -ExpectedType "dxa" `
            -ExpectedValue "2222" `
            -Label "$($case.id)-table-1-target-width-xml"
    } elseif ($case.id -eq "clear-table-cell-width-2222-visual") {
        Assert-TableCellWidthState `
            -JsonPath $setTargetJsonPath `
            -ExpectedTableIndex 0 `
            -ExpectedRowIndex 0 `
    ...
    $summaryCases += [ordered]@{
        ...
        table_0_target_width_xml = $setTargetXmlState
        table_1_target_width_xml = $clearTargetXmlState
...
$summary = [ordered]@{
    ...
    shared_baseline_artifacts = [ordered]@{
        ...
        table_0_target_width_xml = $baselineSetTargetXmlState
        table_1_target_width_xml = $baselineClearTargetXmlState
    }
    cases = $summaryCases
...
Write-Step "Completed table cell width visual regression"
