...
````md
- 用 5 到 8 条 bullet 概括：可视化验证入口、安装包入口、CI metadata、fixed-grid /
  merge / unmerge / 列编辑能力、fixed-grid 四件套联系图，以及当前验证结论。
...
## 核心变化
- 根据 `CHANGELOG.md` 总结本次 API、行为边界或打包层面的主要变化。
...
- 想走最短路径时先看 `VISUAL_VALIDATION_QUICKSTART*.md`；需要更完整的图 ->
  脚本 -> 输出目录映射时，再看 `VISUAL_VALIDATION*.md`。
