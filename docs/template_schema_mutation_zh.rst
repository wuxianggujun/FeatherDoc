...
输出长期趋势 JSON / Markdown。目录模式只递归收集携带 project-template smoke 或
schema approval 字段的 ``summary.json``，避免把无关治理摘要和
...
``mismatched_corpus_metadata_count``。如果候选缺少项目、模板或来源 summary，报告会产生
...
和 ``add_business_template_corpus_role_metadata`` action item；如果候选上的
``business_document_type`` / ``corpus_role`` 与来源语料条目不一致，会产生
``schema_patch_confidence_calibration.mismatched_business_template_corpus_metadata`` warning
和 ``align_business_template_corpus_metadata`` action item。这样后续继续扩大合同、
