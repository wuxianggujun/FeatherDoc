...
- ``content_control_data_binding_governance.md`` / ``summary.json``
- ``project_template_delivery_readiness.md`` / ``summary.json``
- ``table_layout_delivery_rollup.md`` / ``summary.json``
- bundle 入口文档，例如 ``START_HERE.md``、``ARTIFACT_GUIDE.md``、
  ``REVIEWER_CHECKLIST.md``、``release_handoff.md``

能在上游治理报告生成的 warning，应该在源报告阶段就写出结构化字段。例如
``document_skeleton_governance_rollup`` 和 ``numbering_catalog_governance`` 的
style merge suggestion warning 会从源头携带 ``source_schema``、``action`` 与
``style_merge_suggestion_count`` / ``style_merge_suggestion_pending_count``；
对于 document skeleton 来源，这个 count 表示
应用 ``style_merge_suggestion_pending_count`` 与 ``style_merge_suggestion_review``
...
``table_layout_delivery_governance`` 的 evidence skipped、evidence read failure
和缺少 table layout rollup 的 warning 也使用相同结构。
