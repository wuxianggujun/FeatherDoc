...
``style_merge_suggestion_count``；``project_template_delivery_readiness`` 与
``table_layout_delivery_rollup`` 的 warning 会携带 ``source_schema``、``action`` 和
``message``。后续 release blocker rollup / handoff / pipeline 只负责保真透传和
统一渲染。
