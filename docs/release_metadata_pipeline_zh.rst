...
``project_id``、``template_name``、``source_schema`` 与 ``source_json_display``，
同时把 reviewer 可执行的 ``open_command`` 带入 final rollup。
``write_project_template_schema_approval_history.ps1`` 也会按
``project_id`` / ``template_name`` / approval item name 生成复合 entry history，
避免不同项目的同名模板互相合并。reviewer 在 ``START_HERE.md``、
...
onboarding governance 证据，再按 ``project_template`` 明细定位项目和模板，最后按
``open_command`` 同步或复核 schema approval 记录。

schema confidence calibration 会写出
``featherdoc.schema_patch_confidence_calibration_report.v1``。当 calibration 里仍有
pending approval outcome 或 invalid approval record 时，报告会分别把
``schema_patch_confidence_calibration.pending_schema_approvals`` 与
``schema_patch_confidence_calibration.invalid_approval_records`` 写入
``release_blockers``；未打分 candidate 会进入 ``warnings``；所有 recommendation
会同步成 ``action_items`` 并携带重建校准报告的 ``open_command``。
...
handoff details parity 已完成首轮收口：同一批 release blocker、warning 和 action
item 会在 ``release_governance_handoff``、``release_blocker_rollup``、pipeline
``stages[]``、release summary、``final_review.md``、``ARTIFACT_GUIDE.md``、
``REVIEWER_CHECKLIST.md`` 和 ``release_handoff.md`` 中保持字段一致。当前固定的
reviewer-facing 字段是 ``id``、``action``、``message``、``source_schema``、
``source_report_display``、``source_json_display`` 与 action item ``open_command``。
项目模板治理项还会在这些明细中保留 ``project_id`` 与 ``template_name``；bundle
渲染层会把它们合成为 ``project_template: project_id=... template_name=...``。

需要注意的是，``source_report_display`` 表示当前被 handoff 或 rollup 加载的源报告；
在 release candidate auto-discovery 场景中，它可能指向
``auto-discover-output/.../summary.json``。``source_json_display`` 则表示治理项声明的
原始证据入口，例如 ``.\output\schema-patch-confidence-calibration\summary.json``。
内部 ``release_governance_handoff.md`` 会用单反引号 Markdown 展示这些字段，关键片段
如下::

   schema=`featherdoc.project_template_onboarding_governance_report.v1`
   project_template: project_id=project-alpha template_name=invoice-template
   source_json_display: `.\output\project-template-onboarding-governance\summary.json`
   open_command: `pwsh -ExecutionPolicy Bypass -File ...`

任何新增治理源进入发布 pipeline 前，都应先补这一组 reviewer-facing 字段的生成和测试。

这一规则现在由 release note bundle 入口强制执行。``write_release_note_bundle.ps1``
在生成 ``release_handoff.md``、``ARTIFACT_GUIDE.md``、``REVIEWER_CHECKLIST.md`` 和
``START_HERE.md`` 前会校验 ``release_blocker_rollup``、``release_governance_handoff``
以及可选 pipeline ``stages[]`` 中的 reviewer-facing 明细。blocker / warning 缺
``id``、``action``、``message``、``source_schema``、``source_report_display`` 或
``source_json_display`` 会直接失败；action item 缺 ``id``、``action``、
``open_command``、``source_schema``、``source_report_display`` 或
``source_json_display`` 也会失败。

为了避免旧治理源只提供 action id 而没有复核入口，handoff、rollup 和 pipeline 的
归一化层会补安全默认值：handoff 缺 ``open_command`` 时使用默认治理报告的 build
command；rollup 按源 schema 补对应治理报告脚本；pipeline 按 stage 脚本补
``pwsh -ExecutionPolicy Bypass -File ...``。新增治理源仍应优先在自身 summary 中显式
写出 ``open_command``，默认值只用于保证 reviewer 不会拿到无复核入口的 action item。
