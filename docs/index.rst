...
``output_encoding`` = ``UTF-8 without BOM``. Duplicate script references include
``occurrence_lines`` and ``occurrence_groups`` so reviewers can locate repeated
entries without a separate search. The Markdown report also exposes
``powershell_version`` alongside ``checked_at_utc`` so automation can preserve the
