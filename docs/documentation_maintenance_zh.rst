...
8. ``docs/script_task_index_zh.rst``：脚本任务索引，固定 ``scripts`` 目录里
   ...
   ``total_script_reference_count=164``、``unindexed_script_count=0``、
   ``duplicate_script_reference_count=0``、``missing_script_count=0``。
...
9. ``scripts/pdf_import_diagnostics_contract_fields.ps1`` 已纳入
   ``docs/script_task_index_zh.rst`` 的 PDF 保守维护入口分组；它已被多个
   bounded CTest 与 release material 脚本引用，并且已经被脚本索引正式覆盖。
...
6. 清理本地生成产物时只移除被 ``.gitignore`` 明确覆盖的内容，例如
   ``docs/_build/``、根目录 ``START_HERE.md``、``output/`` 和顶层
   ``featherdoc-*.pdf``；受版本控制的归档文档继续保留。


删除规则
--------

删除文件或目录属于高风险操作。已确认的生成产物可直接清理；对受版本控制
的归档文档或历史记录，默认保留，除非它们不再被任何索引、安装规则或契约
测试引用。
