...
1. 同一 ``1.13.x`` patch 线保持已发布公共调用方式的源码兼容，并用冻结的
   ``v1.13.2`` consumer 编译测试守护。
2. ``FeatherDoc`` 公开类包含私有实现布局，当前不承诺整个 ``1.x`` 跨 minor 的
   shared ABI 兼容。Linux/macOS 共享库 ``SOVERSION`` 使用 ``major.minor``，安装包
   通过 ``FeatherDoc_ABI_VERSION`` 暴露该值。
3. 从 ``1.13.x`` 升级到 ``1.14.x`` 时，下游必须重新编译；不得只替换动态库。
   同一 minor 内的 patch 发布以 ABI 兼容为目标，例外必须在 CHANGELOG 和发布说明
   中显式标记。
...
正式发版前推荐启用严格 release gate，把 release blocker rollup 与 governance
handoff 中的 blocker、missing report 和 warning 都当作阻断项处理：
