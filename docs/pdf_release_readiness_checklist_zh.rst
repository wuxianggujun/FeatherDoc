...
6. 发布治理已消费 PDF 结论：
   ...
   * ``release_handoff.md`` 中 ``PDF visual gate summary:`` 行必须直接携带
     ...
     或其它段落替代当前 handoff 证据。
     固定标记：``line_scoped_release_handoff_pdf_visual_trace``。
   * ``release_summary.zh-CN.md`` 的 PDF visual gate 短摘要行必须直接携带
     ``summary.json``、``aggregate-contact-sheet.png``、verdict、CJK count
     和 visual baseline count；``release_body.zh-CN.md`` 的
     PDF visual gate summary / contact-sheet 行也必须直接携带对应路径；
     不能让 detached notes 或其它段落补齐发布说明证据。
