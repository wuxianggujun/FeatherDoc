...
``checked_document_count``、``checked_document_labels``、
``checked_document_relative_paths``、``required_marker_count`` 等数量字段；其中
...
渲染或部分治理 fixture 的测试可以考虑使用 ``-SkipMaterialSafetyAudit``，但正式
发布路径不能跳过 material safety audit。
