...
1. 先低资源确认当前 build 目录、``CMakeCache.txt``、CTest 注册、PDF 输出和可复用渲染 Python 是否完整。
...
但它不是 release-ready evidence；``-SkipPreflight`` 只应在已经确认输出完整时使用。
...
2026-05-20 PDF 预检 runbook 资源回收补充
------------------------------------------

本轮把 PDF visual release gate 的发布阻断 runbook 继续收紧：无论 reviewer 处理
``prepare_pdf_visual_release_gate_build_outputs`` 还是
``rerun_pdf_visual_release_gate_preflight``，固定指引都会提醒先运行轻量预检，并在预检或
完整 gate 结束后只清理本任务启动的 PDF gate 进程和临时输出。

该补充不改变远端 ``origin/codex/*`` 分支状态，也不代表旧 PDF visual baseline 已经通过。
它只把资源回收规则写入 release blocker checklist，避免后续可视化验证阶段误结束外部
Office、浏览器、node、PowerShell 或其它非 FeatherDoc 构建进程。

2026-05-20 PDF CJK 静态契约收口
--------------------------------

本轮继续只读复核 ``origin/codex/pdf-cjk-copy-search-gate`` 和
``origin/codex/pdf-cjk-bullet-fallback``，仍不整分支合并、不强推、不删除远端分支。

manifest ID 对比结果：

* ``origin/codex/pdf-cjk-copy-search-gate``：分支样例 ID 为 70 个，当前 ``dev`` 为
  90 个，分支独有样例 ID 为 0。
* ``origin/codex/pdf-cjk-bullet-fallback``：分支样例 ID 为 73 个，当前 ``dev`` 为
  90 个，分支独有样例 ID 为 0。

本轮按当前 ``dev`` 的轻量契约结构补齐两个静态缺口：

* ``377dc1c``：让 manifest parser 测试显式断言
  ``document-cjk-bullet-list-text`` 和 ``document-cjk-numbered-list-text``。
* ``b41c3a8``：让 ``pdf_document_table_font_matrix_contract_test.ps1`` 从 manifest
  动态计算样例数量，避免旧的 ``82U`` 硬编码再次误报。

已通过的轻量验证均使用 60 秒超时包装，覆盖：

* PDF RTL bidi、font matrix、text decoration、CJK copy/search lite、CJK list regression。
* CJK bullet overlay、numbered list page-flow、list page-flow、CLI CJK export、
  copy/search matrix。
* CJK font embed、wrap mix、style overlay、anchor matrix、table wrap、repeated key、
  vertical merge、complex layout、image wrap stress、fallback table、multi-anchor table、
  search density 和 page boundary。
* document style gallery、document table CJK wrap / merged repeat /
  vertical merged cant-split、document table font matrix、document table vertical merged
  cant-split 和 bidi line layout 静态契约。

未完成项需要保持准确记录：``pdf_ctest_label_contract_test.ps1`` 和
``pdf_ctest_timeout_contract_test.ps1`` 需要带 ``set_tests_properties`` 的真实
``CTestTestfile.cmake``。本轮只找到 preflight fake 输出，里面只有 ``add_test``，
不能替代真实 CTest 产物；低资源约束下未运行 CMake 生成新文件。

因此，当前结论进一步收敛为：两个 PDF CJK 参考分支的 manifest 样例入口已在 ``dev``
...
2026-05-20 PDF 预检目录复用与 CJK 分支直接 diff 复核
----------------------------------------------------

本轮先提交并推送 ``186ff73 Auto-detect reusable PDF preflight build dir``。
该提交让 ``scripts/check_pdf_visual_release_gate_preflight.ps1`` 在默认
``.bpdf-roundtrip-msvc`` 不存在时，自动复用现有 ``build`` 或 ``out\build``，
并在 summary 中记录 ``build_dir_source`` 和 ``requested_build_dir``。

提交后真实只读预检结果仍为 ``not_ready``，但状态已经更明确：

* ``build_dir`` 解析为当前工作区的 ``build``。
* ``build_dir_source`` 为 ``auto:build``。
* 阻断项仍是缺少真实 ``CMakeCache.txt``、``CTestTestfile.cmake``、``pdf_cli_export`` baseline PDFs、
  42 个 visual baseline PDFs 和 43 个 CJK text-layer PDFs。

后续 ``3264183 Tighten PDF preflight build auto-detection`` 又收紧了这个规则：默认
``.bpdf-roundtrip-msvc`` 不存在时，``build`` / ``out\build`` 只有在包含
``CMakeCache.txt`` 或 ``CTestTestfile.cmake`` 时才会自动选为可复用 build。当前真实
轻量预检因此回到 ``build_dir_source = requested``，并明确把缺失
``.bpdf-roundtrip-msvc`` 本身计入阻断项；这比把普通 ``build`` 目录误判为可复用
build 更适合后续 release blocker 和分支清理判断。

随后改用 ``dev..origin/codex/...`` 直接比较分支尖端，而不是只看
``dev...origin/codex/...`` 的共同祖先差异。结论是：

* ``origin/codex/pdf-cjk-copy-search-gate`` 和
  ``origin/codex/pdf-cjk-bullet-fallback`` 都明显落后于当前 ``dev``，直接切换到
  分支版本会删除当前已经恢复和治理过的大量脚本、文档、PDF import 文件和静态契约测试。
* 当前 ``dev`` 已有 ``check_pdf_text_layer.py``、CJK copy/search gate、CLI CJK
  export coverage、CJK / East Asia font fallback、exact-height table header fitting、
  CJK list/table/font matrix/copy-search manifest 入口和静态契约。
* 本轮只从旧分支保留一个低风险注释意图：在 ``check_pdf_text_layer.py`` 中说明文本层匹配
  同时检查原文、折叠空白和去空白三种形态，以容忍 PDF 断行、CJK 换行和排版间距差异。

因此，两个 PDF CJK 参考分支当前没有新的独立源码小补丁适合继续搬入。后续最小风险动作是
补齐可复用 build / CTest manifest / PDF baseline 输出，让预检从 ``not_ready`` 进入
``ready``，再执行受控可视化验证；在此之前不删除远端 PDF 参考分支。

2026-05-20 release governance 与 PDF 参考分支小批次收口
----------------------------------------------------------------

本轮继续复用当前 ``dev`` 工作区，未运行 CMake、CTest、Ninja、MSBuild、Word、
LibreOffice、浏览器或 PDF 渲染。只做 git、PowerShell、JSON / Markdown 级别的低资源复核。

本轮已经落入 ``dev`` 并推送的低风险补丁包括：

* ``compare_pdf_reference_branch_manifest.ps1``：把两个 PDF CJK 参考分支的 manifest
  样例 ID 与当前 ``dev`` 做机器可复核比对。真实比对结果显示
  ``origin/codex/pdf-cjk-copy-search-gate`` 的 70 个样例 ID 和
  ``origin/codex/pdf-cjk-bullet-fallback`` 的 73 个样例 ID 在当前 ``dev`` 中缺口均为 0。
* release governance pipeline 会在既有
  ``pdf-visual-release-gate-preflight-governance\summary.json`` 存在时，把它自动纳入
  final governance inputs，避免 PDF preflight blocker 游离在最终发布治理之外。
* ``build_release_governance_pipeline_report_test.ps1 -Scenario fail_on_blocker`` 已注册到
  CMake 测试入口，保证 release blocker 非空时的失败路径不只停留在脚本参数层。
* release governance pipeline 顶层 summary 现在直接暴露最终 rollup 的
  ``release_blockers``、``action_items``、``warnings`` 和 ``governance_detail_source``，
  发布材料可以从顶层 summary 追踪 blocker 来源，而不必手工翻嵌套 stage。

本轮复核后，四个远端参考分支仍不适合整分支合并：

* ``origin/codex/pdf-cjk-copy-search-gate``：仍有 31 个右侧提交，但 manifest 样例入口缺口
  已为 0；剩余价值主要是旧 visual baseline、大型样例和历史 gate 形态。
* ``origin/codex/pdf-cjk-bullet-fallback``：仍有 36 个右侧提交，但当前 ``dev`` 已覆盖 CJK
  list / font fallback / exact-height table header / CLI CJK export 等关键契约；整分支会回退
  当前 PDF shaping 和治理脚本。
* ``origin/codex/release-governance-rollup-details``：仍有 8 个右侧提交。当前 ``dev`` 已覆盖
  schema calibration candidate routing、release blocker rollup 的 candidate/project/template
  明细和 pipeline 顶层治理详情；剩余提交不再作为可直接摘入的小补丁处理。
* ``origin/codex/release-governance-warning-entrypoints``：仍有 46 个右侧提交。当前 ``dev`` 已覆盖
  source schema / source JSON / open command / PDF preflight runbook / reviewer checklist
  入口；剩余大块包含旧 style-merge 恢复流程和旧 release metadata 入口，不能为减少分支数量而
  直接倒灌。
...
2026-05-20 受控 PDF smoke 检查器后的参考分支状态
-----------------------------------------------

本轮在 ``dev`` 与 ``origin/dev`` 对齐、工作区干净的前提下继续推进。新增并推送
``ee81820 Add controlled PDF visual smoke checker``，把既有
``output/pdf-controlled-visual-smoke-20260520`` 的人工 smoke 证据固化为低资源机器检查。

已验证结果：

* ``pdf_controlled_visual_smoke_check_test.ps1`` 在 60 秒超时包装下通过，覆盖正常 PNG
  通过和空白 PNG 失败两个路径。
* ``check_pdf_controlled_visual_smoke.ps1`` 对真实 ``minimal`` / ``rerun`` smoke 产物返回
  ``pass``；页面 PNG 可解码、尺寸满足阈值、非白像素比例足够，文本层锚点存在。
* ``compare_pdf_reference_branch_manifest.ps1 -FailOnMissingInDev`` 再次返回通过：
  ``origin/codex/pdf-cjk-copy-search-gate`` 样例数 70、缺口 0；
  ``origin/codex/pdf-cjk-bullet-fallback`` 样例数 73、缺口 0；当前 ``dev`` 样例数 90。

分支处理结论不变，但证据更强：两个 PDF CJK 参考分支不再有明确 manifest 样例 ID
缺口，也没有新的低风险源码小补丁适合继续拆入。它们仍不能立即删除，因为完整 PDF visual
release gate 还缺当前 build / CTest / PDF baseline 输出，旧分支仍保留 visual baseline
和重型 gate 历史证据。下一步不是继续搬分支代码，而是准备可复用 build 输出，让 PDF
preflight 从 ``not_ready`` 进入 ``ready`` 后再做受控可视化验证。


Archived continuation: ``docs/stale_codex_branch_inventory_archive_2026_05_zh.rst``.
