...
6. 公开材料安全审计已覆盖：
   ...
   * ``release_assets_manifest.json`` 必须保留
     ...
     ``source_json_display``。两条契约的 source display 必须能识别自身证据源：
   ...
   * ``START_HERE.md``、``ARTIFACT_GUIDE.md`` 与 ``REVIEWER_CHECKLIST.md`` 必须
     ...
     ``schema_approval_status_summary``、``source_report_display`` 和
     ``source_json_display``。这条人工签核入口不能
   ...
   * ``scripts/run_release_candidate_checks.ps1`` 生成的 ``summary.json`` 还必须提供
     ...
     两条 required contracts、7 required fields 和
   ...
   * ``scripts/package_release_assets.ps1`` 写出的 ``release_assets_manifest.json`` 必须继续
     ...
     ``path_display``、两条 project-template contract、7 required fields 与
