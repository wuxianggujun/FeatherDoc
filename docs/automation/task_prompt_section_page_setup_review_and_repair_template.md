# Section Page Setup Review And Repair Task
...
请先做严格视觉检查；若检查不通过，再继续修复 section page setup
生成逻辑并重新做完整回归。不要手工改 Word 成品，也不要只修单页。

强制约束：

1. 一切结论必须基于 Word 最终截图证据，而不是 sample 返回值、
   CLI inspect 输出或 XML 结构。
...
- `api-sample`：第一页是否仍为竖版，第二页是否仍为横版。
- `cli-rewrite`：第一页是否成功从竖版变成横版，且第二页仍保持横版。
- 两个 case 是否都保持两页输出，没有裁切、边距错乱、页眉页脚漂移或
  非预期分页变化。
- CLI 改写后的页面几何是否与预期一致，且没有把未指定字段写坏。
