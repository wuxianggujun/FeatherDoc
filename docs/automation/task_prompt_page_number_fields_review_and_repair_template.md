# Page Number Fields Review And Repair Task
...
请先做严格视觉检查；若检查不通过，再继续修复页码字段生成逻辑并重新做
完整回归。不要手工改 Word 成品，也不要只修单页。

强制约束：

1. 一切结论必须基于 Word 最终截图证据，而不是 sample 返回值、CLI
   JSON、字段 XML 或字段计数。
2. 第一轮优先使用当前任务目录里的现成 bundle 证据；只有在关键证据缺失、
   路径无效或明显过期时，才执行 `{{BUNDLE_REFRESH_COMMAND}}`。
3. 第一轮必须先查看 aggregate contact sheet，再根据
   `bundle_review_manifest.json` 和 case `field_summary.json` 逐 case 检查。
4. 不得覆盖原始 source bundle。
5. 每一轮修复都必须写到 `{{REPAIR_DIR}}` 下的新子目录，例如
   `fix-01\\bundle-regression`。
6. 每次修复后都必须重新执行完整 page number fields 双 case 回归，
   不允许只重跑 `api-sample` 或 `cli-append` 之一。
...
1. 完成一轮完整视觉检查：
   ...
   - 按 manifest 的 `expected_visual_cues` 与 case `field_summary.json`
     逐 case 检查；
   - 输出“通过 / 不通过 / 无法判定”。
2. 若结论为“不通过”，根据截图判断问题更像是：
   - `PAGE` / `NUMPAGES` 字段插入到了错误的 part；
   - section header/footer 继承关系写坏；
   - 字段插入后导致页眉页脚布局溢出或裁切；
   - CLI 局部更新覆盖了不该改动的内容；
   - reopen 后 Word 未正确刷新字段显示。
3. 修改生成逻辑，而不是手工改 Word 成品。
4. 每轮修复后执行一套新的双 case 回归，输出到 `repair\\fix-XX` 下的
   repo-relative 目录。例如：
   `{{BUNDLE_REPAIR_COMMAND_EXAMPLE}}`
5. 基于新回归目录中的 aggregate contact sheet、review manifest、
   case `field_summary.json` 和截图证据重新做完整检查。
6. 必须将本轮结果同步回写到：
...
- `api-sample`：页眉中的当前页码与页脚中的总页数是否都已真实渲染。
- `cli-append`：两个 section 的 header/footer 是否都能正确显示新增字段。
- `cli-append`：第一页与第二页的当前页码是否不同，而总页数是否保持一致。
- 两个 case 是否都没有出现裁切、错位、页眉页脚丢失或非预期分页变化。
- CLI 追加字段后，原有 portrait/landscape 几何与 section 边界是否仍正常。
