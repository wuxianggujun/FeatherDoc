...
这些 content-control 治理项现在会在源报告中固定写出 ``source_schema``、``source_json_display`` 与 action item 的 ``open_command``；修复计划里的 action-derived 条目也会继续保留同一个 ``open_command``，并通过 release blocker rollup 进入 release candidate ``summary.json``、``final_review.md`` 和 reviewer-facing bundle，避免 reviewer 只能看到计数而缺少证据入口。
