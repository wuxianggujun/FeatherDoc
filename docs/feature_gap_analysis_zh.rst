...
当前列表、编号定义、样式挂接编号已经有不错基础，但对既有文档里的
复杂 numbering catalog 还缺少更系统的治理入口。

建议补齐：

- 导出既有 numbering catalog 为稳定 JSON
- 从 JSON 或 exemplar 文档导入 numbering definition
- 检查 paragraph style 与 numbering definition 的绑定关系
- 管理 level override、restart、abstractNum / num 实例映射
- 检测重复、孤儿、冲突的 numbering 定义
...
当前已经有样式检查、继承解析、局部 rebase 和 materialize 能力。
下一步缺的是更完整的“样式重构 workflow”。

建议补齐：

- 批量重命名 style id，并更新引用
- 合并重复样式
- 删除未使用样式
- 检查 basedOn 链路循环、断链和隐藏样式
- 生成 style usage report
...
- numbering catalog import / export
- style usage report
- style rename / merge / prune
