...
- 发布治理 handoff、rollup 和 pipeline 明细在 reviewer-facing bundle 中的一致展示
...
PDF 子系统当前不再评估切换到 LibreOffice 方案。历史 LibreOffice PDF
研究页已经从 Sphinx 主文档中删除，策略背景保留在 ``design/00-vision.md`` 和
``design/dependencies/pdfio.md``。后续 PDF 差距只按 PDFio / PDFium 路线管理：
中文复制 / 搜索验收、复杂导入边界、视觉发布门禁和必要的 PDFium raw 形态验证。
...
的长期变化和每个模板 entry 的审批走势。entry history 现在按 ``project_id`` /
``template_name`` / approval item name 生成复合定位，避免同名模板在不同项目之间
合并审批历史。onboarding summary 和人工复核文档会同步展示
...
blocker / action item 现在会携带 ``project_id``、``template_name``、
``source_schema``、``source_json_display`` 与 reviewer ``open_command``，并经
...
把无效审批记录也升级成 release blocker，把未打分候选保留为 warning，并把
recommendation 同步为带 ``open_command`` 的 action item；默认 release blocker auto-discovery、governance
...
``open_command``，而不必只依赖 handoff 计数。项目模板治理项还会保留
``project_id`` 与 ``template_name``，bundle 渲染层会输出
...
第一阶段：发布治理 handoff 明细收口（已完成首轮）
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

当前已完成：

- 已核对 ``release_governance_handoff``、``release_blocker_rollup`` 与
  pipeline ``stages[]`` 的 blocker / warning / action item 字段是否一致
- 已确保 release summary、``final_review.md``、``ARTIFACT_GUIDE.md``、
  ``REVIEWER_CHECKLIST.md`` 和 ``release_handoff.md`` 展示同一批明细
- 已固定 ``source_schema``、``source_report_display``、``source_json_display`` 与
  action item ``open_command`` 的 reviewer-facing 展示规则
- release note bundle 入口已增加 admission gate，缺上述字段的 handoff / rollup /
  pipeline stage 明细会在生成 reviewer-facing bundle 前失败

阶段目标：让 reviewer 能直接打开证据和复核命令，而不是只看到 handoff 计数。
