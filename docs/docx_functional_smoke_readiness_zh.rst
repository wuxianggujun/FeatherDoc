...
JSON summary 和 Markdown 报告固定使用 ``UTF-8 without BOM``。summary 中的
``output_encoding`` 固定写为 ``UTF-8 without BOM``，Markdown 报告也会显示
``Output encoding``，便于 release reviewer 和自动化在不同 PowerShell 运行环境下
确认产物编码。
...
重新渲染文档。
发布治理路径固定为 ``output/docx-functional-smoke-readiness/summary.json`` 和
``output/docx-functional-smoke-readiness/docx_functional_smoke_readiness.md``；低资源
手工复核默认路径固定为 ``output/docx-functional-smoke-readiness-current/summary.json``
与 ``output/docx-functional-smoke-readiness-current/docx_functional_smoke_readiness.md``。

如果必需证据缺失，summary 中的 blocker 会使用固定 action
``restore_docx_functional_smoke_evidence``。release reviewer 需要先打开
``source_report_display`` / ``source_json_display`` 指向的
``docx-functional-smoke-readiness`` 证据，恢复缺失的 DOCX package、功能样例或
Word visual smoke PNG 证据，再运行
``check_docx_functional_smoke_readiness.ps1``。该 action 的固定 runbook 明确说明
此检查仍是 read-only、低资源证据复用，不是新鲜 Word COM 渲染；通过后还需要重建
release governance pipeline / handoff 并重新生成 release note bundle。
...
``docx_functional_smoke_readiness.md``；对当前本地复核目录，这两个文件分别是
...
如果没有显式传入 ``-VisualSmokeRoots``，而默认的历史 Word visual smoke 目录已经被
清理或只剩空壳，脚本会在 ``-ReleaseAssetsRoot`` 指向的 release assets 根目录下查找
与当前 ``CMakeLists.txt`` 版本一致的 ``FeatherDoc-v*-visual-validation-gallery.zip``
和 ``FeatherDoc-v*-release-evidence.zip``。当 gallery 中的 visual smoke contact sheet /
page PNG 非空，且 release evidence 中的标准 Word visual review metadata 为
``reviewed`` / ``pass`` 时，summary 会记录
``visual_evidence_mode = release_asset_visual_gallery``，并把详情写入
``release_asset_visual_gallery_evidence``。这只是复用已发布/已打包的持久化截图证据，
仍不会启动新的 Word COM 渲染。
...
.. code-block:: powershell
   ...
   powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\check_docx_functional_smoke_readiness.ps1 `
     -RepoRoot . `
     -OutputDir .\output\docx-functional-smoke-readiness-current `
     -ReleaseAssetsRoot .\output\release-assets
