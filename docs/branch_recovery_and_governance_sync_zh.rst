...
继续只读复核 ``b52bf60``、``96bac82`` 和 ``11dc255`` 后，当前 ``dev`` 已经覆盖
它们的关键源码能力：``pdf_font_resolver.cpp`` 已具备 FreeType 字形检查、
Unicode prefix 到 East Asia / CJK 字体链路的 fallback、日文假名、韩文音节和东亚
兼容符号识别；``test/pdf_font_resolver_tests.cpp`` 和
