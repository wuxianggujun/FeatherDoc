...
3. ``P1-RELEASE-01`` 继续作为 DOING 项：release body / summary 与 GitHub
   Release 同步路径已由值级回归锁定；下一轮优先复核 release blocker rollup、
   release governance handoff、final review、bundle 和 checklist 的同源字段是否
   仍存在漂移，特别关注新增业务维度字段是否继续出现在 manifest signoff 与入口材料中。
   优先补薄弱测试，不做大重构。
