...
3. ``P1-RELEASE-01`` 继续作为 DOING 项：优先复核 release blocker rollup、
   ...
   仍存在漂移；特别关注新增业务维度字段是否继续出现在 manifest signoff、入口材料、
   release body / summary 和 GitHub Release 同步路径中。release body / summary 已由
   release note bundle 回归锁定，下一步优先审计 GitHub Release 同步路径。优先补薄弱
   测试，不做大重构。
