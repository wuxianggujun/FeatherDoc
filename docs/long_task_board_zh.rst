...
7. ``P1-RELEASE-01``：release governance 材料一致性
   ...
   * 当前产物：``repair_action_classes`` 已从 content-control governance report
     ...
     ``/releases/download/<tag>/<asset>.zip``，path basename 必须等于同名正式 ZIP
     文件名，并包含请求发布 tag 的 release download 路径，防止 wrong-file URL、
     wrong-tag URL、query / fragment 参数污染、query-only 文件名或非 release
     download 路径被写入 ``release_assets_manifest.json``。
     ``upload.release_url`` 也已纳入同一类 path 级 contract：必须是有效
     HTTP(S) absolute URL，不能带 query / fragment，且 path 必须指向
     ``/releases/tag/<tag>``，防止 wrong-tag、query-only tag、query / fragment
     参数污染、非 release tag path 或非 HTTP URL 被记录为
     正式发布页证据。
...
2. 继续守护本轮 ``upload.remote_assets`` 静态 contract、远端 URL release
   download path / 文件名 / tag / query-fragment / scheme / host / release path prefix 绑定、
