...
8. ``P2-STYLE-01``：样式建议置信度校准
   ...
   * 当前产物：``write_style_merge_suggestion_review.ps1`` 已把低于
     ...
     ``manual_review_before_apply``。对应单测和 style merge confidence route 文档契约已覆盖。
...
6. ``P2-STYLE-01`` 已补低置信度 style merge manual-review reasons；下一轮若继续该项，
   优先把这组字段透传到 document skeleton governance / release blocker rollup。
