...
3. ``P1-TEMPLATE-01``：真实业务模板语料扩展
   ...
   * 当前产物：``samples/project_template_smoke.manifest.json`` 已增加
     ...
     规划 tender；contract 已通过 ``contract-template`` 注册为轻量 schema-baseline /
     ...
     readiness warning 已暴露 registered / planned corpus 计数。当前 tender 没有可直接
     注册的轻量采购样本，因此先保持 planned，不硬推到 registered。
     ``check_project_template_smoke_manifest_test.ps1`` 现在还会直接校验仓库真实
     manifest，锁住 invoice、contract、policy、report、notice、tender 6 类
     document type，以及 5 个 registered / 1 个 planned corpus 入口；剩余 planned
...
4. ``P1-SCHEMA-01`` 本轮已补 ``business_document_type`` 缺失治理信号与固定
   reviewer action；``P1-TEMPLATE-01`` 已把 notice、policy、report、contract 从
   planned 推进为 registered。下一轮优先找或生成 tender 小样本；若没有新的 tender
   样本资产，就先保持 tender 为 planned，继续维护文档和治理说明；等样本到位后，再做
   注册薄片、补脚本、测试和文档，再提交到 ``dev``。
