...
3. ``P1-TEMPLATE-01``：真实业务模板语料扩展
   ...
   * 当前产物：``samples/project_template_smoke.manifest.json`` 已增加
     ``business_template_corpus``，注册 invoice、notice、policy，规划 contract、report、
     tender；notice 已通过 ``part-template-validation-smoke`` 注册为轻量通知模板语料，
     policy 已通过 ``resolved-schema-baseline-smoke`` 注册为轻量 schema-baseline 语料；
     ``describe_project_template_smoke_manifest.ps1`` 和 delivery readiness warning 已暴露
     registered / planned corpus 计数。
     ``check_project_template_smoke_manifest_test.ps1`` 现在还会直接校验仓库真实
     manifest，锁住 invoice、contract、policy、report、notice、tender 6 类
     document type，以及 3 个 registered / 3 个 planned corpus 入口；剩余 planned
...
4. ``P1-SCHEMA-01`` 本轮已补 ``business_document_type`` 缺失治理信号与固定
   reviewer action；``P1-TEMPLATE-01`` 已把 notice 和 policy 从 planned 推进为
   registered。下一轮若要继续推进功能，继续做 contract / report / tender 注册薄片，
   补脚本、测试和文档，再提交到 ``dev``。
