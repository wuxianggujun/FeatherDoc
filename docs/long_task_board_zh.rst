...
7. ``P1-RELEASE-01``：release governance 材料一致性
   ...
   * 当前产物：``repair_action_classes`` 已从 content-control governance report
     ...
     ``be2af9a541c92f4f859b5efdd9a0a7ccdc157f01`` 已补齐这两份 Markdown 对
...
1. 复查最新 ``dev`` CI；当前跟踪对象是
   ``be2af9a541c92f4f859b5efdd9a0a7ccdc157f01`` 的 Linux、Windows 和 macOS
   CMake workflow，失败就先修失败。
...
3. reviewer action Markdown 透传已完成；release material safety 已补
   ``upload.remote_assets`` 的严格整数校验和 decimal 负例。继续复核 release
   asset manifest 对新增发布字段的静态 contract 是否仍存在盲区；优先补薄弱
   测试，不做大重构。
