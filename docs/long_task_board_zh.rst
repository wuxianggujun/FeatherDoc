...
3. ``P1-TEMPLATE-01``：真实业务模板语料扩展
   ...
   * 当前产物：``samples/project_template_smoke.manifest.json`` 已增加
     ``business_template_corpus``，注册 invoice、notice、policy、report，规划
     contract、tender；notice 已通过 ``part-template-validation-smoke`` 注册为轻量
     ...
     visual-smoke 语料；``describe_project_template_smoke_manifest.ps1`` 和 delivery
     readiness warning 已暴露 registered / planned corpus 计数。当前仓库没有可直接
     注册的 contract.docx / tender.docx 轻量样本，因此这两项先保持 planned，不硬推
     到 registered。
     ``check_project_template_smoke_manifest_test.ps1`` 现在还会直接校验仓库真实
     manifest，锁住 invoice、contract、policy、report、notice、tender 6 类
     document type，以及 4 个 registered / 2 个 planned corpus 入口；剩余 planned
     入口仍必须保留 ``registration_blocker`` 与 ``next_action``，并由
     ``describe_project_template_smoke_manifest.ps1`` 汇总为
     ``planned_business_template_registration_actions``，让下一轮可以直接按阻断原因把
     计划语料推进到 registered。manifest-only delivery readiness warning 也会把
     这组 ``planned_business_template_registration_actions`` 透传到 JSON 与
     Markdown，包含每条 action 的 ``id``、``registration_blocker`` 和
     ``next_action``。
...
7. ``P1-RELEASE-01``：release governance 材料一致性
   ...
   * 当前产物：``repair_action_classes`` 已从 content-control governance report
     ...
     最新提交 ``70fca44c39d0342cf552109a2e2c7dc43c01c1a9`` 进一步把
...
4. ``P1-SCHEMA-01`` 本轮已补 ``business_document_type`` 缺失治理信号与固定
   reviewer action；``P1-TEMPLATE-01`` 已把 notice、policy、report 从 planned 推进为
   registered。下一轮若没有新的 contract / tender 样本资产，就先保持这两项为
   planned，优先维护文档和治理说明；等样本到位后，再做注册薄片、补脚本、测试和
   文档，再提交到 ``dev``。
