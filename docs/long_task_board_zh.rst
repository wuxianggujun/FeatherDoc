...
7. ``P1-RELEASE-01``：release governance 材料一致性
   ...
   * 当前产物：``repair_action_classes`` 已从 content-control governance report
     ...
     必须包含同名正式 ZIP 文件名，并包含请求发布 tag 的路径片段，防止
     wrong-file URL 或 wrong-tag URL 被写入 ``release_assets_manifest.json``。
...
2. 继续守护本轮 ``upload.remote_assets`` 静态 contract、远端 URL 文件名/tag
   绑定、warning-only reviewer action 回归和 content-control 同块断言的 CI；
