...
1. ``P0-CI-01``：dev CI 守护
   ...
   * 当前修复：``ec098480b4b9259afdc0b5964c74ccff5b11751d`` 的 Linux、macOS 与
     Windows CMake CI 均长时间停在 Build step。Windows 已有 Build step timeout
     与 verbose；本轮继续给 Linux/macOS Build step 补 ``timeout-minutes`` 与
     ``--verbose``，避免后续构建卡住时没有定位输出。
   * 备注：这项优先级高于继续堆叠功能。
...
3. ``P1-TEMPLATE-01``：真实业务模板语料扩展
   ...
   * 当前产物：``samples/project_template_smoke.manifest.json`` 已增加
     ...
     document type，以及 1 个 registered / 5 个 planned corpus 入口；planned
     入口现在还必须保留 ``registration_blocker`` 与 ``next_action``，并由
...
1. 继续等待当前 ``dev`` 的 Windows MSVC CI 完成；失败就先抓日志修失败。
...
3. ``P1-RELEASE-01`` 继续作为 DOING 项：优先复核 release blocker rollup、
   release governance handoff、final review、bundle 和 checklist 的同源字段是否
   仍存在漂移；优先补薄弱测试，不做大重构。
4. 若要继续推进功能，优先从 ``P1-SCHEMA-01`` 或 ``P1-TEMPLATE-01`` 里选一个
   最小闭环，补脚本、测试和文档，再提交到 ``dev``。
