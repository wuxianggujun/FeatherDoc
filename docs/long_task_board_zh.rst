...
2. ``P1-SCHEMA-01``：schema patch confidence 的真实业务语料来源追踪
   ...
   * 当前产物：``business_template_corpus_summary``、
     ...
     本轮又把这组字段继续接入 release candidate 的 ``release_handoff.md`` 与
     ``final_review.md`` 断言，确保 ``business_document_type``、
     ``source_business_document_type``、``corpus_role``、``source_corpus_role`` 和
     mismatch count / flag 不会只停留在 JSON handoff 中。
...
4. ``P1-SCHEMA-01`` 本轮已补 ``business_document_type`` / ``corpus_role`` 缺失治理信号、
   固定 reviewer action，以及 schema/corpus metadata mismatch 在 handoff / final review
   中的 reviewer-facing 字段展示；``P1-TEMPLATE-01`` 已把 notice、policy、report、
