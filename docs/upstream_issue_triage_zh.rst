...
``#83 Can I create a table?``
    这个缺口已经部分补上。FeatherDoc 现在支持
    ``append_table()`` / ``append_row()`` / ``append_cell()`` 这一层的基础
    表格创建与扩展，也已经覆盖单元格横向 / 纵向合并、单元格 / 表格宽度、
    单元格 / 表格边框、表格样式引用、表格固定 / 自动布局切换、表格对齐 /
    缩进、表格级默认单元格边距，以及单元格边距 / 底纹、行高控制、
    行分页控制、单元格垂直对齐，以及重复表头行。
    当前剩下的主要缺口是更高层的自定义表格样式定义编辑，以及更细粒度的
    表格版式属性。
...
- 更高层的高级图片版式控制 API，例如更细粒度的浮动定位、复杂文字环绕与裁剪
- 更高层的自定义表格样式 / 更细粒度表格版式 API
- 公式读取 API
- 加密 ``.docx`` 支持
