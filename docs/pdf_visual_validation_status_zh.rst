...
资源窗口重新生成。
公共 API 入口头变更不代表 PDF visual gate 证据刷新；缺少本地 ``output/``
产物时仍按既有 preflight/readiness 边界解释。
...
fresh 非 ``FinalizeOnly`` attempt 未完成且辅助证据仍不完整的事实写成顶层
``warnings[]``，稳定 id 为
``pdf_visual_gate_attempt.incomplete_fresh_render``。该 warning 必须直接携带
...
release candidate / governance 不再把该事实计入 warning_count；单次 fresh full
gate 未闭合边界仍保留在 readiness 与 segmented evidence 字段里。
...
``visual_gate_segmented_full_coverage_evidence = true`` 并接受为发布证据；若
``attempt-summary.json`` 也已补齐 44/44 与 contact sheet pass，该边界不再占用
release candidate / governance warning_count。
