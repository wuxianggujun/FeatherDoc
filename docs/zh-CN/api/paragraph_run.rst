...
底层 ``w:sz`` 按 ``1..UINT32_MAX`` 范围内的严格无符号 half-point 文本解析。
空值、符号、尾随字符、零和越界值都会返回空 optional，不再接受部分合法前缀。

文本修改事务与句柄
------------------

``Paragraph::set_text(...)``、``Run::set_text(...)``、``add_run(...)``、相邻
段落/run 插入，以及 ``insert_*_like_*`` 都先构建完整的未发布 XML。节点名、属性名、
属性值、文本、换行和格式复制中的任一分配失败都会返回 ``false`` 或空句柄，不留下
空段落、空 run 或不完整格式；原 XML 和已有句柄保持不变。中文、emoji、前后空白及
``CR``/``LF``/``CRLF`` 文本均按 UTF-8 处理。

成功执行 ``Paragraph::set_text(...)`` 后，原 ``Paragraph`` 句柄仍有效，但被替换的
旧 ``Run`` 及其后代句柄失效；应从该段落重新获取 run。成功执行
``Run::set_text(...)`` 后，原 ``Run`` 句柄仍有效。插入和追加成功只新增返回句柄所指向
的子树，不会使未删除的兄弟句柄失效。
