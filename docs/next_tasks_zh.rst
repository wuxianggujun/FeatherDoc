...
3. 维护 release material safety：
   ...
   * ``upload.remote_assets`` 的 URL contract 继续收紧：三份正式 ZIP 的 URL
     ...
     文件名只在 query 中出现，以及 tag 出现在非 release download 路径中的场景。
...
4. ``P1-SCHEMA-01`` 已先补 schema patch confidence calibration 的业务模板来源摘要和
   ...
   path、文件名、tag、userinfo、query / fragment、scheme、host 和 release path prefix 绑定，并把
   ``upload.release_url`` 收紧到无 userinfo、query / fragment 的 release tag path。下一步继续守护最新 ``dev`` CI，
