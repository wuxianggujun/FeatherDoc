...
3. 维护 release material safety：
   ...
   * ``upload.remote_assets`` 的 URL contract 继续收紧：三份正式 ZIP 的 URL
     必须是无 query / fragment 的规范 URL，并通过 URL path 指向
     ``/releases/download/<tag>/<asset>.zip``；其中 path basename 必须等于同名资产文件，
     release download 路径必须包含请求发布 tag。material safety 负例已覆盖
     wrong-file URL、wrong-tag URL、query / fragment 参数污染、文件名只在 query
     中出现，以及 tag 出现在非 release download 路径中的场景。
   * ``upload.release_url`` 也已纳入 path 级 release tag contract：必须是有效
     HTTP(S) absolute URL，不能带 query / fragment，并且 URL path 必须指向
     ``/releases/tag/<tag>``；wrong-tag、query-only tag、query / fragment 参数污染、
     非 release tag path 和非 HTTP URL 均会触发
     material safety 失败。
...
4. ``P1-SCHEMA-01`` 已先补 schema patch confidence calibration 的业务模板来源摘要和
   ...
   path、文件名、tag、query / fragment、scheme、host 和 release path prefix 绑定，并把
   ``upload.release_url`` 收紧到无 query / fragment 的 release tag path。下一步继续守护最新 ``dev`` CI，
