...
同时开启 PDFio 写出和 PDFium 读入后，可以直接跑首批 45 个 regression manifest 样本。
`pdf_regression_*` 当前覆盖 46 个 CTest，其中包含 manifest 校验测试。
