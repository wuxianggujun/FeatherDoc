...
$sampleId = "document-cjk-font-embed-matrix-text"
$sampleKind = "document_cjk_font_embed_matrix_text"
$builderName = "build_document_cjk_font_embed_matrix_text_sample"
$styleBuilderName = "define_document_cjk_font_embed_lite_styles"
$outputFile = "featherdoc-pdf-regression-document-cjk-font-embed-matrix-text.pdf"
...
Assert-ContainsText -Text $sampleText -ExpectedText "Document CJK Font Embed Lite" `
    -Message "PDF CJK font embed matrix should map an East Asia font family."
Assert-ContainsText -Text $sampleText -ExpectedText "Document CJK font embed matrix sample" `
    -Message "PDF CJK font embed matrix should keep the full-id title."
Assert-ContainsText -Text $sampleText -ExpectedText "ensure_section_header_paragraphs" `
    -Message "PDF CJK font embed matrix should keep header coverage."
Assert-ContainsText -Text $sampleText -ExpectedText "ensure_section_footer_paragraphs" `
    -Message "PDF CJK font embed matrix should keep footer coverage."
Assert-ContainsText -Text $sampleText -ExpectedText "render_headers_and_footers = true" `
    -Message "PDF CJK font embed matrix should render headers and footers."
Assert-ContainsText -Text $sampleText -ExpectedText "expand_header_footer_page_placeholders = true" `
    -Message "PDF CJK font embed matrix should expand page placeholders."
Assert-ContainsText -Text $sampleText -ExpectedText "append_table(6U, 2U)" `
    -Message "PDF CJK font embed matrix should keep table key coverage."
Assert-ContainsText -Text $sampleText -ExpectedText "set_repeats_header" `
    -Message "PDF CJK font embed matrix should keep repeated header coverage."
Assert-ContainsText -Text $sampleText -ExpectedText "DocumentPdfCjkFontEmbedLiteAccent" `
    -Message "PDF CJK font embed matrix should exercise styled accent text."
Assert-ContainsText -Text $sampleText -ExpectedText "DocumentPdfCjkFontEmbedLiteLarge" `
    -Message "PDF CJK font embed matrix should exercise large styled text."
...
foreach ($stableKey in @(
    ...
    Assert-ContainsText -Text $sampleText -ExpectedText $stableKey `
        -Message "PDF CJK font embed matrix sample should keep stable key '$stableKey'."
    Assert-ContainsText -Text $manifestSampleBlock -ExpectedText $stableKey `
        -Message "PDF CJK font embed matrix manifest should include stable key '$stableKey'."
}
...
Assert-ContainsText -Text $manifestSampleBlock -ExpectedText '"expected_pages": 1' `
    -Message "PDF CJK font embed matrix should stay a one-page lightweight sample."
Assert-ContainsText -Text $manifestSampleBlock -ExpectedText '"expect_cjk": true' `
    -Message "PDF CJK font embed matrix manifest should require CJK handling."
Assert-ContainsText -Text $manifestSampleBlock -ExpectedText '"expect_unicode": true' `
    -Message "PDF CJK font embed matrix manifest should require Unicode handling."
Assert-ContainsText -Text $manifestSampleBlock -ExpectedText '"expect_styled_text": true' `
    -Message "PDF CJK font embed matrix manifest should mark styled text."
Assert-ContainsText -Text $manifestSampleBlock -ExpectedText '"expect_visual_baseline": true' `
    -Message "PDF CJK font embed matrix manifest should keep visual baseline marker."
...
Assert-ContainsText -Text $manifestTestText -ExpectedText "REQUIRE_EQ(samples.size(), 84U)" `
...
Assert-ContainsText -Text $cmakeText -ExpectedText "pdf_cjk_font_embed_matrix_contract" `
    -Message "CMake should register the PDF CJK font embed matrix static contract."
Assert-ContainsText -Text $cmakeText -ExpectedText "pdf_cjk_font_embed_matrix_contract_test.ps1" `
    -Message "CMake should point at the PDF CJK font embed matrix static contract script."

Write-Host "PDF CJK font embed matrix contract passed."
