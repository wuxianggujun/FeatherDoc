...
(@{
    generated_at = "2026-04-20T15:00:00"
...
(@{
    generated_at = "2026-04-20T15:05:00"
...
Assert-True -Condition $failedAsExpected `
    -Message "sync_latest_visual_review_verdict.ps1 unexpectedly accepted a missing explicit release summary path."
Assert-True -Condition ($errorText -match [regex]::Escape("Expected explicit release-candidate summary was not found: $releaseSummaryPath")) `
    -Message "Missing release summary error did not include the summary path: $errorText"

Assert-True -Condition ((Get-Content -Raw -LiteralPath $gateSummaryPath) -eq $gateSummaryBefore) `
    -Message "Gate summary should not be mutated when explicit release summary path is missing."
Assert-PathNotExists -Path $releaseSummaryPath -Label "release summary"
...
Write-Host "Sync latest visual review verdict missing release summary regression passed."
