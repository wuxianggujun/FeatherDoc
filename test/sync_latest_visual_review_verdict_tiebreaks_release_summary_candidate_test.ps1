...
$releaseRoot = Join-Path $resolvedWorkingDir "release-candidate-checks-alpha"
$releaseReportDir = Join-Path $releaseRoot "report"
$olderReleaseRoot = Join-Path $resolvedWorkingDir "release-candidate-checks-zulu"
$olderReleaseReportDir = Join-Path $olderReleaseRoot "report"
...
New-Item -ItemType Directory -Path $olderReleaseReportDir -Force | Out-Null
...
$olderReleaseSummaryPath = Join-Path $olderReleaseReportDir "summary.json"
...
(@{
    generated_at = "2026-04-14T08:55:00"
    execution_status = "pass"
    visual_verdict = "pending_manual_review"
    gate_output_dir = $gateRoot
    task_output_root = $tasksRoot
    build_dir = (Join-Path $resolvedWorkingDir "older-build")
    install_dir = (Join-Path $resolvedWorkingDir "older-install")
    consumer_build_dir = (Join-Path $resolvedWorkingDir "older-consumer")
    release_handoff = (Join-Path $olderReleaseReportDir "release_handoff.md")
    release_body_zh_cn = (Join-Path $olderReleaseReportDir "release_body.zh-CN.md")
    release_summary_zh_cn = (Join-Path $olderReleaseReportDir "release_summary.zh-CN.md")
    artifact_guide = (Join-Path $olderReleaseReportDir "ARTIFACT_GUIDE.md")
    reviewer_checklist = (Join-Path $olderReleaseReportDir "REVIEWER_CHECKLIST.md")
    start_here = (Join-Path $olderReleaseRoot "START_HERE.md")
    readme_gallery = @{ status = "not_requested" }
    steps = @{
        configure = @{ status = "completed" }
        build = @{ status = "completed" }
...
} | ConvertTo-Json -Depth 8) | Set-Content -LiteralPath $olderReleaseSummaryPath -Encoding UTF8
(Get-Item -LiteralPath $olderReleaseSummaryPath).LastWriteTimeUtc = [datetime]"2026-04-14T09:00:00Z"
(Get-Item -LiteralPath $releaseSummaryPath).LastWriteTimeUtc = [datetime]"2026-04-14T09:00:00Z"
$olderReleaseSummaryBefore = Get-Content -Raw -LiteralPath $olderReleaseSummaryPath
...
$olderReleaseFinalReviewPath = Join-Path $olderReleaseReportDir "final_review.md"
$olderReleaseHandoffPath = Join-Path $olderReleaseReportDir "release_handoff.md"
$olderReleaseBodyPath = Join-Path $olderReleaseReportDir "release_body.zh-CN.md"
$olderArtifactGuidePath = Join-Path $olderReleaseReportDir "ARTIFACT_GUIDE.md"
$olderReviewerChecklistPath = Join-Path $olderReleaseReportDir "REVIEWER_CHECKLIST.md"
$olderStartHerePath = Join-Path $olderReleaseRoot "START_HERE.md"
...
Assert-True -Condition ((Get-Content -Raw -LiteralPath $olderReleaseSummaryPath) -eq $olderReleaseSummaryBefore) `
    -Message "Path-sorted later matching release summary should not be refreshed when timestamps tie."
foreach ($pathToCheck in @($olderReleaseFinalReviewPath, $olderReleaseHandoffPath, $olderReleaseBodyPath, $olderArtifactGuidePath, $olderReviewerChecklistPath, $olderStartHerePath)) {
...
Write-Host "Sync latest visual review verdict release-summary candidate tie-break regression passed."
