...
(@{
    ...
    page_number_fields = @{
        status = "completed"
        summary_json = (Join-Path $gateRoot "page-number-fields\summary.json")
    }
    review_tasks = @{
        document = @{
        ...
        page_number_fields = @{
            ...
            review_result_path = (Join-Path $pageNumberFieldsTaskDir "report\review_result.json")
            final_review_path = (Join-Path $pageNumberFieldsTaskDir "report\final_review.md")
        }
    }
...
Assert-True -Condition ($gateSummary.manual_review.tasks.page_number_fields.verdict -eq "pass") `
