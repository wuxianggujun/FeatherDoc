...
foreach ($pair in @(
        @{ Text = $englishParagraphRunApi; Markers = @("featherdoc::Paragraph", "featherdoc::Run", "Common Tasks", "Success And Failure Semantics", "Short Example", "Typed Signature Guide", "bool set_text(const std::string &text) const", "Run add_run(const std::string &text, formatting_flag formatting = formatting_flag::none)", "Paragraph insert_paragraph_after(const std::string &text, formatting_flag formatting = formatting_flag::none)", "bool set_font_family(std::string_view font_family) const", "set_alignment", "set_font_family", "set_rtl") },
        @{ Text = $englishTableApi; Markers = @("featherdoc::Table", "TableRow", "TableCell", "Common Tasks", "Success And Failure Semantics", "Short Example", "Indexing, Units, And Return Semantics", "Typed Signature Guide", "std::optional<TableCell> find_cell(std::size_t row_index, std::size_t cell_index)", "bool set_cell_block_texts(std::size_t start_row_index, std::size_t start_cell_index, const std::vector<std::vector<std::string>> &rows)", "bool TableCell::merge_right(std::size_t additional_cells = 1U)", "find_cell_by_grid_column", "set_cell_block_texts", "set_repeats_header", "merge_right", "set_fill_color") },
        @{ Text = $englishImagesApi; Markers = @("featherdoc::inline_image_info", "featherdoc::drawing_image_info", "Common Tasks", "Success And Failure Semantics", "Short Example", "FDOC_EN_IMAGES_TYPED_SIGNATURE_GUIDE", "bool append_image(const std::filesystem::path &image_path, std::uint32_t width_px, std::uint32_t height_px)", "bool extract_inline_image(std::size_t image_index, const std::filesystem::path &output_path) const", "append_image", "append_floating_image", "extract_drawing_image", "replace_inline_image", "remove_inline_image") },
