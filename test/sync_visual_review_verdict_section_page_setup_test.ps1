...
function New-TaskReviewSeed {
    ...
    (@{
        ...
        review_note = "internal section page setup sign-off note"
        findings = @()
        notes = @("verified")
...
foreach ($pathToCheck in @($releaseHandoffPath, $artifactGuidePath, $reviewerChecklistPath, $startHerePath)) {
    Assert-Contains -Path $pathToCheck -ExpectedText "Section page setup reviewed at: 2026-04-13T12:33:00" -Label $pathToCheck
    Assert-Contains -Path $pathToCheck -ExpectedText "Section page setup review method: operator_supplied" -Label $pathToCheck
}

Assert-NotContains -Path $releaseBodyPath -UnexpectedText "2026-04-13T12:33:00" -Label "release_body.zh-CN.md"
Assert-NotContains -Path $releaseBodyPath -UnexpectedText "operator_supplied" -Label "release_body.zh-CN.md"
Assert-NotContains -Path $releaseBodyPath -UnexpectedText "internal section page setup sign-off note" -Label "release_body.zh-CN.md"
