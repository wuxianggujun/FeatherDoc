...
auto find_numbering_abstract_xml_node(pugi::xml_node numbering_root,
    ...
    for (auto abstract_num = numbering_root.child("w:abstractNum");
        ...
        if (name != pugi::xml_node{} &&
            std::string_view{name.attribute("w:val").value()} ==
                definition_name) {
            return abstract_num;
        }
    }
...
auto find_numbering_level_xml_node(pugi::xml_node abstract_num,
                                   std::string_view level) -> pugi::xml_node {
    for (auto numbering_level = abstract_num.child("w:lvl");
        ...
        if (std::string_view{numbering_level.attribute("w:ilvl").value()} ==
            level) {
...
TEST_CASE("cli set-paragraph-style-numbering links a custom numbering "
          "definition to a style") {
    const fs::path working_directory = fs::current_path();
    const fs::path source =
        working_directory / "cli_style_numbering_source.docx";
    const fs::path updated =
        working_directory / "cli_style_numbering_updated.docx";
    const fs::path output =
        working_directory / "cli_style_numbering_output.json";
    ...
    CHECK_EQ(
        run_cli({"set-paragraph-style-numbering", source.string(),
                 "LegalHeading", "--definition-name", "LegalHeadingOutline",
                 "--numbering-level", "0:decimal:1:%1.", "--numbering-level",
                 "1:decimal:1:%1.%2.", "--style-level", "1", "--output",
                 updated.string(), "--json"},
                output),
        0);
    CHECK_EQ(
        ...
        std::string{
            ...
            "\"style_id\":\"LegalHeading\",\"definition_name\":"
            "\"LegalHeadingOutline\","
            "\"style_level\":1,\"definition_levels\":[{\"level\":0,\"kind\":"
            "\"decimal\","
            "\"start\":1,\"text_pattern\":\"%1.\"},{\"level\":1,\"kind\":"
            "\"decimal\","
    ...
    CHECK_EQ(
        std::string_view{
            style_num_pr.child("w:ilvl").attribute("w:val").value()},
        "1");
    CHECK_NE(
        std::string_view{
            style_num_pr.child("w:numId").attribute("w:val").value()},
        "");
    ...
    CHECK_EQ(
        std::string_view{
            level_zero.child("w:numFmt").attribute("w:val").value()},
        "decimal");
    CHECK_EQ(
        std::string_view{
            level_zero.child("w:start").attribute("w:val").value()},
        "1");
    CHECK_EQ(
        std::string_view{
            level_zero.child("w:lvlText").attribute("w:val").value()},
        "%1.");
    ...
    CHECK_EQ(
        std::string_view{
            level_one.child("w:numFmt").attribute("w:val").value()},
        "decimal");
    CHECK_EQ(
        std::string_view{level_one.child("w:start").attribute("w:val").value()},
        "1");
    CHECK_EQ(
        std::string_view{
            level_one.child("w:lvlText").attribute("w:val").value()},
        "%1.%2.");

    CHECK_EQ(run_cli({"inspect-styles", updated.string(), "--style",
                      "LegalHeading", "--json"},
                     inspect_output),
             0);
    ...
    const auto definition_id =
        std::string{abstract_num.attribute("w:abstractNumId").value()};
    CHECK_EQ(
        read_text_file(inspect_output),
        std::string{"{\"style\":{\"style_id\":\"LegalHeading\",\"name\":"
                    "\"Legal Heading\","
                    "\"based_on\":\"Heading1\",\"kind\":\"paragraph\",\"type\":"
                    "\"paragraph\","
                    "\"numbering\":{\"num_id\":"} +
            style_num_id + ",\"level\":1,\"definition_id\":" + definition_id +
            ",\"definition_name\":\"LegalHeadingOutline\",\"instance\":{"
            "\"instance_id\":" +
            style_num_id +
            ",\"level_overrides\":[]}},"
    ...
    CHECK_EQ(
        run_cli({"inspect-styles", updated.string(), "--style", "LegalHeading"},
                inspect_text_output),
        0);
    const auto inspect_text = read_text_file(inspect_text_output);
    CHECK_NE(inspect_text.find(
                 "numbering: (level=1, num_id=" + style_num_id +
...
TEST_CASE("cli inspect-style-numbering lists numbered paragraph styles only") {
    ...
    CHECK_EQ(
        run_cli({"ensure-style-linked-numbering", source.string(),
                 "--definition-name", "SharedOutline", "--numbering-level",
                 "0:decimal:1:%1.", "--numbering-level", "1:decimal:1:%1.%2.",
                 "--style-link", "LegalHeading:0", "--style-link",
                 "BodyNumbered:1", "--output", updated.string(), "--json"},
                mutate_output),
        0);
    ...
    CHECK_EQ(
        run_cli({"inspect-style-numbering", updated.string()}, text_output), 0);
...
TEST_CASE("cli inspect-style-numbering reports missing input as json") {
    ...
    CHECK_EQ(
        read_text_file(output),
        std::string{"{\"command\":\"inspect-style-numbering\",\"ok\":false,"
...
TEST_CASE("cli audit-style-numbering reports clean numbered styles") {
    ...
    CHECK_EQ(
        run_cli({"set-paragraph-style-numbering", source.string(),
                 "LegalHeading", "--definition-name", "LegalHeadingOutline",
                 "--numbering-level", "0:decimal:1:%1.", "--output",
                 updated.string(), "--json"},
                mutate_output),
        0);

    CHECK_EQ(run_cli({"audit-style-numbering", updated.string(), "--json",
                      "--fail-on-issue"},
                     audit_output),
             0);
    ...
    CHECK_NE(audit_json.find("\"numbered_style_count\":1"), std::string::npos);
    CHECK_NE(audit_json.find("\"issue_count\":0"), std::string::npos);
    CHECK_NE(audit_json.find("\"suggestion_count\":0"), std::string::npos);
...
TEST_CASE("cli audit-style-numbering reports broken style bindings") {
    ...
    CHECK_EQ(run_cli({"audit-style-numbering", source.string(), "--json",
                      "--fail-on-issue"},
                     json_output),
             1);
    const auto audit_json = read_text_file(json_output);
    CHECK_NE(audit_json.find("\"clean\":false"), std::string::npos);
    CHECK_NE(audit_json.find("\"paragraph_style_count\":4"), std::string::npos);
    CHECK_NE(audit_json.find("\"numbered_style_count\":2"), std::string::npos);
    CHECK_NE(audit_json.find("\"issue_count\":2"), std::string::npos);
    CHECK_NE(audit_json.find("\"suggestion_count\":2"), std::string::npos);
    CHECK_NE(audit_json.find("\"kind\":\"missing_num_id\""), std::string::npos);
    ...
    CHECK_NE(audit_json.find("\"action\":\"clear_style_numbering\""),
             std::string::npos);
    CHECK_NE(audit_json.find("\"command_template\":\"featherdoc_cli "
                             "clear-paragraph-style-numbering <input.docx> "
                             "MissingNumId --output <output.docx> --json\""),
             std::string::npos);
    CHECK_NE(audit_json.find("\"command_template\":\"featherdoc_cli "
                             "clear-paragraph-style-numbering <input.docx> "
                             "OrphanNumId --output <output.docx> --json\""),
             std::string::npos);
    CHECK_EQ(audit_json.find("\"style_id\":\"PlainBody\""), std::string::npos);

    CHECK_EQ(run_cli({"audit-style-numbering", source.string()}, text_output),
             0);
    ...
    CHECK_NE(audit_text.find(
                 "command=featherdoc_cli clear-paragraph-style-numbering "
                 "<input.docx> MissingNumId --output <output.docx> --json"),
             std::string::npos);
    ...
    CHECK_EQ(
        ...
        std::string{"{\"command\":\"audit-style-numbering\",\"ok\":false,"
...
TEST_CASE("cli repair-style-numbering plans and applies safe clear repairs") {
    ...
    const fs::path output_without_apply =
        working_directory /
        "cli_repair_style_numbering_output_without_apply.json";
    const fs::path apply_without_output =
        working_directory /
        "cli_repair_style_numbering_apply_without_output.json";
    ...
    CHECK_NE(plan_json.find("\"command_template\":\"featherdoc_cli "
                            "clear-paragraph-style-numbering <input.docx> "
                            "MissingNumId --output <output.docx> --json\""),
             std::string::npos);

    CHECK_EQ(run_cli({"repair-style-numbering", source.string(), "--apply",
                      "--output", repaired.string(), "--json"},
                     apply_output),
             0);
    ...
    CHECK_NE(apply_json.find("\"output_path\":" + json_quote_path(repaired)),
             std::string::npos);
    ...
    CHECK_NE(source_styles_xml.find("<w:numId w:val=\"777\"/>"),
             std::string::npos);
    const auto repaired_styles_xml =
        read_docx_entry(repaired, "word/styles.xml");
    CHECK_EQ(repaired_styles_xml.find("<w:numPr>"), std::string::npos);

    CHECK_EQ(run_cli({"repair-style-numbering", source.string(), "--output",
                      repaired.string(), "--json"},
                     output_without_apply),
             2);
    CHECK_EQ(read_text_file(output_without_apply),
             std::string{"{\"command\":\"repair-style-numbering\",\"ok\":false,"
                         "\"stage\":\"parse\",\"message\":\"--output requires "
                         "--apply\"}\n"});
...
TEST_CASE("cli repair-style-numbering applies based-on alignment repairs") {
    ...
    const fs::path repaired_audit_output =
        working_directory /
        "cli_repair_style_numbering_based_on_repaired_audit.json";
...
TEST_CASE("cli repair-style-numbering relinks missing levels to unique "
    ...
    const fs::path repaired_audit_output =
        working_directory /
        "cli_repair_style_numbering_relink_repaired_audit.json";
    ...
    CHECK_NE(audit_json.find("\"action\":\"relink_style_numbering\""),
             std::string::npos);
    CHECK_NE(audit_json.find("\"target_definition_id\":2"), std::string::npos);
    ...
    CHECK_NE(apply_json.find("\"after_issue_count\":1"), std::string::npos);
