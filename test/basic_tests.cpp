...
TEST_CASE("template schema patch builder creates replayable slot-level changes") {
    ...
    featherdoc::template_schema right{{
        ...
        {{}, {"new_optional", featherdoc::template_slot_kind::text, false}},
        {{}, {"line_items", featherdoc::template_slot_kind::table_rows, true}},
    }};

    const auto patch = featherdoc::build_template_schema_patch(left, right);
    CHECK_EQ(patch.rename_slots.size(), 1U);
    REQUIRE(patch.rename_slots.size() == 1U);
    CHECK_EQ(patch.rename_slots.front().slot.bookmark_name, "old_optional");
    CHECK_EQ(patch.rename_slots.front().new_bookmark_name, "new_optional");
    CHECK_EQ(patch.remove_slots.size(), 1U);
    REQUIRE(patch.remove_slots.size() == 1U);
    CHECK_EQ(patch.remove_slots.front().bookmark_name, "stale");
