...
TEST_CASE(
    "cli build-template-schema-patch keeps cross-target changes explicit") {
    const fs::path working_directory = fs::current_path();
    const fs::path left_schema =
        working_directory /
        "cli_build_template_schema_patch_cross_target_left.json";
    const fs::path right_schema =
        working_directory /
        "cli_build_template_schema_patch_cross_target_right.json";
    const fs::path patch_output =
        working_directory /
        "cli_build_template_schema_patch_cross_target_output.json";
    const fs::path build_output =
        working_directory /
        "cli_build_template_schema_patch_cross_target_summary.json";
    const fs::path applied_schema =
        working_directory /
        "cli_build_template_schema_patch_cross_target_applied.json";
    const fs::path apply_output =
        working_directory /
        "cli_build_template_schema_patch_cross_target_apply_summary.json";
...
TEST_CASE("cli build-template-schema-patch replaces changed target identity") {
    ...
    const fs::path left_schema =
        working_directory /
        "cli_build_template_schema_patch_target_identity_left.json";
    const fs::path right_schema =
        working_directory /
        "cli_build_template_schema_patch_target_identity_right.json";
    const fs::path patch_output =
        working_directory /
        "cli_build_template_schema_patch_target_identity_output.json";
    const fs::path build_output =
        working_directory /
        "cli_build_template_schema_patch_target_identity_summary.json";
    const fs::path applied_schema =
        working_directory /
        "cli_build_template_schema_patch_target_identity_applied.json";
    const fs::path apply_output =
        working_directory /
        "cli_build_template_schema_patch_target_identity_apply_summary.json";
    ...
    write_binary_file(left_schema,
                      std::string{"{\"targets\":[{\"part\":\"section-header\","
    ...
    CHECK_EQ(read_text_file(patch_output),
             std::string{"{\"remove_targets\":[{\"part\":\"section-header\","
                         "\"section\":1,\"kind\":\"default\","
                         "\"resolved_from_section\":0,"
...
TEST_CASE("cli build-template-schema-patch emits occurrence clear update") {
    ...
    const fs::path build_output =
        working_directory /
        "cli_build_template_schema_patch_clear_summary.json";
    const fs::path applied_schema =
        working_directory /
        "cli_build_template_schema_patch_clear_applied.json";
    const fs::path apply_output =
        working_directory /
        "cli_build_template_schema_patch_clear_apply_summary.json";
...
TEST_CASE("cli build-template-schema-patch emits empty patch for equivalent "
          "schemas") {
    const fs::path working_directory = fs::current_path();
    const fs::path left_schema =
...
TEST_CASE("cli build-template-schema-patch reports parse errors") {
    ...
    CHECK_EQ(run_cli({"build-template-schema-patch", schema.string(), "--json"},
                     output),
             2);
    CHECK_EQ(
