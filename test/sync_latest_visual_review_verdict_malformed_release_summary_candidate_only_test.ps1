...
$malformedReleaseRoot = Join-Path $resolvedWorkingDir "release-candidate-malformed"
$malformedReleaseReportDir = Join-Path $malformedReleaseRoot "report"
...
New-Item -ItemType Directory -Path $malformedReleaseReportDir -Force | Out-Null
...
New-TaskReviewSeed -Root $oldDocumentTaskDir -TaskId "document-task-old" -Status "pending_review" -Verdict "undecided" `
    -SourceKind "document" -SourcePath $docxPath -GeneratedAt "2026-04-21T10:00:00"
New-TaskReviewSeed -Root $newDocumentTaskDir -TaskId "document-task-new" -Status "reviewed" -Verdict "pass" `
    -SourceKind "document" -SourcePath $docxPath -GeneratedAt "2026-04-21T10:05:00" `
    -ReviewedAt "2026-04-21T10:05:00" -ReviewMethod "operator_supplied"

$gateSummaryPath = Join-Path $gateReportDir "gate_summary.json"
$malformedReleaseSummaryPath = Join-Path $malformedReleaseReportDir "summary.json"
...
Set-Content -LiteralPath $malformedReleaseSummaryPath -Encoding UTF8 -Value '{"generated_at":"2026-04-21T10:00:00",'
$malformedReleaseSummaryBefore = Get-Content -Raw -LiteralPath $malformedReleaseSummaryPath

(@{
    generated_at = "2026-04-21T10:05:00"
...
Assert-Contains -Path $gateFinalReviewPath -ExpectedText "Document reviewed at: 2026-04-21T10:05:00" -Label "gate_final_review.md"

Assert-True -Condition ((Get-Content -Raw -LiteralPath $malformedReleaseSummaryPath) -eq $malformedReleaseSummaryBefore) `
    -Message "Malformed release summary candidate should be ignored without being rewritten."
Assert-PathNotExists -Path (Join-Path $malformedReleaseReportDir "final_review.md") -Label "malformed release final review"
Assert-PathNotExists -Path (Join-Path $malformedReleaseReportDir "release_handoff.md") -Label "malformed release handoff"
Assert-PathNotExists -Path (Join-Path $malformedReleaseReportDir "release_body.zh-CN.md") -Label "malformed release body"
Assert-PathNotExists -Path (Join-Path $malformedReleaseReportDir "ARTIFACT_GUIDE.md") -Label "malformed artifact guide"
Assert-PathNotExists -Path (Join-Path $malformedReleaseReportDir "REVIEWER_CHECKLIST.md") -Label "malformed reviewer checklist"
Assert-PathNotExists -Path (Join-Path $malformedReleaseRoot "START_HERE.md") -Label "malformed start-here"

Write-Host "Sync latest visual review verdict malformed-only release summary candidate regression passed."
