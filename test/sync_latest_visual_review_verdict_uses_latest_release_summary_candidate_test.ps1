...
$releaseRoot = Join-Path $resolvedWorkingDir "release-candidate-checks-newer"
$releaseReportDir = Join-Path $releaseRoot "report"
$olderReleaseRoot = Join-Path $resolvedWorkingDir "release-candidate-checks-older"
...
(Get-Item -LiteralPath $olderReleaseSummaryPath).LastWriteTimeUtc = [datetime]"2026-04-14T08:55:00Z"
...
Assert-True -Condition ((Get-Content -Raw -LiteralPath $olderReleaseSummaryPath) -eq $olderReleaseSummaryBefore) `
    -Message "Older matching release summary should not be refreshed when a newer match exists."
...
Write-Host "Sync latest visual review verdict latest release-summary candidate regression passed."
