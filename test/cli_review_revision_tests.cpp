...
TEST_CASE("cli revision authoring appends insertion and deletion revisions") {
    ...
    const fs::path source =
        working_directory / "cli_revision_authoring_source.docx";
    const fs::path inserted =
        working_directory / "cli_revision_authoring_inserted.docx";
    const fs::path deleted =
        working_directory / "cli_revision_authoring_deleted.docx";
    ...
    const fs::path inspect_output =
        working_directory / "cli_revision_authoring_inspect.json";
    ...
    CHECK_EQ(run_cli({"append-insertion-revision", source.string(), "--text",
                      "CLI inserted revision", "--author", "CLI Author",
                      "--date", "2026-05-02T12:00:00Z", "--output",
                      inserted.string(), "--json"},
                     output),
             0);
    CHECK_NE(read_text_file(output).find(R"("affected":1)"), std::string::npos);

    CHECK_EQ(run_cli({"append-deletion-revision", inserted.string(), "--text",
                      "CLI deleted revision", "--author", "CLI Reviewer",
                      "--date", "2026-05-02T13:00:00Z", "--output",
                      deleted.string(), "--json"},
                     output),
             0);
    CHECK_NE(read_text_file(output).find(R"("affected":1)"), std::string::npos);
    ...
    CHECK_EQ(
        run_cli({"inspect-review", deleted.string(), "--json"}, inspect_output),
        0);
    ...
    CHECK_NE(inspect_json.find(R"("author":"CLI Reviewer")"),
             std::string::npos);
    ...
    CHECK_EQ(
        run_cli({"set-revision-metadata", deleted.string(), "2", "--author",
                 "CLI Updated Author", "--date", "2026-05-02T12:30:00Z",
                 "--output", metadata_updated.string(), "--json"},
                output),
        0);
    ...
    CHECK_EQ(run_cli({"set-revision-metadata", metadata_updated.string(), "3",
                      "--clear-author", "--date", "2026-05-02T13:30:00Z",
                      "--output", metadata_cleared.string(), "--json"},
                     output),
             0);
...
TEST_CASE("cli run revision authoring creates in-place revisions") {
    const fs::path working_directory = fs::current_path();
    const fs::path source =
        working_directory / "cli_run_revision_authoring_source.docx";
    const fs::path inserted =
        working_directory / "cli_run_revision_authoring_inserted.docx";
    const fs::path deleted =
        working_directory / "cli_run_revision_authoring_deleted.docx";
    const fs::path replaced =
        working_directory / "cli_run_revision_authoring_replaced.docx";
    const fs::path output =
        working_directory / "cli_run_revision_authoring.json";
    const fs::path inspect_output =
        working_directory / "cli_run_revision_authoring_inspect.json";
    ...
    CHECK_EQ(run_cli({"insert-run-revision-after", source.string(), "0", "0",
                      "--text", "CLI in-place insertion", "--author",
                      "CLI Run Author", "--date", "2026-05-02T17:00:00Z",
                      "--output", inserted.string(), "--json"},
                     output),
             0);
    ...
    CHECK_EQ(
        run_cli({"delete-run-revision", inserted.string(), "0", "1", "--author",
                 "CLI Run Reviewer", "--date", "2026-05-02T18:00:00Z",
                 "--output", deleted.string(), "--json"},
                output),
        0);

    CHECK_EQ(run_cli({"replace-run-revision", deleted.string(), "0", "1",
                      "--text", "CLI replacement", "--author", "CLI Run Editor",
                      "--date", "2026-05-02T19:00:00Z", "--output",
                      replaced.string(), "--json"},
                     output),
             0);
...
TEST_CASE("cli paragraph text revision authoring creates range revisions") {
    ...
    const fs::path inserted =
        working_directory /
        "cli_paragraph_text_revision_authoring_inserted.docx";
    const fs::path deleted =
        working_directory /
        "cli_paragraph_text_revision_authoring_deleted.docx";
    const fs::path replaced =
        working_directory /
        "cli_paragraph_text_revision_authoring_replaced.docx";
    const fs::path output =
        working_directory / "cli_paragraph_text_revision_authoring.json";
    const fs::path inspect_output =
        working_directory /
        "cli_paragraph_text_revision_authoring_inspect.json";
    ...
    CHECK_EQ(run_cli({"insert-paragraph-text-revision", source.string(), "0",
                      "5", "--text", "CLI range insertion ", "--author",
                      "CLI Range Author", "--date", "2026-05-02T20:00:00Z",
                      "--output", inserted.string(), "--json"},
                     output),
             0);
    ...
    CHECK_EQ(run_cli({"insert-paragraph-text-revision", source.string(), "0",
                      "5", "--text", "CLI range insertion ", "--expected-text",
                      "Middle", "--json"},
                     output),
             2);
    ...
    CHECK_EQ(run_cli({"delete-paragraph-text-revision", inserted.string(), "0",
                      "5", "6", "--expected-text", "Middle", "--expected-text",
                      "Middle", "--json"},
                     output),
             2);
    output_json = read_text_file(output);
    ...
    CHECK_EQ(run_cli({"delete-paragraph-text-revision", inserted.string(), "0",
                      "5", "6", "--expected-text", "Wrong paragraph text",
                      "--output", deleted.string(), "--json"},
                     output),
             1);
    output_json = read_text_file(output);
    ...
    CHECK_NE(output_json.find(R"("expected_text":"Wrong paragraph text")"),
             std::string::npos);
    CHECK_NE(output_json.find(R"("actual_text":"Middle")"), std::string::npos);
    ...
    CHECK_NE(
        output_json.find(
            R"("paragraph_index":0,"text_offset":5,"text_length":6,"text":"Middle")"),
        std::string::npos);

    CHECK_EQ(run_cli({"delete-paragraph-text-revision", inserted.string(), "0",
                      "5", "6", "--expected-text", "Middle", "--author",
                      "CLI Range Reviewer", "--date", "2026-05-02T21:00:00Z",
                      "--output", deleted.string(), "--json"},
                     output),
             0);
    output_json = read_text_file(output);
    ...
    CHECK_EQ(run_cli({"replace-paragraph-text-revision", deleted.string(), "0",
                      "5", "6", "--text", "CLI range replacement", "--author",
                      "CLI Range Editor", "--date", "2026-05-02T22:00:00Z",
                      "--output", replaced.string(), "--json"},
                     output),
             0);
...
TEST_CASE(
    "cli text range revision authoring creates cross-paragraph revisions") {
    const fs::path working_directory = fs::current_path();
    const fs::path source =
    ...
    CHECK_EQ(run_cli({"preview-text-range", source.string(), "0", "6", "2", "5",
                      "--json"},
                     output),
             0);
    auto output_json = read_text_file(output);
    ...
    CHECK_NE(output_json.find(R"("start_paragraph_index":0)"),
             std::string::npos);
    CHECK_NE(output_json.find(R"("end_paragraph_index":2)"), std::string::npos);
    ...
    CHECK_NE(
        output_json.find(
            R"("paragraph_index":1,"text_offset":0,"text_length":11,"text":"Middle Text")"),
        std::string::npos);

    CHECK_EQ(run_cli({"preview-text-range", source.string(), "2", "0", "1", "1",
                      "--json"},
                     output),
             1);
    output_json = read_text_file(output);
    ...
    CHECK_EQ(run_cli({"insert-text-range-revision", source.string(), "1", "7",
                      "--text", "CLI cross insertion ", "--expected-text",
                      "Text", "--json"},
                     output),
             2);
    output_json = read_text_file(output);
    ...
    CHECK_EQ(run_cli({"insert-text-range-revision", source.string(), "1", "7",
                      "--text", "CLI cross insertion ", "--author",
                      "CLI Cross Author", "--date", "2026-05-03T02:00:00Z",
                      "--output", inserted.string(), "--json"},
                     output),
             0);
    output_json = read_text_file(output);
    CHECK_NE(output_json.find(R"("start_paragraph_index":1)"),
             std::string::npos);
    CHECK_NE(output_json.find(R"("start_text_offset":7)"), std::string::npos);

    CHECK_EQ(run_cli({"delete-text-range-revision", source.string(), "0", "6",
                      "2", "5", "--expected-text", "Wrong selected text",
                      "--output", deleted.string(), "--json"},
                     output),
             1);
    output_json = read_text_file(output);
    ...
    CHECK_EQ(
        run_cli({"delete-text-range-revision", source.string(), "0", "6", "2",
                 "5", "--expected-text", "BetaMiddle TextGamma", "--author",
                 "CLI Cross Reviewer", "--date", "2026-05-03T03:00:00Z",
                 "--output", deleted.string(), "--json"},
                output),
        0);
    output_json = read_text_file(output);
    CHECK_NE(output_json.find(R"("end_paragraph_index":2)"), std::string::npos);
    CHECK_NE(output_json.find(R"("end_text_offset":5)"), std::string::npos);

    CHECK_EQ(run_cli({"replace-text-range-revision", source.string(), "0", "6",
                      "2", "5", "--text", "CLI cross replacement ",
                      "--expected-text", "BetaMiddle TextGamma", "--author",
                      "CLI Cross Editor", "--date", "2026-05-03T04:00:00Z",
                      "--output", replaced.string(), "--json"},
                     output),
             0);
...
TEST_CASE("cli find text ranges reports paragraph offsets") {
    ...
    CHECK_EQ(run_cli({"find-text-ranges", source.string(), "--text", "Beta",
                      "--json"},
                     output),
             0);
    auto output_json = read_text_file(output);
    ...
    CHECK_NE(output_json.find(R"("start_paragraph_index":0)"),
             std::string::npos);
    CHECK_NE(output_json.find(R"("start_text_offset":6)"), std::string::npos);
    CHECK_NE(output_json.find(R"("start_paragraph_index":2)"),
             std::string::npos);
    CHECK_NE(output_json.find(R"("start_text_offset":0)"), std::string::npos);

    CHECK_EQ(run_cli({"find-text-ranges", source.string(), "--text",
                      "BetaMiddle TextBeta", "--json"},
                     output),
             0);
    output_json = read_text_file(output);
    ...
    CHECK_EQ(
        run_cli({"find-text-ranges", source.string(), "--text", "", "--json"},
                output),
        1);
    output_json = read_text_file(output);
...
TEST_CASE("cli run-recipe executes batch replace") {
    ...
    write_binary_file(
        inputs, R"({"inputs":{"source_dir":")" + json_escape_path(source_dir) +
                    R"(","find_text":"Beta","replace_text":"Omega"}})");

    CHECK_EQ(
        run_cli({"run-recipe", "--recipe", recipe.string(), "--inputs",
                 inputs.string(), "--output", output_dir.string(), "--json"},
                output),
        0);
    ...
    CHECK_NE(output_json.find(R"("replacements_count":2)"), std::string::npos);
    CHECK(fs::exists(replaced));

    CHECK_EQ(run_cli({"find-text-ranges", replaced.string(), "--text", "Omega",
                      "--json"},
                     find_output),
             0);
...
TEST_CASE("cli revision mutation accepts and rejects revisions") {
    const fs::path working_directory = fs::current_path();
    const fs::path source =
        working_directory / "cli_revision_mutation_source.docx";
    const fs::path accepted =
        working_directory / "cli_revision_mutation_accepted.docx";
    const fs::path clean =
        working_directory / "cli_revision_mutation_clean.docx";
    const fs::path output = working_directory / "cli_revision_mutation.json";
    const fs::path inspect_output =
        working_directory / "cli_revision_mutation_inspect.json";
    ...
    CHECK_EQ(run_cli({"accept-revision", source.string(), "0", "--output",
    ...
    CHECK_EQ(run_cli({"reject-all-revisions", accepted.string(), "--output",
                      clean.string(), "--json"},
                     output),
             0);
    CHECK_NE(read_text_file(output).find(R"("affected":1)"), std::string::npos);
    ...
    CHECK_EQ(
        run_cli({"inspect-review", clean.string(), "--json"}, inspect_output),
        0);
