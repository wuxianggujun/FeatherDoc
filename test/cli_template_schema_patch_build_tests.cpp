...
TEST_CASE("cli build-template-schema-patch generates reusable patch output") {
    ...
    const fs::path apply_output =
        working_directory /
        "cli_build_template_schema_patch_apply_summary.json";
    ...
    CHECK_EQ(run_cli({"build-template-schema-patch", left_schema.string(),
                      right_schema.string(), "--output", patch_output.string(),
                      "--json"},
                     build_output),
             0);
    CHECK_EQ(
    ...
    CHECK_EQ(run_cli({"patch-template-schema", left_schema.string(),
                      "--patch-file", patch_output.string(), "--output",
                      applied_schema.string(), "--json"},
                     apply_output),
             0);
    CHECK_EQ(read_text_file(applied_schema),
             read_text_file(right_schema) + "\n");
...
TEST_CASE("cli build-template-schema-patch emits slot-level rename update and "
          "removal") {
    const fs::path working_directory = fs::current_path();
    const fs::path left_schema =
        working_directory /
        "cli_build_template_schema_patch_slot_ops_left.json";
    const fs::path right_schema =
        working_directory /
        "cli_build_template_schema_patch_slot_ops_right.json";
    const fs::path patch_output =
        working_directory /
        "cli_build_template_schema_patch_slot_ops_output.json";
    const fs::path build_output =
        working_directory /
        "cli_build_template_schema_patch_slot_ops_summary.json";
    const fs::path applied_schema =
        working_directory /
        "cli_build_template_schema_patch_slot_ops_applied.json";
    const fs::path apply_output =
        working_directory /
        "cli_build_template_schema_patch_slot_ops_apply_summary.json";
    ...
    CHECK_EQ(read_text_file(patch_output),
             std::string{"{\"remove_slots\":[{\"part\":\"body\",\"bookmark\":"
                         "\"obsolete_note\"}],"
...
TEST_CASE("cli build-template-schema-patch emits source-aware rename update") {
    ...
    const fs::path patch_output =
        working_directory /
        "cli_build_template_schema_patch_source_output.json";
    const fs::path build_output =
        working_directory /
        "cli_build_template_schema_patch_source_summary.json";
    const fs::path applied_schema =
        working_directory /
        "cli_build_template_schema_patch_source_applied.json";
    const fs::path apply_output =
        working_directory /
        "cli_build_template_schema_patch_source_apply_summary.json";
...
TEST_CASE(
    "cli build-template-schema-patch emits rename occurrence clear update") {
    const fs::path working_directory = fs::current_path();
    const fs::path left_schema =
        working_directory /
        "cli_build_template_schema_patch_rename_clear_left.json";
    const fs::path right_schema =
        working_directory /
        "cli_build_template_schema_patch_rename_clear_right.json";
    const fs::path patch_output =
        working_directory /
        "cli_build_template_schema_patch_rename_clear_output.json";
    const fs::path build_output =
        working_directory /
        "cli_build_template_schema_patch_rename_clear_summary.json";
    const fs::path applied_schema =
        working_directory /
        "cli_build_template_schema_patch_rename_clear_applied.json";
    const fs::path apply_output =
        working_directory /
        "cli_build_template_schema_patch_rename_clear_apply_summary.json";
    ...
    write_binary_file(
        left_schema,
        std::string{
            R"({"targets":[{"part":"body","slots":[{"bookmark":"old_customer","kind":"text","count":2}]}]})"});
    write_binary_file(
        right_schema,
        std::string{
    ...
    CHECK_EQ(
        read_text_file(patch_output),
        std::string{
            R"({"rename_slots":[{"part":"body","bookmark":"old_customer","new_bookmark":"customer"}],"update_slots":[{"part":"body","bookmark":"customer","clear_min_occurrences":true,"clear_max_occurrences":true}]})"} +
            "\n");
...
TEST_CASE("cli build-template-schema-patch keeps ambiguous renames explicit") {
    ...
    const fs::path left_schema =
        working_directory /
        "cli_build_template_schema_patch_ambiguous_left.json";
    const fs::path right_schema =
        working_directory /
        "cli_build_template_schema_patch_ambiguous_right.json";
    const fs::path patch_output =
        working_directory /
        "cli_build_template_schema_patch_ambiguous_output.json";
    const fs::path build_output =
        working_directory /
        "cli_build_template_schema_patch_ambiguous_summary.json";
    const fs::path applied_schema =
        working_directory /
        "cli_build_template_schema_patch_ambiguous_applied.json";
    const fs::path apply_output =
        working_directory /
        "cli_build_template_schema_patch_ambiguous_apply_summary.json";
...
TEST_CASE(
    "cli build-template-schema-patch keeps cross-source changes explicit") {
    const fs::path working_directory = fs::current_path();
    const fs::path left_schema =
        working_directory /
        "cli_build_template_schema_patch_cross_source_left.json";
    const fs::path right_schema =
        working_directory /
        "cli_build_template_schema_patch_cross_source_right.json";
    const fs::path patch_output =
        working_directory /
        "cli_build_template_schema_patch_cross_source_output.json";
    const fs::path build_output =
        working_directory /
        "cli_build_template_schema_patch_cross_source_summary.json";
    const fs::path applied_schema =
        working_directory /
        "cli_build_template_schema_patch_cross_source_applied.json";
    const fs::path apply_output =
        working_directory /
        "cli_build_template_schema_patch_cross_source_apply_summary.json";
