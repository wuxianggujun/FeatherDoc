...
TEST_CASE("cli inspect-numbering lists custom numbering definitions") {
    ...
    const auto num_id =
        std::string{numbering_instance.attribute("w:numId").value()};
    ...
    CHECK_NE(
        inspect_text.find("definition[0]: id=" + std::to_string(*numbering_id) +
                          " name=LegalOutline levels=2 instances=" + num_id),
        std::string::npos);
    CHECK_NE(
        inspect_text.find("level[0]: kind=decimal start=3 text_pattern=%1."),
        std::string::npos);
    CHECK_NE(
        inspect_text.find("level[1]: kind=decimal start=1 text_pattern=%1.%2."),
        std::string::npos);
...
TEST_CASE(
    "cli inspect-numbering supports single-definition json output and errors") {
    const fs::path working_directory = fs::current_path();
    const fs::path source =
        working_directory / "cli_numbering_single_source.docx";
    ...
    definition.levels = {
        featherdoc::numbering_level_definition{featherdoc::list_kind::decimal,
                                               5U, 0U, "(%1)"},
        featherdoc::numbering_level_definition{featherdoc::list_kind::bullet,
                                               1U, 1U, "o"},
    };
    ...
    CHECK_EQ(run_cli({"inspect-numbering", source.string(), "--definition",
                     ...
                     definition_output),
             0);
    CHECK_EQ(read_text_file(definition_output),
             std::string{"{\"definition\":{\"definition_id\":"} +
                 std::to_string(*numbering_id) +
                 ",\"name\":\"PolicyOutline\",\"levels\":[{\"level\":0,"
                 "\"kind\":\"decimal\","
                 "\"start\":5,\"text_pattern\":\"(%1)\"},{\"level\":1,\"kind\":"
                 "\"bullet\","
    ...
    INFO(missing_json);
    CHECK_NE(missing_json.find("\"command\":\"inspect-numbering\""),
             std::string::npos);
    ...
    CHECK_NE(missing_json.find(
                 ...
                 "'word/numbering.xml'\""),
             std::string::npos);
    CHECK_NE(missing_json.find("\"entry\":\"word/numbering.xml\""),
             std::string::npos);
...
TEST_CASE(
    "cli inspect-numbering reports instance overrides for restarted lists") {
    const fs::path working_directory = fs::current_path();
    const fs::path source =
        working_directory / "cli_numbering_restart_source.docx";
    const fs::path list_output =
        working_directory / "cli_numbering_restart_output.txt";
    const fs::path json_output =
        working_directory / "cli_numbering_restart_output.json";
    ...
    REQUIRE(document.set_paragraph_list(first_item,
    ...
    REQUIRE(document.restart_paragraph_list(restarted_item,
                                            featherdoc::list_kind::decimal));
    ...
    const auto managed_definition = std::find_if(
    ...
    const auto first_num_id =
        std::to_string(managed_definition->instance_ids[0]);
    const auto restarted_num_id =
        std::to_string(managed_definition->instance_ids[1]);
    ...
    CHECK_NE(inspect_text.find("id=" + std::to_string(definition_id) +
             ...
             std::string::npos);
    CHECK_NE(
        inspect_text.find("instances=" + first_num_id + "," + restarted_num_id),
        std::string::npos);
    CHECK_NE(
        inspect_text.find("instance[" + first_num_id + "]: override_count=0"),
        std::string::npos);
    CHECK_NE(inspect_text.find("instance[" + restarted_num_id +
                               "]: override_count=1"),
             std::string::npos);
    ...
    CHECK_NE(
        inspect_json.find("\"definition_id\":" + std::to_string(definition_id)),
        std::string::npos);
    CHECK_NE(inspect_json.find("\"instance_ids\":[" + first_num_id + "," +
                               restarted_num_id + "]"),
             std::string::npos);
    CHECK_NE(
        inspect_json.find("\"instances\":[{\"instance_id\":" + first_num_id +
...
TEST_CASE("cli inspect-numbering filters a single numbering instance") {
    const fs::path working_directory = fs::current_path();
    const fs::path source =
        working_directory / "cli_numbering_instance_source.docx";
    const fs::path text_output =
        working_directory / "cli_numbering_instance_output.txt";
    const fs::path json_output =
        working_directory / "cli_numbering_instance_output.json";
    const fs::path missing_output =
        working_directory / "cli_numbering_instance_missing.json";
    ...
    const auto definition_id =
        std::to_string(managed_definition->definition_id);
    const auto restarted_num_id =
        std::to_string(managed_definition->instance_ids[1]);

    REQUIRE_FALSE(document.save());

    CHECK_EQ(run_cli({"inspect-numbering", source.string(), "--instance",
                      restarted_num_id},
                     text_output),
             0);
    const auto inspect_text = read_text_file(text_output);
    CHECK_NE(inspect_text.find("definition_id: " + definition_id),
             std::string::npos);
    CHECK_NE(inspect_text.find("name: FeatherDocDecimalList"),
             std::string::npos);
    CHECK_NE(inspect_text.find("instance[" + restarted_num_id +
                               "]: override_count=1"),
             std::string::npos);
    ...
    CHECK_EQ(run_cli({"inspect-numbering", source.string(), "--instance",
                      restarted_num_id, "--json"},
                     json_output),
             0);
    CHECK_EQ(read_text_file(json_output),
             std::string{"{\"instance\":{\"definition_id\":"} + definition_id +
                 ",\"definition_name\":\"FeatherDocDecimalList\",\"instance\":{"
                 "\"instance_id\":" +
                 restarted_num_id +
                 ",\"level_overrides\":[{\"level\":0,\"start_override\":1,"
                 "\"level_definition\":null}]}}}\n");

    CHECK_EQ(run_cli({"inspect-numbering", source.string(), "--instance", "999",
                      "--json"},
                     missing_output),
             1);
    const auto missing_json = read_text_file(missing_output);
...
TEST_CASE("cli export and import numbering catalog preserves instances and "
          "overrides") {
    const fs::path working_directory = fs::current_path();
    const fs::path source =
        working_directory / "cli_numbering_catalog_source.docx";
    const fs::path target =
        working_directory / "cli_numbering_catalog_target.docx";
    const fs::path imported =
        working_directory / "cli_numbering_catalog_imported.docx";
    ...
    const auto import_summary =
        source_document.import_numbering_catalog(catalog);
    ...
    CHECK_EQ(run_cli({"export-numbering-catalog", source.string(), "--output",
             ...
             0);
    CHECK_EQ(
        read_text_file(export_output),
        std::string{"{\"command\":\"export-numbering-catalog\",\"ok\":true,"} +
            "\"output_path\":" + json_quote_path(catalog_json) +
    ...
    CHECK_EQ(run_cli({"import-numbering-catalog", target.string(),
                      "--catalog-file", catalog_json.string(), "--output",
    ...
    CHECK_NE(
        import_json.find("\"catalog_file\":" + json_quote_path(catalog_json)),
        std::string::npos);
    CHECK_NE(import_json.find("\"input_definition_count\":1"),
             std::string::npos);
    ...
    const auto imported_definitions =
        imported_document.list_numbering_definitions();
    ...
    const auto restarted_imported_instance = std::find_if(
        imported_definition->instances.begin(),
        imported_definition->instances.end(), [](const auto &instance) {
            return instance.level_overrides.size() == 2U;
        });
    REQUIRE(restarted_imported_instance !=
            imported_definition->instances.end());
    CHECK_EQ(restarted_imported_instance->level_overrides[0].level, 0U);
    REQUIRE(restarted_imported_instance->level_overrides[0]
                .start_override.has_value());
    CHECK_EQ(*restarted_imported_instance->level_overrides[0].start_override,
             7U);
...
TEST_CASE("cli numbering catalog summaries emit Unicode paths as UTF-8") {
    const auto unicode_directory =
        fs::current_path() /
        featherdoc::detail::path_from_utf8("目录-中文-日本語-🙂 空格");
    const auto source = unicode_directory /
                        featherdoc::detail::path_from_utf8("输入 文档.docx");
    const auto json_catalog =
        unicode_directory /
        featherdoc::detail::path_from_utf8("目录-编号-日本語-🙂.json");
    const auto text_catalog =
        unicode_directory /
        featherdoc::detail::path_from_utf8("文本-编号-中文.json");
    const auto json_output =
        unicode_directory /
        featherdoc::detail::path_from_utf8("结果-JSON-中文.txt");
    const auto text_output =
        unicode_directory /
        featherdoc::detail::path_from_utf8("结果-TEXT-日本語.txt");
    ...
    CHECK_EQ(run_cli({"export-numbering-catalog", cli_path_text(source),
                      "--output", cli_path_text(json_catalog), "--json"},
                     json_output),
             0);
    const auto json = read_text_file(json_output);
    CHECK(featherdoc::detail::is_valid_utf8(json));
    CHECK_NE(json.find("\"output_path\":" + json_quote_path(json_catalog)),
             std::string::npos);

    CHECK_EQ(run_cli({"export-numbering-catalog", cli_path_text(source),
                      "--output", cli_path_text(text_catalog)},
                     text_output),
             0);
    const auto text = read_text_file(text_output);
    CHECK(featherdoc::detail::is_valid_utf8(text));
    CHECK_NE(text.find("output_path: " + cli_path_text(text_catalog)),
             std::string::npos);
