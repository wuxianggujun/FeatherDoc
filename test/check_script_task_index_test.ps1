...
. (Join-Path $PSScriptRoot "check_script_task_index_test_helpers.ps1")
...
foreach ($releaseSupportScript in @(
        "scripts\release_blocker_metadata_helpers.ps1",
        "scripts\release_visual_metadata_helpers.ps1"
    )) {
    Assert-ArrayContains `
        -Values @($passingSummary.checked_scripts | ForEach-Object { $_.relative_path }) `
        -ExpectedValue $releaseSupportScript `
        -Message "Summary should list the release support script $releaseSupportScript."
}
Assert-ArrayContains `
    -Values @($passingSummary.checked_scripts | ForEach-Object { $_.relative_path }) `
...
foreach ($sectionVisualScript in @(
        ...
        "scripts\run_section_order_visual_regression.ps1",
        "scripts\run_section_part_refs_visual_regression.ps1",
        "scripts\run_section_text_multiline_visual_regression.ps1"
    )) {
    Assert-ArrayContains `
        -Values @($passingSummary.checked_scripts | ForEach-Object { $_.relative_path }) `
...
foreach ($paragraphVisualScript in @(
...
foreach ($pdfCheckScript in @(
        "scripts\check_pdf_controlled_visual_smoke.py",
        "scripts\check_pdf_text_layer.py"
    )) {
    Assert-ArrayContains `
        -Values @($passingSummary.checked_scripts | ForEach-Object { $_.relative_path }) `
        -ExpectedValue $pdfCheckScript `
        -Message "Summary should list the PDF check script $pdfCheckScript."
}
foreach ($contentControlVisualScript in @(
        "scripts\run_content_control_rich_replacement_visual_regression.ps1",
        "scripts\run_content_control_image_replacement_visual_regression.ps1"
    )) {
    Assert-ArrayContains `
        -Values @($passingSummary.checked_scripts | ForEach-Object { $_.relative_path }) `
        -ExpectedValue $contentControlVisualScript `
        -Message "Summary should list the content-control visual regression script $contentControlVisualScript."
}
foreach ($pageNumberScript in @(
        "scripts\run_page_number_fields_regression.ps1",
        "scripts\run_page_number_fields_visual_regression.ps1"
    )) {
    Assert-ArrayContains `
        -Values @($passingSummary.checked_scripts | ForEach-Object { $_.relative_path }) `
        -ExpectedValue $pageNumberScript `
        -Message "Summary should list the page-number fields script $pageNumberScript."
}
foreach ($reviewVisualScript in @(
        "scripts\run_review_inspection_visual_regression.ps1",
        "scripts\run_review_mutation_visual_regression.ps1"
    )) {
    Assert-ArrayContains `
        -Values @($passingSummary.checked_scripts | ForEach-Object { $_.relative_path }) `
