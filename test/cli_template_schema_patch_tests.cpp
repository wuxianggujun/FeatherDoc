...
TEST_CASE(
    "cli patch-template-schema applies upserts removals and slot pruning") {
    ...
    CHECK_EQ(run_cli({"patch-template-schema", schema_input.string(),
                      "--patch-file", patch_input.string(), "--output",
                      schema_output.string(), "--json"},
                     output),
             0);
    CHECK_EQ(read_text_file(output),
             std::string{"{\"command\":\"patch-template-schema\",\"ok\":true,"
                         "\"output_path\":\"" +
...
TEST_CASE("cli patch-template-schema applies content control slot selectors") {
    ...
    const fs::path schema_input =
        working_directory /
        "cli_patch_template_schema_content_controls_input.json";
    const fs::path patch_input =
        working_directory /
        "cli_patch_template_schema_content_controls_patch.json";
    const fs::path schema_output =
        working_directory /
        "cli_patch_template_schema_content_controls_output.json";
    const fs::path output =
        working_directory /
        "cli_patch_template_schema_content_controls_summary.json";
...
TEST_CASE("cli patch-template-schema updates slot properties") {
    ...
    const fs::path schema_output =
        working_directory /
        "cli_patch_template_schema_update_slots_output.json";
    const fs::path output =
        working_directory /
        "cli_patch_template_schema_update_slots_summary.json";
...
TEST_CASE("cli patch-template-schema reports parse errors") {
    ...
    write_binary_file(
        schema_input,
        std::string{"{\"targets\":[{\"part\":\"body\",\"slots\":["
                    "{\"bookmark\":\"customer\",\"kind\":\"text\"}]}]}\n"});

    CHECK_EQ(run_cli({"patch-template-schema", schema_input.string(), "--json"},
                     output),
             2);
    CHECK_EQ(read_text_file(output),
             std::string{"{\"command\":\"patch-template-schema\",\"ok\":false,"
                         "\"stage\":\"parse\",\"message\":\"missing required "
                         "--patch-file <path> option\"}\n"});

    write_binary_file(patch_input, std::string{"{}\n"});
    CHECK_EQ(run_cli({"patch-template-schema", schema_input.string(),
                      "--patch-file", patch_input.string(), "--json"},
                     output),
             0);
    CHECK_EQ(
        read_text_file(output),
        std::string{"{\"targets\":[{\"part\":\"body\",\"slots\":[{\"bookmark\":"
                    "\"customer\",\"kind\":\"text\"}]}]}\n"});
...
TEST_CASE(
    "cli preview-template-schema-patch copies patch file to output patch") {
    const fs::path working_directory = fs::current_path();
    ...
    const fs::path copied_patch_output =
        working_directory /
        "cli_preview_template_schema_patch_copy_output.json";
    const fs::path preview_output =
        working_directory /
        "cli_preview_template_schema_patch_copy_summary.json";
    ...
    write_binary_file(
        patch_input,
        std::string{"{\"upsert_targets\":[{\"part\":\"body\",\"slots\":["
                    "{\"bookmark\":\"total\",\"kind\":\"text\"}]}]}\n"});

    CHECK_EQ(run_cli({"preview-template-schema-patch", left_schema.string(),
                      "--patch-file", patch_input.string(), "--output-patch",
                      copied_patch_output.string(), "--json"},
                     preview_output),
             0);
    CHECK_EQ(
        read_text_file(preview_output),
        std::string{
            "{\"command\":\"preview-template-schema-patch\",\"ok\":true,"
            "\"output_patch_path\":\""} +
            json_escape_path(copied_patch_output) +
...
TEST_CASE("cli preview-template-schema-patch writes update slot output patch") {
    ...
    const fs::path left_schema =
        working_directory /
        "cli_preview_template_schema_patch_update_left.json";
    const fs::path patch_input =
        working_directory /
        "cli_preview_template_schema_patch_update_input.json";
    const fs::path copied_patch_output =
        working_directory /
        "cli_preview_template_schema_patch_update_output.json";
    const fs::path preview_output =
        working_directory /
        "cli_preview_template_schema_patch_update_summary.json";
    ...
    write_binary_file(
        ...
        std::string{
            ...
            "{\"bookmark\":\"customer\",\"kind\":\"text\",\"count\":1}]}]}\n"});
    write_binary_file(
        patch_input, std::string{"{\"update_slots\":[{\"part\":\"body\","
                                 "\"bookmark\":\"customer\",\"kind\":\"block\","
    ...
    CHECK_EQ(
        read_text_file(copied_patch_output),
        std::string{"{\"update_slots\":[{\"part\":\"body\",\"bookmark\":"
...
TEST_CASE(
    "cli preview-template-schema-patch writes generated output patch file") {
    const fs::path working_directory = fs::current_path();
    const fs::path left_schema =
        working_directory /
        "cli_preview_template_schema_patch_generate_left.json";
    const fs::path right_schema =
        working_directory /
        "cli_preview_template_schema_patch_generate_right.json";
    const fs::path patch_input =
        working_directory /
        "cli_preview_template_schema_patch_generate_expected.json";
    const fs::path generated_patch_output =
        working_directory /
        "cli_preview_template_schema_patch_generate_output.json";
    const fs::path preview_output =
        working_directory /
        "cli_preview_template_schema_patch_generate_summary.json";
    ...
    CHECK_EQ(run_cli({"preview-template-schema-patch", left_schema.string(),
                      right_schema.string(), "--output-patch",
                      generated_patch_output.string(), "--json"},
                     preview_output),
             0);
    CHECK_EQ(read_text_file(preview_output),
             std::string{
                 "{\"command\":\"preview-template-schema-patch\",\"ok\":true,"
                 "\"output_patch_path\":\""} +
                 json_escape_path(generated_patch_output) +
    ...
    CHECK_EQ(read_text_file(generated_patch_output),
             read_text_file(patch_input));
...
TEST_CASE("cli template schema patch review json writes stable file") {
    ...
    const fs::path preview_patch_output =
        working_directory /
        "cli_template_schema_patch_review_preview_patch.json";
    ...
    const fs::path preview_output =
        working_directory /
        "cli_template_schema_patch_review_preview_summary.json";
    const fs::path build_output =
        working_directory /
        "cli_template_schema_patch_review_build_summary.json";
    ...
    write_binary_file(
        left_schema, std::string{"{\"targets\":[{\"part\":\"body\",\"slots\":["
                                 "{\"bookmark\":\"customer\",\"kind\":\"text\","
                                 "\"required\":false},"
    ...
    CHECK_EQ(run_cli({"preview-template-schema-patch", left_schema.string(),
                      right_schema.string(), "--output-patch",
                      preview_patch_output.string(), "--review-json",
                      preview_review_output.string(), "--json"},
                     preview_output),
             0);
    CHECK_EQ(read_text_file(preview_output),
             std::string{
                 "{\"command\":\"preview-template-schema-patch\",\"ok\":true,"
                 "\"output_patch_path\":\""} +
                 json_escape_path(preview_patch_output) +
                 "\",\"review_json_path\":\"" +
                 json_escape_path(preview_review_output) +
                 ...
                 "\"changed\":true}\n");
    CHECK_EQ(read_text_file(preview_review_output),
             expected_review_json + "\n");

    CHECK_EQ(
        run_cli({"build-template-schema-patch", left_schema.string(),
                 right_schema.string(), "--output", build_patch_output.string(),
                 "--review-json", build_review_output.string(), "--json"},
                build_output),
        0);
    CHECK_EQ(
        read_text_file(build_output),
        std::string{"{\"command\":\"build-template-schema-patch\",\"ok\":true,"
                    "\"output_path\":\""} +
            json_escape_path(build_patch_output) +
            "\",\"review_json_path\":\"" +
            json_escape_path(build_review_output) +
