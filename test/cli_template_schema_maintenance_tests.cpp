...
TEST_CASE("cli normalize-template-schema writes canonical schema file and "
          "summary json") {
    ...
    CHECK_EQ(run_cli({"normalize-template-schema", schema_input.string(),
                      "--output", schema_output.string(), "--json"},
                     output),
             0);
    CHECK_EQ(
        read_text_file(output),
        std::string{"{\"command\":\"normalize-template-schema\",\"ok\":true,"
                    "\"output_path\":\"" +
...
TEST_CASE("cli lint-template-schema reports clean normalized schemas") {
    ...
    CHECK_EQ(run_cli({"lint-template-schema", schema_input.string(), "--json"},
                     output),
             0);
    CHECK_EQ(
        read_text_file(output),
        std::string{"{\"command\":\"lint-template-schema\",\"ok\":true,"
...
TEST_CASE("cli lint-template-schema reports parse errors") {
    ...
    CHECK_EQ(
        read_text_file(output),
        std::string{"{\"command\":\"lint-template-schema\",\"ok\":false,"
...
TEST_CASE("cli repair-template-schema canonicalizes and deduplicates schema") {
    ...
    CHECK_EQ(run_cli({"repair-template-schema", schema_input.string(),
                      "--output", schema_output.string(), "--json"},
                     output),
             0);
    CHECK_EQ(read_text_file(output),
             std::string{"{\"command\":\"repair-template-schema\",\"ok\":true,"
                         "\"output_path\":\"" +
                         json_escape_path(schema_output) +
...
TEST_CASE("cli repair-template-schema reports parse errors") {
    ...
    CHECK_EQ(
        read_text_file(output),
        std::string{"{\"command\":\"repair-template-schema\",\"ok\":false,"
                    "\"stage\":\"parse\",\"message\":\"repair-template-schema "
                    "expects a schema path\"}\n"});

    CHECK_EQ(run_cli({"repair-template-schema", schema_input.string(), "--json",
                      "--bad-option"},
                     output),
             2);
    CHECK_EQ(read_text_file(output),
...
TEST_CASE("cli merge-template-schema merges targets and replaces later slot "
          "definitions") {
    const fs::path working_directory = fs::current_path();
    ...
    CHECK_EQ(run_cli({"merge-template-schema", left_schema.string(),
                      right_schema.string(), "--output", merged_schema.string(),
                      "--json"},
                     output),
             0);
    CHECK_EQ(
        read_text_file(output),
        std::string{"{\"command\":\"merge-template-schema\",\"ok\":true,"
                    "\"output_path\":\"" +
                    json_escape_path(merged_schema) +
                    "\",\"input_count\":2,\"target_count\":3,\"slot_count\":5,"
                    "\"updated_target_count\":1,\"replaced_slot_count\":1}\n"});
    CHECK_EQ(
        read_text_file(merged_schema),
        std::string{"{\"targets\":["
                    "{\"part\":\"body\",\"slots\":["
                    "{\"bookmark\":\"customer\",\"kind\":\"text\",\"count\":2},"
                    "{\"bookmark\":\"invoice_no\",\"kind\":\"text\"},"
...
TEST_CASE("cli merge-template-schema reports parse errors") {
    ...
    CHECK_EQ(
        run_cli({"merge-template-schema", schema.string(), "--json"}, output),
        2);
    CHECK_EQ(
        read_text_file(output),
        std::string{"{\"command\":\"merge-template-schema\",\"ok\":false,"
