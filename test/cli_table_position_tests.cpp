...
TEST_CASE("cli plans table position preset application") {
    ...
    const fs::path source =
        working_directory / "cli_table_position_plan_source.docx";
    ...
    const fs::path missing_fingerprint_apply_output =
        working_directory /
        "cli_table_position_plan_missing_fingerprint_apply.json";
    ...
    CHECK_EQ(run_cli({"plan-table-position-presets", source.string(),
                      "--preset", "paragraph-callout", "--json"},
                     json_output),
             0);
    ...
    CHECK_NE(json.find(json_quote_path(source)), std::string::npos);
    CHECK_NE(json.find(R"("preset":"paragraph-callout")"), std::string::npos);
    ...
    CHECK_NE(json.find(R"("already_matching_table_indices":[])"),
             std::string::npos);
    CHECK_NE(json.find(R"("review_table_indices":[])"), std::string::npos);
    ...
    CHECK_NE(json.find("\"resolved_output_path\":" +
                       json_quote_path(source_plan_output)),
             std::string::npos);
    CHECK_NE(
        json.find(
            "\"resolved_recommended_batch_command\":\"set-table-position " +
            json_escape_path(source) +
            " all --preset paragraph-callout --output " +
            json_escape_path(source_plan_output) + "\""),
        std::string::npos);
    ...
    CHECK_NE(
        ...
        std::string::npos);
    CHECK_NE(
        json.find("\"resolved_recommended_command\":\"set-table-position " +
                  json_escape_path(source) +
    ...
    CHECK_NE(text.find("table_position_preset_plan: changes"),
             std::string::npos);
    CHECK_NE(text.find("input_path: " + cli_path_text(source)),
             std::string::npos);
    ...
    CHECK_NE(text.find("automatic_table_indices: [0,1]"), std::string::npos);
    CHECK_NE(text.find("recommended_batch_command: set-table-position "
                       "<input.docx> all --preset paragraph-callout"),
             std::string::npos);
    CHECK_NE(
        text.find("resolved_output_path: " + cli_path_text(source_plan_output)),
        std::string::npos);
    CHECK_NE(
        text.find("resolved_recommended_batch_command: set-table-position " +
                  cli_path_text(source) +
                  " all --preset paragraph-callout --output " +
                  cli_path_text(source_plan_output)),
        std::string::npos);
    CHECK_NE(text.find("recommended_command=\"set-table-position <input.docx> "
                       "0 --preset paragraph-callout\""),
             std::string::npos);
    ...
    CHECK_EQ(
        run_cli({"plan-table-position-presets", source.string(), "--preset",
                 "paragraph-callout", "--fail-on-change", "--json"},
                fail_output),
        1);
    ...
    CHECK_EQ(run_cli({"plan-table-position-presets", source.string(),
                      "--preset", "paragraph-callout", "--output-plan",
                      saved_plan.string(), "--json"},
                     saved_plan_stdout),
             0);
    ...
    CHECK_NE(saved_plan_json.find(json_quote_path(source)), std::string::npos);
    CHECK_NE(saved_plan_json.find(R"("output_plan_path":)"), std::string::npos);
    CHECK_NE(saved_plan_json.find(json_quote_path(saved_plan)),
             std::string::npos);
    ...
    CHECK_EQ(run_cli({"apply-table-position-plan", stale_plan.string(),
                      "--output", stale_applied_from_plan.string(), "--json"},
                     stale_apply_output),
             1);
    ...
    const auto fingerprint_next_key_offset = missing_fingerprint_plan_json.find(
        R"(,"items")", fingerprint_key_offset + 1U);
    REQUIRE_NE(fingerprint_next_key_offset, std::string::npos);
    missing_fingerprint_plan_json.erase(fingerprint_key_offset,
                                        fingerprint_next_key_offset -
                                            fingerprint_key_offset);
    write_binary_file(missing_fingerprint_plan, missing_fingerprint_plan_json);
    CHECK_EQ(run_cli({"apply-table-position-plan",
                      missing_fingerprint_plan.string(), "--dry-run", "--json"},
                     missing_fingerprint_apply_output),
             1);
    const auto missing_fingerprint_apply_json =
        read_text_file(missing_fingerprint_apply_output);
    CHECK_NE(
        missing_fingerprint_apply_json.find(R"("message":"invalid argument")"),
        std::string::npos);
    CHECK_NE(missing_fingerprint_apply_json.find("table_fingerprints"),
             std::string::npos);

    CHECK_EQ(run_cli({"apply-table-position-plan", saved_plan.string(),
                      "--dry-run", "--json"},
                     dry_run_output),
             0);
    ...
    CHECK_NE(dry_run_json.find(R"("fingerprint_checked_count":2)"),
             std::string::npos);
    CHECK_NE(dry_run_json.find(R"("would_apply_count":2)"), std::string::npos);
    CHECK_NE(dry_run_json.find(R"("table_indices":[0,1])"), std::string::npos);
    ...
    CHECK_EQ(run_cli({"plan-table-position-presets", source.string(),
                      "--preset", "paragraph-callout", "--output",
                      custom_plan_output.string(), "--json"},
                     override_output),
             0);
    const auto override_json = read_text_file(override_output);
    CHECK_NE(override_json.find("\"output_path\":" +
                                json_quote_path(custom_plan_output)),
             std::string::npos);
    ...
    CHECK_NE(
        override_json.find(
            ...
            json_escape_path(source) +
            " all --preset paragraph-callout --output " +
            json_escape_path(custom_plan_output) + "\""),
        std::string::npos);
    CHECK_FALSE(fs::exists(custom_plan_output));

    CHECK_EQ(run_cli({"apply-table-position-plan", saved_plan.string(),
                      "--output", applied_from_plan.string(), "--json"},
                     apply_output),
             0);
    ...
    CHECK_EQ(run_cli({"set-table-position", source.string(), "all", "--preset",
                      "paragraph-callout", "--output", positioned.string(),
                      "--json"},
                     set_output),
             0);
    CHECK_EQ(run_cli({"plan-table-position-presets", positioned.string(),
                      "--preset", "paragraph-callout", "--json"},
                     clean_output),
             0);
    ...
    CHECK_NE(review_json.find(R"("action":"review-existing-position")"),
             std::string::npos);
    CHECK_NE(
        review_json.find("\"resolved_recommended_command\":\"inspect-tables " +
                         json_escape_path(positioned) + " --table 0 --json\""),
        std::string::npos);
    CHECK_EQ(review_json.find(" --output "), std::string::npos);

    CHECK_EQ(
        run_cli({"plan-table-position-presets", positioned.string(), "--preset",
                 "page-corner", "--replace-positioned", "--json"},
                replace_output),
        0);
    ...
    CHECK_NE(replace_json.find("\"resolved_output_path\":" +
                               json_quote_path(positioned_plan_output)),
             std::string::npos);
    CHECK_NE(
        replace_json.find(
            "\"resolved_recommended_batch_command\":\"set-table-position " +
            json_escape_path(positioned) +
            " all --preset page-corner --output " +
            json_escape_path(positioned_plan_output) + "\""),
        std::string::npos);
