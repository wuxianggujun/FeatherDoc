...
$malformedReleaseRoot = Join-Path $resolvedWorkingDir "release-candidate-checks-malformed"
...
Set-Content -LiteralPath $malformedReleaseSummaryPath -Encoding UTF8 -Value '{"generated_at":"2026-04-14T09:10:00",'
(Get-Item -LiteralPath $releaseSummaryPath).LastWriteTimeUtc = [datetime]"2026-04-14T09:00:00Z"
(Get-Item -LiteralPath $malformedReleaseSummaryPath).LastWriteTimeUtc = [datetime]"2026-04-14T09:10:00Z"
...
Assert-True -Condition ((Get-Content -Raw -LiteralPath $malformedReleaseSummaryPath) -eq $malformedReleaseSummaryBefore) `
    -Message "Malformed release summary candidate should be skipped without being rewritten."
...
Write-Host "Sync latest visual review verdict malformed release-summary candidate regression passed."
