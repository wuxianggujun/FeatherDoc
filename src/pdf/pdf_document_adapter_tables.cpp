...
[[nodiscard]] bool row_overflows_page_area(
    const std::vector<TableRowLayout> &rows, std::size_t row_index,
    double row_top, const PdfDocumentAdapterOptions &options,
    double cell_spacing_points) noexcept {
    auto row_bottom = row_top - rows[row_index].height_points;
    for (const auto &cell : rows[row_index].cells) {
        if (is_vertical_merge_continuation(cell.cell)) {
            continue;
        }
        row_bottom = std::min(
            row_bottom,
            spanned_row_bottom(rows, row_index, cell, row_top,
                               cell_spacing_points));
    }
    return row_bottom < options.margin_bottom_points - 0.01;
}

[[nodiscard]] double row_emission_height(
    const std::vector<TableRowLayout> &rows, std::size_t row_index,
    double row_top, double cell_spacing_points) noexcept {
    auto row_bottom = row_top - rows[row_index].height_points;
    for (const auto &cell : rows[row_index].cells) {
        if (is_vertical_merge_continuation(cell.cell)) {
            continue;
...
[[nodiscard]] bool repeated_headers_fit_with_row(
    ...
    const auto fresh_page_top =
        first_baseline_y(options) + options.font_size_points;
    const auto repeated_group_height =
        header_block_height +
        row_emission_height(rows, row_index, fresh_page_top,
                            cell_spacing_points);
...
void emit_table(featherdoc::Document &document, PdfPageLayout *&page,
    ...
    for (std::size_t row_index = 0U; row_index < rows.size(); ++row_index) {
        ...
        if (!positioned_row_top.has_value() && !at_fresh_page &&
            row_overflows_page_area(rows, row_index, row_top, options,
                                    cell_spacing_points)) {
