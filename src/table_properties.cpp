...
bool Table::clear_indent() {
    ...
    try {
        ...
        const auto replacement_indent =
            staged_properties->replacement.child("w:tblInd");
        if (replacement_indent == pugi::xml_node{} ||
            replacement_indent.parent() != staged_properties->replacement ||
            !staged_properties->replacement.remove_child(
                replacement_indent)) {
            rollback();
            return false;
...
bool Table::clear_cell_margin(featherdoc::cell_margin_edge edge) {
    ...
    const auto margins = table_properties.child("w:tblCellMar");
    ...
    try {
        ...
        auto replacement_margins =
            replacement_properties.child("w:tblCellMar");
        ...
        if (std::string_view{replacement_properties.name()} != "w:tblPr" ||
            ...
            std::string_view{replacement_margins.name()} != "w:tblCellMar" ||
...
bool Table::clear_style_id() {
    ...
    try {
        ...
        const auto replacement_style =
            staged_properties->replacement.child("w:tblStyle");
        if (replacement_style == pugi::xml_node{} ||
            replacement_style.parent() != staged_properties->replacement ||
            !staged_properties->replacement.remove_child(replacement_style)) {
            rollback();
            return false;
        }
...
bool Table::clear_style_look() {
    ...
    try {
        ...
        const auto replacement_table_look =
            staged_properties->replacement.child("w:tblLook");
        if (replacement_table_look == pugi::xml_node{} ||
            replacement_table_look.parent() != staged_properties->replacement ||
            !staged_properties->replacement.remove_child(
                replacement_table_look)) {
            rollback();
            return false;
        }
