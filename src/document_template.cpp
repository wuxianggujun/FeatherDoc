...
constexpr auto markup_compatibility_namespace_uri = std::string_view{
    "http://schemas.openxmlformats.org/markup-compatibility/2006"};
constexpr auto hyperlink_relationship_type =
    std::string_view{"http://schemas.openxmlformats.org/officeDocument/2006/"
...
bool hyperlink_relationship_id_is_used(pugi::xml_node node,
    ...
    for (auto current = node.first_child(); current != pugi::xml_node{};
        ...
        if (std::string_view{current.name()} == "w:hyperlink" &&
            std::string_view{current.attribute("r:id").value()} ==
                relationship_id) {
            return true;
        }
    }
...
bool rewrite_hyperlink_plain_text(pugi::xml_node hyperlink,
...
auto collect_block_bookmark_placeholders(
    featherdoc::document_error_info &last_error_info,
    pugi::xml_document &document, std::string_view entry_name,
    std::string_view bookmark_name,
    std::vector<block_bookmark_placeholder> &placeholders) -> bool {
    ...
    for (const auto bookmark_start : bookmark_starts) {
        ...
        if (paragraph == pugi::xml_node{} ||
            ...
            set_last_error(last_error_info,
                           ...
                           "block bookmark replacement requires the bookmark "
                           "to live directly "
                           "inside a paragraph",
        ...
        if (!paragraph_is_block_placeholder(paragraph, bookmark_start,
                                            bookmark_end)) {
            set_last_error(last_error_info,
                           std::make_error_code(std::errc::invalid_argument),
                           "block bookmark replacement requires the target "
                           "bookmark to occupy "
                           "its own paragraph",
                           std::string{entry_name});
            return false;
...
auto collect_bookmark_block_placeholders(
    ...
    for (const auto bookmark_start : bookmark_starts) {
        ...
        const auto bookmark_end =
            find_matching_bookmark_end_in_document_order(bookmark_start);
        if (bookmark_end == pugi::xml_node{}) {
            set_last_error(last_error_info,
                           std::make_error_code(std::errc::invalid_argument),
                           "bookmark block visibility requires a matching "
                           "bookmark end marker",
                           std::string{entry_name});
            return false;
        }
        ...
        if (start_paragraph == end_paragraph) {
            set_last_error(last_error_info,
                           std::make_error_code(std::errc::invalid_argument),
                           "bookmark block visibility requires start and end "
                           "markers to live "
        ...
        if (container == pugi::xml_node{} ||
            end_paragraph.parent() != container) {
            set_last_error(
                last_error_info,
                std::make_error_code(std::errc::invalid_argument),
...
auto rewrite_paragraph_plain_text(pugi::xml_node paragraph,
        ...
        if (std::string_view{child.name()} == "w:r") {
            ...
            if (source_run_properties != pugi::xml_node{}) {
                if (featherdoc::detail::checked_append_copy_xml_node(
                        source_run_properties, run_properties_storage) !=
                    featherdoc::detail::xml_document_clone_status::success) {
                    return false;
                }
                run_properties = run_properties_storage.first_child();
                if (run_properties == pugi::xml_node{} ||
                    std::string_view{run_properties.name()} != "w:rPr") {
                    return false;
                }
                break;
            }
    ...
    if (run_properties != pugi::xml_node{} &&
        featherdoc::detail::checked_append_copy_xml_node(
            run_properties, run) !=
            featherdoc::detail::xml_document_clone_status::success) {
        return false;
    }
